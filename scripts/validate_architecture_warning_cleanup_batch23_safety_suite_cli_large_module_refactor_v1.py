# project-path: scripts/validate_architecture_warning_cleanup_batch23_safety_suite_cli_large_module_refactor_v1.py
"""Validate Batch 23 safety-suite CLI large-module refactor."""
...
FEATURE_ID = "architecture-warning-cleanup-batch23-safety-suite-cli-large-module-refactor-v1"
PROJECT_ROOT = Path(__file__).resolve().parents[1]
TARGET_FILES = (
    Path("kanda_reasoner_app/safety_suite_cli/commands.py"),
    Path("kanda_reasoner_app/safety_suite_cli/commands_catalog_private.py"),
    Path("kanda_reasoner_app/safety_suite_cli/commands_parsers_private.py"),
    Path("kanda_reasoner_app/safety_suite_cli/commands_actions_private.py"),
    Path("kanda_reasoner_app/safety_suite_cli/commands_output_private.py"),
    Path("tests/test_architecture_warning_cleanup_batch23_safety_suite_cli_large_module_refactor.py"),
    Path("scripts/validate_architecture_warning_cleanup_batch23_safety_suite_cli_large_module_refactor_v1.py"),
)
LINE_COUNT_TARGETS = (
    Path("kanda_reasoner_app/safety_suite_cli/commands.py"),
    Path("kanda_reasoner_app/safety_suite_cli/commands_catalog_private.py"),
...
TEST_FILE = Path("tests/test_architecture_warning_cleanup_batch23_safety_suite_cli_large_module_refactor.py")
...
def _assert_private_helper_public_surface() -> None:
    for relative_path in PRIVATE_HELPER_TARGETS:
        ...
        value = getattr(all_node, "value", None)
...
def _run_characterization_test() -> None:
    ...
    spec = importlib.util.spec_from_file_location("batch23_safety_suite_cli_test", test_path)
    if spec is None or spec.loader is None:
        raise AssertionError("Unable to load characterization test module")
    module = importlib.util.module_from_spec(spec)
    sys.modules[spec.name] = module
    spec.loader.exec_module(module)
    module.test_batch23_safety_suite_cli_public_facade_still_runs()
...
def _run_architecture_validation() -> str:
    ...
    target_fragment = "MODULE_TOO_LARGE             kanda_reasoner_app/safety_suite_cli/commands.py"
    if target_fragment in output:
        raise AssertionError("Target large-module warning is still present")
