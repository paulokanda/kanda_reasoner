...
Use only the active KANDA prompt workspace path for install, validation, manifest, and handoff instructions. Any appearance of unrelated project roots as active install, validation, manifest, or handoff targets is a regression. If an unrelated project root appears as the active target, treat it as a regression and stop before implementation.
