---
prompt_id: A022
prompt_name: Working Effectively with Legacy Code Python Prompt
source_file: python_legacy_code_workflow.md
audit_chunk: 002
audit_decision: UPDATE
classification: SPECIALIST_PROMPT
status: audited_candidate_after_update
primary_owner: legacy-code stabilization, characterization testing, seams, safe incremental change
updated_by_audit: true
---
...
This prompt owns **legacy-code stabilization**: untested code, characterization tests, seams, dependency breaking, sprout/wrap techniques, scratch refactoring, and safe incremental change.

It complements but does not replace:

- **Testing Python**: owns general pytest strategy, property testing, mutation testing, fixtures, and QA structure.
- **Refactoring Python**: owns behavior-preserving structural improvement once a safety net exists.
- **Clean Code / Clean Architecture**: own general design quality and architectural direction after legacy behavior is pinned.
- **Sustaining Python**: owns versioning, deprecation, lifecycle, and long-term maintenance policy.

Rule: when code has no reliable tests, this prompt must run before ordinary refactoring or feature implementation.
...
- After writing a characterization test, add it to the test suite and prepare a logical checkpoint/commit when the human or workflow authorizes it.
...
Never make a change and refactor in the same step. The sequence:
    ...
    Prepare a logical checkpoint after each successful step; only create an actual VCS commit when the human or project workflow authorizes it.
...
### 9. Learn the Codebase with “Scratch Refactoring”
    ...
    Do not finalize or commit scratch work unless you have tests and explicit workflow permission. Delete the scratch after learning.
...
Anti‑Patterns in Legacy Code Work (Forbidden)
    ...
    ❌ Adding a feature and refactoring in the same change checkpoint – Impossible to debug failures.
