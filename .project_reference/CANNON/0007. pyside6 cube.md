...
Here's a complete PySide6 example that embeds an interactive 3D cube (using QtWebEngineWidgets and Three.js) inside a dashboard. Clicking the cube navigates to another page within the same application (e.g., switches a stacked widget). The cube rotates continuously – that's your "spinner".
Requirements

Install PySide6 with WebEngine:
bash
...
Create a file cube_dashboard_pyside.py:
python
...
How it works
    ...
    PySide6 WebEngine displays the HTML/Three.js cube as a full GUI component.

    The cube rotates continuously (spinner effect).

    When clicked, JavaScript changes window.location to a custom scheme pyside://cube_clicked.

    A QWebChannel intercepts that and emits a Python signal.

    The signal triggers a slot that switches the QStackedWidget to a second page.

    A back button returns to the cube.

Running the code
bash

python cube_dashboard_pyside.py

Make sure you have PySide6 and PySide6-WebEngine installed. The cube will appear in the window – click it to navigate.
Customization

    Change the destination page: modify self.other_page (e.g., add your own widgets, forms, plots).

    Adjust cube appearance: edit the material properties or lighting inside the HTML string.

    Add multiple cubes or other 3D objects by extending the Three.js script.

This gives you a fully functional, pure Python (PySide6) dashboard with an interactive 3D cube that navigates on click – no browser tabs needed, everything runs inside your desktop application.
