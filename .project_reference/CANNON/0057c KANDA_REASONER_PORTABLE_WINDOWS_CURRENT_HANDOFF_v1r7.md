...
Feature: `kanda-reasoner-portable-builder-install-v1r7`  
...
```text
PORTABLE BUILDER VERSION: v1r7
PORTABLE BUILDER FEATURE ID: kanda-reasoner-portable-builder-install-v1r7
```
...
## v1r7 installer failure-propagation correction

A prior installer could catch an installation failure, print an error, and
return control without a reliable nonzero status. That allowed an older
installed creator to remain active while later commands continued.

The canonical v1r7 installation entry is:

```text
INSTALL_AND_VERIFY.ps1
```

It must not report success until an external verifier has:

1. compared the exact installed member set with the installer manifest;
2. compared every installed file hash;
3. executed the installed static/functional validator;
4. executed the installed `--version` command;
5. confirmed v1r7 feature identity;
6. confirmed no `portable.__installing__*` folder remains in the project.

Any installation failure must propagate as a real PowerShell error. The prior
installed folder must be restored, and no success marker may be printed.

Before a long build, the creator must print:

```text
PORTABLE BUILDER VERSION: v1r7
PORTABLE BUILDER FEATURE ID:
kanda-reasoner-portable-builder-install-v1r7
```

If those lines are absent, stop. The active creator is stale.
