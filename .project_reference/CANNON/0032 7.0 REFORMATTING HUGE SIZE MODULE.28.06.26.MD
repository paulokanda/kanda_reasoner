...
The protocol must favor incremental, validated extraction over cosmetic reorganization, must favor deterministic AST evidence and refactor island manifests over manual guessing, and must favor planning several candidate islands together before implementation while keeping install, validate, and freeze strictly scoped. When several safe follow-up patches can be prepared from the same candidate queue, the AI may deliver them inside one outer patch-train bundle, but each inner patch remains a separate governed unit with its own install, validate, freeze, feature id, freeze hint, and rollback boundary.

When the existing module is very large and many modules remain to refactor, the protocol may use a faster wide-cluster strategy, but only by dependency/responsibility island. When two islands are proven independent by the Multi-Island Execution Layer, they may travel in one governed patch ZIP. When several such governed patches can be prepared safely, they may be delivered together as an outer patch-train bundle, but they must be installed, validated, and frozen in strict order. Do not split by physical halves, arbitrary line ranges, visual file position, or line-count-only targets.
...
* if this delivery is part of a patch train, state the inner patch id, the required predecessor patch, and the rule that later inner patches must not be installed until this patch has passed validation and freeze;
...
A patch train is an outer delivery ZIP that contains multiple separate governed patch ZIPs. It is a packaging convenience, not one giant patch and not a relaxation of validation or freeze discipline.

Use a patch train when several sequential patches can be safely prepared from one AST-assisted candidate queue or roadmap, such as:

```text
Outer delivery bundle
  Patch 16A: one island or one approved two-island batch
  Patch 16B: one island or one approved two-island batch
  Patch 16C: one island or one approved two-island batch
```

Rules:

* each inner patch has its own feature id, SHA, install script, validate script, freeze script, validator, freeze hint, and freeze entry;
* each inner patch may contain at most two islands, and only if that patch passes the Multi-Island Execution Layer independence test;
* the outer bundle must include an execution order manifest that lists each inner patch and its predecessor dependency;
* for one target module, Patch B is built against the result of Patch A, and Patch C is built against the result of Patch B;
* execution is strictly serial: install Patch A -> validate Patch A -> freeze Patch A, then install Patch B -> validate Patch B -> freeze Patch B;
* if any inner patch fails install, validation, or freeze, stop the train and do not proceed to later inner patches;
* do not freeze the outer bundle as one vague memory entry; freeze each inner patch separately with its own named islands and evidence;
* do not use a patch train to hide unrelated warning cleanup, behavior changes, or cross-module ambiguity.
...
The train may reduce download and chat friction, but it must not reduce traceability, attribution, rollback safety, or freeze quality.
...
* an outer patch-train bundle may contain several separate governed patch ZIPs, but each inner patch must be installed, validated, and frozen in strict sequence;
...
* do not write one freeze entry for an outer patch train; write one freeze entry per validated inner patch.
...
The audit should report unresolved names, missing imports, runtime API imports missing after split, Qt/GUI symbols leaking into non-GUI helpers, dependency-direction violations, facade/helper binding gaps, symbols present in sibling helpers but not imported, import smoke failures, dumping-ground helpers, and missed cross-island coupling in multi-island patches.
...
* Do not pair more than two islands in one patch.
