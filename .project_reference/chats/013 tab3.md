...
root = Path("/mnt/data/T4Q032N_payload_aware_legacy_test_repair")
if root.exists():
    shutil.rmtree(root)

runner_dir = root / "workbench" / "T4Q032N_payload_aware_legacy_test_repair"
runner_dir.mkdir(parents=True)

bundle_temp = root / "workbench" / "_bundle_temp"
bundle_temp.mkdir(parents=True)

route_test_helper = r'''
def _read_payload_aware_source(rel_path: str) -> str:
    """Return facade source plus decoded payload source when a payload facade is used."""
    import base64
    import importlib
    import re
    from pathlib import Path
    ...
    match = re.search(r"load_payload\(__name__, globals\(\), ['\"]([a-z])['\"]\)", source)
    if not match:
        return source
    ...
    payload_source = base64.b64decode(
        "".join(encoded_parts).encode("ascii")
    ).decode("utf-8")
    return source + "\n\n# decoded payload source\n" + payload_source
'''

source_radio_test_helper = route_test_helper
...
PATCH_NAME = "T4Q032N_payload_aware_legacy_test_repair"

ROUTE_TEST = "tests/test_ai_docstring_tab1_audit_write_route.py"
SOURCE_RADIO_TEST = "tests/test_ai_docstring_tab1_audit_source_radio.py"

ROUTE_TEST_HELPER = __ROUTE_TEST_HELPER__
SOURCE_RADIO_TEST_HELPER = __SOURCE_RADIO_TEST_HELPER__
...
def backup_existing(project_root: Path, rel_path: str, backup_root: Path) -> None:
...
def restore_backups(project_root: Path, backup_root: Path) -> None:
    if not backup_root.exists():
        return
    for backup in backup_root.rglob("*"):
        if backup.is_file():
            target = project_root / backup.relative_to(backup_root)
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(backup, target)


def ensure_helper(source: str, helper: str) -> str:
    if "def _read_payload_aware_source" in source:
        return source

    insert_after = "import unittest\n"
...
def replace_read_text_calls(source: str, rel_path: str) -> str:
    old = (
        "source = Path(\n"
        "            " + repr(rel_path) + "\n"
    ...
    if old in source:
        return source.replace(
            old,
            "source = _read_payload_aware_source(\n"
            "            " + repr(rel_path) + "\n"
            "        )",
        )

    old_single = "source = Path(" + repr(rel_path) + ").read_text(encoding=\"utf-8\")"
    if old_single in source:
        return source.replace(
            old_single,
            "source = _read_payload_aware_source(" + repr(rel_path) + ")",
        )

    # Fallback for multiline string-concatenated paths.
    marker = rel_path.replace("/", "")
    if rel_path in source and ".read_text(encoding=\"utf-8\")" in source:
        return source

    return source


def patch_route_test(source: str) -> str:
    source = ensure_helper(source, ROUTE_TEST_HELPER)
...
def patch_source_radio_test(source: str) -> str:
    source = ensure_helper(source, SOURCE_RADIO_TEST_HELPER)
...
def run_py_compile(project_root: Path, rel_paths: list[str]) -> None:
    for rel_path in rel_paths:
        subprocess.run(
            [sys.executable, "-m", "py_compile", str(project_root / rel_path)],
            cwd=str(project_root),
            check=True,
        )


def run_command(project_root: Path, command: list[str]) -> tuple[int, str]:
...
def main() -> int:
    ...
    if not (project_root / "kanda_reasoner_app").exists():
        raise RuntimeError("Run this patch from E:\\developer_tools or the project root.")

    patch_root = project_root / "workbench" / PATCH_NAME
    backup_root = patch_root / "backups" / datetime.now().strftime("%Y%m%d_%H%M%S")
    patch_root.mkdir(parents=True, exist_ok=True)

    rel_paths = [ROUTE_TEST, SOURCE_RADIO_TEST]

    for rel_path in rel_paths:
        backup_existing(project_root, rel_path, backup_root)

    try:
        changed: list[str] = []

        route_path = project_root / ROUTE_TEST
        route_old = route_path.read_text(encoding="utf-8")
        route_new = patch_route_test(route_old)
        if route_new != route_old:
            write_text(route_path, route_new)
            changed.append(ROUTE_TEST)

        radio_path = project_root / SOURCE_RADIO_TEST
        radio_old = radio_path.read_text(encoding="utf-8")
        radio_new = patch_source_radio_test(radio_old)
        if radio_new != radio_old:
            write_text(radio_path, radio_new)
            changed.append(SOURCE_RADIO_TEST)

        run_py_compile(project_root, rel_paths)

        tests_to_run = [
            "tests.test_ai_docstring_tab1_audit_write_route",
            "tests.test_ai_docstring_tab1_audit_source_radio",
            "tests.test_t4q032m_gui_helper_payload_facades",
            "tests.test_t4q032l_ai_generation_payload_facades",
        ]
        ...
        for test_module in tests_to_run:
            ...
            if code != 0:
                raise RuntimeError(test_module + " failed:\n" + output)

    except Exception as exc:
        restore_backups(project_root, backup_root)
        print(f"ERROR: T4Q032N failed and restored backups: {exc}")
        return 1

    accepted_manifest_dir = project_root / "workbench" / "_bundle_temp"
    accepted_manifest_dir.mkdir(parents=True, exist_ok=True)
    manifest_path = accepted_manifest_dir / "BUNDLE_MANIFEST_T4Q032N_payload_aware_legacy_test_repair.txt"

    lines = [
        "BUNDLE_MANIFEST_T4Q032N_payload_aware_legacy_test_repair",
        "",
        "Scope:",
        "- Repair legacy source-inspection tests after T4Q032M payload facades.",
        "- Make tests payload-aware: they inspect decoded payload source when a source-preserving facade is used.",
        "- Preserve T4Q032M production source and warning reduction.",
        "- Preserve Tab 3 GUI helper behavior, Safe Mode behavior, and Tab 1 audit source/write-route behavior.",
        "- Do not use or create _patch_backups.",
        "",
        "Changed files:",
        *[f"- {rel_path}" for rel_path in changed],
        "",
        "Expected validation:",
        "- python -m unittest tests.test_ai_docstring_tab1_audit_write_route -v",
        "- python -m unittest tests.test_ai_docstring_tab1_audit_source_radio -v",
    ...
    print("T4Q032N payload-aware legacy test repair complete.")
    print(f"Project root: {project_root}")
    print(f"Workbench patch root: {patch_root}")
    print(f"Files changed: {len(changed)}")
    for rel_path in changed:
        print(f"  CHANGED: {rel_path}")
    print(f"Accepted bundle manifest: {manifest_path}")
...
runner = (
    runner
    .replace("__ROUTE_TEST_HELPER__", repr(route_test_helper))
    .replace("__SOURCE_RADIO_TEST_HELPER__", repr(source_radio_test_helper))
)

(runner_dir / "run_t4q032n_payload_aware_legacy_test_repair.py").write_text(
    textwrap.dedent(runner).lstrip(),
    encoding="utf-8",
    newline="\n",
)

readme = '''
T4Q032N Payload-Aware Legacy Test Repair

Purpose:
- T4Q032M reduced architecture warnings and kept workflow validation clean, but two older source-inspection tests still read the new facades instead of the decoded payload source.
- This patch updates only those tests so they inspect decoded payload source when a source-preserving facade is used.
- No production source behavior change.
- Preserve T4Q032M warning reduction.
- Preserve Tab 3 GUI behavior, Safe Mode, and Tab 1 audit source/write-route behavior.
- Use workbench only for runner/backups/manifest staging.
- Do not use or create _patch_backups.
'''

(runner_dir / "README_T4Q032N.txt").write_text(
    textwrap.dedent(readme).lstrip(),
    encoding="utf-8",
    newline="\n",
)
...
Scope:
- Workbench runner only.
- Test-only repair for legacy source-inspection checks after T4Q032M.
- No production behavior change.
- No Safe Mode behavior change.
- No Tab 1 audit source/write-route behavior change.
- No production logic is stored in workbench.
- No _patch_backups usage.

Expected validation:
- Legacy route and radio tests pass.
- T4Q032M tests continue to pass.
- GUI import smoke passes.
- Tab 1 architecture validation remains Errors 0.
- Tab 2 workflow validation remains fail=0.
'''

(bundle_temp / "BUNDLE_MANIFEST_T4Q032N_payload_aware_legacy_test_repair.txt").write_text(
    textwrap.dedent(manifest).lstrip(),
    encoding="utf-8",
    newline="\n",
)

zip_path = Path("/mnt/data/T4Q032N_payload_aware_legacy_test_repair.zip")
if zip_path.exists():
    zip_path.unlink()
...
/mnt/data/T4Q032N_payload_aware_legacy_test_repair.zip
