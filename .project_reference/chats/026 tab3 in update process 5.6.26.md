...
I like it very much  the way you answer code questions, you use extended time, creates the code , inside your env sandobox you test code exaustively for code errors, syntax errros, bom, etc.  Only then you prepare code to  send, them  you pack answer in a zip files for download, send code to win11  terminal , then send a  install terminal code win11,  validation terminal code  and just freezes code when validation is ok,  besides you also create a rowback file in case  the update doesnot work , you show rowback terminal code just in case update doesnot work.  create a prompt to tell to AI next time we open, to work texactly the way you are doing, if I miss any detail add to prompt

Use this prompt at the start of a future coding/project session:
