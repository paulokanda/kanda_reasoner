...
def _rr_RuntimeCollectorWindow__configure_trace_impl(self) -> None:
    ...
    try:
        ...
        QMessageBox.critical(self, "Cannot create output directory", traceback.format_exc())
        return

    try:
        configure_runtime_trace(
            project_root=project_root or str(DEFAULT_PROJECT_ROOT),
            output_path=str(output_path),
            entry_script=entry_script,
        )
    except Exception:
        QMessageBox.critical(self, "Configure failed", traceback.format_exc())
...
def _rr_RuntimeCollectorWindow__save_trace_impl(self) -> None:
    if not self._trace_configured:
        QMessageBox.warning(
            self,
            "Not configured",
            "Please click 'Configure Trace' before saving.",
        )
        return
...
def _rr_RuntimeCollectorWindow__on_save_error_impl(self, tb_str: str) -> None:
    ...
    QMessageBox.critical(self, "Save failed", tb_str)
...
# PASS_065F_RUNTIME_PART3_BINDINGS START
...
DEFAULT_PROJECT_ROOT = _pass_073b_import_attr(
...
# PASS_065F_RUNTIME_PART3_BINDINGS END
