...
# PURPOSE       : Collector subprocess command building and lifecycle handling.
...
from kanda_reasoner_app.project_reasoner_v10.main_window_help.ui_components import (
    shorten_path,
)
...
class AnalysisController:
    ...
    def run_analysis(self, window) -> None:
        ...
        if not process.waitForStarted(3000):
            ...
            window._refresh_workflow_controls()
            QMessageBox.critical(window, "Analysis error", "Failed to start collector process.")
                ...
                if window.analysis_auto_load_checkbox.isChecked():
                    try:
                        ...
                        QMessageBox.warning(
                            window,
                            "Auto-load failed",
                            "Analysis finished but JSON auto-load failed:\n" + str(exc),
                        )
            else:
