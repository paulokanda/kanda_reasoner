...
The active artifact folder stores generated JSON files under the selected
project root in project_analysis_evidence. The hidden
.project_reference folder is reference-only and not project source. The legacy
_project_reference folder may be read by migration tools, but new reference
artifacts should use .project_reference when a reference folder is required.
"""
...
def project_analysis_evidence_root(project_root: str | Path) -> Path:
    """Return the canonical folder for generated project-analysis artifacts."""
    return Path(project_root) / PROJECT_ANALYSIS_EVIDENCE_DIR


def _normalize_project_analysis_evidence_dir_casing(project_root: str | Path) -> None:
    """Rename a legacy evidence folder casing to the canonical lowercase name."""
    root = Path(project_root)
    if not root.exists() or not root.is_dir():
        return

    canonical = root / PROJECT_ANALYSIS_EVIDENCE_DIR
    for child in root.iterdir():
        if not child.is_dir():
            continue
        if child.name.lower() != PROJECT_ANALYSIS_EVIDENCE_DIR:
            continue
        if child.name == PROJECT_ANALYSIS_EVIDENCE_DIR:
            return
        if canonical.exists() and not _paths_refer_to_same_location(child, canonical):
            return

        temporary = root / (PROJECT_ANALYSIS_EVIDENCE_DIR + ".case_rename_tmp")
        if temporary.exists():
            return
...
def analysis_json_complete_dir(project_root: str | Path) -> Path:
    """Return the canonical folder for complete generated JSON artifacts."""
    return project_analysis_evidence_root(project_root) / JSON_COMPLETE_DIR


def analysis_json_parts_dir(project_root: str | Path) -> Path:
    """Return the canonical folder for generated split JSON artifacts."""
...
def ensure_project_analysis_evidence_dirs(project_root: str | Path) -> Path:
    """Create and return the generated evidence root and child folders."""
    _normalize_project_analysis_evidence_dir_casing(project_root)
...
def relative_evidence_path(*parts: str) -> str:
    """Return a POSIX relative path inside the canonical evidence folder."""
    return str(
