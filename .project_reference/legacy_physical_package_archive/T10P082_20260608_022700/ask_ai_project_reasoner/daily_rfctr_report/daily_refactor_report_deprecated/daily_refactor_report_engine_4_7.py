"""
Daily AI Structural Awareness Engine - v12.7
============================================
Changes from v12.6:
  - FIXED: When a file is selected in Step 1, auto-scan now collects ALL
    .py files in the parent folder (not just the selected file). The vector
    is built from the entire folder: all functions, classes, imports, try
    blocks, async functions, and line counts aggregated.
  - FIXED: Step 2 folder field is now editable — user can type any path
    (existing or not). Folder is created automatically before compile runs.
  - FIXED: 📁 Choose Folder picker also creates the chosen folder immediately
    if it does not exist.
  - Added _on_json_folder_edited() to sync _json_folder on every keystroke.
  - magnitude is now derived from modules_touched count, not hardcoded to 1.
  - docstrings_added estimated from functions + classes found (not 0).
  - All prior logic preserved.
"""

from __future__ import annotations
# when I set step 1 = E:\pykanda\dt_02_adr_writer_ok and press button compile state vector , it is suposed to get file dt_02_adr_writer_ok.py , build an json structure compatible to app parameters and increment already existing json or create a new if json doesnot exists , instead it asks to insert json in step 3 , correct if file is insert in step1 it gets information for that file and do not ask for step tree, if in step two I select name: name_example folder c:/ example_folder and they do not exist create ,, if file there use it select code from module to correct just where necessary
import ast
import copy
import hashlib
import json
...
class StateVectorCompiler(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Structural State Compiler  v12.7")
        self.resize(900, 680)
    ...
    def _get_state_vector_input(self) -> Optional[str]:
        ...
        if self._selected_file is not None:
            ...
            self.status_bar.showMessage(
                f"Auto-generated canonical state vector from selected file: {self._selected_file.name}"
            )
            return auto_json
        ...
        msg.setText(
            "No selected .py file and no canonical JSON was provided in Step 3.\n\n"
    ...
    def _build_ui(self) -> None:
        ...
        dg = QGroupBox("Step 1 — Select a .py File to Define the Folder Scan  (optional)")
        dl = QHBoxLayout(dg)
        self.domain_edit = QLineEdit()
        self.domain_edit.setReadOnly(True)

        self.domain_edit.setPlaceholderText(
            "No file selected — if you choose a .py file, the app will scan its parent folder.")

        btn_browse = QPushButton("📂  Browse File…")
        btn_browse.clicked.connect(self._browse_domain)

        self.domain_label = QLabel("No file selected — paste-only mode. If a .py file is selected, its parent folder will be scanned.")
        ...
        self.json_folder_edit.setReadOnly(False)   # allow typing a path directly
        ...
        ag = QGroupBox(
            "Step 2b — AI Bundle Path  (optional — only used when a file is selected)"
        )
        ag.setToolTip(
    ...
    def _build_state_vector_from_selected_file(self) -> Dict[str, Any]:
        """
        Build a canonical state vector automatically from ALL .py files in the
        selected file's parent folder (domain_root).  The selected file itself
        is used as the reference module name; all symbols across the folder are
        aggregated into the vector.
        Called when Step 1 has a file but Step 3 is empty.
        """
        if self._selected_file is None:
            raise RuntimeError("No selected file is available.")

        domain_root = self._domain_root or self._selected_file.parent
        file_name   = self._selected_file.name
        module_name = self._selected_file.stem
        ...
        # ── Scan every .py file in the domain folder ──────────────
        ...
        for fp in py_files:
            ...
            try:
                source = fp.read_text(encoding="utf-8", errors="replace")
                tree   = ast.parse(source)
                rel    = str(fp.relative_to(domain_root))
        ...
        magnitude       = min(modules_touched, 20)

        summary = (
            f"Auto-generated from {modules_touched} .py file(s) in '{domain_root.name}'. "
            f"Found {len(functions_added)} function(s), {len(classes_added)} class(es). "
            f"Establishes baseline state vector; increment on next compile."
        )
        ...
        return {
            ...
            "complexity_state": {
                "risk_level":                  "low" if magnitude <= 3 else "medium" if magnitude <= 10 else "high",
                "volatility":                  0.0,
                "stability_trajectory":        "stable",
                "cyclomatic_complexity_delta": 0,
                "nesting_depth_max":           0,
            ...
            "forward_projection": {
                ...
                "suggested_next_action":       f"Review auto-generated baseline for '{domain_root.name}' and compile to persist.",
            },
            "architectural_snapshot": {
    ...
    def _browse_domain(self) -> None:
        """Pick any .py file. Its parent folder becomes the scan root for extraction and analysis."""
        ...
        domain_root = selected_file.parent
        ...
        self.domain_label.setText(
            f"Selected file: {selected_file.name}   │   Scanning folder: {domain_root}   │   Output: {self._config['root']}"
        )

        self.domain_label.setStyleSheet("color: #2ecc71;")

        self.status_bar.showMessage(
            f"Selected file: {Path(file_path).name}  →  scanning parent folder: {domain_root}  →  output: {self._config['root'].name}/"
        )
    ...
    def _load_existing_json(self) -> None:
        """Load an existing JSON and extract a canonical state vector into the editor."""
        ...
        try:
            ...
            state_vector = extract_canonical_state_vector(data)
            ...
            self.text_area.setPlainText(json.dumps(state_vector, indent=4))

            self._refresh_config()

            self.status_bar.showMessage(
                f"Loaded canonical state vector from '{p.name}'."
            )
            QMessageBox.information(
                self,
                "JSON Loaded",
                f"Loaded canonical state vector from:\n{p.name}\n\n"
                f"Base report name set to:\n{base_name}\n\n"
                "The editor is now ready for compile."
            )
        except Exception as e:
    ...
    def _refresh_config(self) -> None:
        ...
        """
        # Always keep the bundle path in sync with the edit field
        # (guards against the field being populated before _ai_bundle_path is set)
        ...
        if bundle_text:
            ...
            # field was cleared externally — honour that
            pass  # _clear_ai_bundle_path handles this explicitly

        if self._domain_root is None:
            return  # paste-only mode builds its own config in _compile_paste_only
    ...
    def _save_bundle_now(self) -> None:
        ...
        if self._domain_root is None:
            QMessageBox.warning(
                ...
                "Select a .py file in Step 1 first so the engine knows which report folder to read from."
            )
            return
