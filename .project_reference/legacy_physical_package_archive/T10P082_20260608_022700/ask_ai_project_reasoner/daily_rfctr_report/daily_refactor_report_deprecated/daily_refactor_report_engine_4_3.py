...
which can be redirected to a separate path via the "AI Bundle Path" field.
    ...
    ├── ai_upload_bundle.json            ← single file to give the AI
...
def build_config_named(
    ...
    # ai_upload_bundle.json destination — default to report_dir
    if ai_bundle_path is not None:
        bundle_path = ai_bundle_path
        bundle_path.parent.mkdir(parents=True, exist_ok=True)
    else:
        bundle_path = report_dir / "ai_upload_bundle.json"
...
def phase_ai_bundle(config: Dict) -> None:
    ...
    ai_upload_bundle.json — the one file to give the AI for full project
...
class StateVectorCompiler(QMainWindow):
    ...
    def _compile(self, force: bool = False) -> None:
        ...
        try:
            ...
            # Phase 2 — Extraction (skip if domain unchanged)
            self.status_bar.showMessage("Phase 2/7 — Checking domain fingerprint…")
            QApplication.processEvents()
            derived_data, was_skipped = phase_extraction(config, self._domain_root, force=force)

            if was_skipped:
                self.status_bar.showMessage(
                    "Phase 2/7 — CACHED: domain unchanged, skipping extraction & analysis…"
                )
            else:
                self.status_bar.showMessage("Phase 2/7 — Extracting project snapshot…")
            QApplication.processEvents()

            # Phase 3 — Analysis (skip if extraction was skipped)
            if not was_skipped:
            ...
            cache_note   = "  [extraction CACHED]" if was_skipped else "  [full extraction]"
            bundle_loc   = str(config["ai_bundle"])
            self.status_bar.showMessage(
                f"✓ Compiled successfully — {config['prefix']}  →  {config['root']}{cache_note}"
            )
            QMessageBox.information(
                self, "Success",
                ...
                f"{'⚡ Extraction CACHED (domain unchanged)' if was_skipped else '🔄 Full extraction ran (domain changed)'}"
            )
