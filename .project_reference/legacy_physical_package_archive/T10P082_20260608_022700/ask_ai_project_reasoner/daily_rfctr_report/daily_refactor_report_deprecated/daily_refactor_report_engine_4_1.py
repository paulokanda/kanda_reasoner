"""
Daily AI Structural Awareness Engine - v12.1
============================================
Changes from v12.0:
  - Smart skip logic: SHA-256 domain fingerprint saved after each run.
    On next compile, if no .py file in the domain has changed, phases 2
    (extraction) and 3 (analysis) are skipped entirely and cached derived
    data is reused. Only phases 4-7 always run (state vector + persistence
    + archive + AI bundle).
  - domain_fingerprint.json written to derived/ after each successful run.
  - Status bar reports "CACHED — skipping extraction" when skip fires.
  - Force-refresh button added to UI to bypass cache when needed.
  - All v12.0 logic preserved.
"""
...
class StateVectorCompiler(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Structural State Compiler  v12.1")
        self.resize(900, 680)
