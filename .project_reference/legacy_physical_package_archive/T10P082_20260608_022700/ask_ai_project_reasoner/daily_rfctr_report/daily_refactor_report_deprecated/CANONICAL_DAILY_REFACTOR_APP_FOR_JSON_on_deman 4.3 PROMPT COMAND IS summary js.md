...
user-chosen output folder. The ai_upload_bundle.json may optionally be
redirected to a separate user-defined path via the "AI Bundle Path" field
in the UI (Step 2b). The bundle metadata includes both "report_folder" and
"bundle_path" fields to record where each file lives.
