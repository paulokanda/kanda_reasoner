    ...
    def __init__(self) -> None:
        ...
        try:
            for index, spec in enumerate(TOOLS, start=1):
                ...
                self.tabs.addTab(page, f"{index}. {spec.step_title}")

            # ---------- ADD TAB 8: Project Exclusion Rules ----------
            self.ignore_rules_tab = IgnoreRulesTab(self._prefs_path())
            self.ignore_rules_tab.set_project_root(self.current_project_root)
            self.tabs.addTab(self.ignore_rules_tab, "8. Project Exclusion Rules")
            # --------------------------------------------------------

            # ---------- ADD TAB 9: Prompt Engineering Library --------
            self.prompt_library_tab = PromptLibraryTab()
            self.tabs.addTab(self.prompt_library_tab, "9. Prompt Engineering Library")
            # --------------------------------------------------------
        finally:
