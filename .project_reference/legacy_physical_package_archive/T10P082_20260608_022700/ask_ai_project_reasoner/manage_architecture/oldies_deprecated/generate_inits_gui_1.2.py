...
def generate_init_content(symbols_by_file):
    """
    Generate fresh __init__.py content.
    ...
    for rel_path, symbols in symbols_by_file.items():
        # Convert path to importable form (e.g., 'subdir.module')
        import_path = f'.{rel_path}'
        auto_imports.append(f'from {import_path} import {", ".join(sorted(symbols))}')
        for sym in symbols:
    ...
    if auto_imports:
        ...
        lines.append('')
    lines.append(f'__all__ = {sorted(all_symbols)}')
    lines.append('')
    lines.append(f'__module_map__ = {module_map}')
    lines.append('')
    return '\n'.join(lines)
