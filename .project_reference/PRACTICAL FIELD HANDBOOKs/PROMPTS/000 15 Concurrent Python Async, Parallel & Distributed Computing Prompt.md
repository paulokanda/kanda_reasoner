...
async def main():
    async with httpx.AsyncClient(timeout=5.0) as session:
        tasks = [fetch_user(session, uid) for uid in range(1, 101)]
        results = await asyncio.gather(*tasks, return_exceptions=True)
        # results contains either dict or Exception
        for r in results:
...
GIL removal future: Python 3.13 has experimental --disable-gil build (PEP 703). Not yet production default.
