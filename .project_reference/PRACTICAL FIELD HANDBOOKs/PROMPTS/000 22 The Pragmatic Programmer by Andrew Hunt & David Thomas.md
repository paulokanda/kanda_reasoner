# The Pragmatic Programmer Prompt for AI Code Generation (Python‑Centric, Pragmatic)
...
- Use assertions, type hints, and runtime checks (e.g., `validate_arguments` decorator) to enforce expectations.
- **Precondition**: what must be true before a function is called. Use `assert` or `if not condition: raise ValueError`.
- **Postcondition**: what must be true after. Check with `assert` or return value validation.
...
10. **Commit early, commit often** – Each commit is a logical, tested unit.
