Clean Code Python Prompt Creation

We need to create a prompt based on "Patterns of Enterprise Application Architecture" (PoEAA) by Martin Fowler. The user wants a state-of-the-art prompt with important features from that book, tailored for Python, respecting pragmatic use, modern language features, performance considerations, and pleasure to create. Must include warnings about overuse, language-specific solutions, functional alternatives, anti-patterns. Also note that this book adds value beyond other books. The prompt should complement previous prompts (Clean Code, Clean Architecture, Refactoring, Design Patterns). Focus on enterprise patterns: Data Mapper, Unit of Work, Repository, Identity Map, Transaction Script, Domain Model, Service Layer, Offline Concurrency, etc. Ensure Pythonic implementation, performance considerations, and clarity.

We'll produce a comprehensive prompt.
