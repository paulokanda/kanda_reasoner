# project-path: tools/validate_error_memory_duplicate_cleanup_v16.py
"""Focused validation for Error Memory duplicate cleanup v16."""
...
FEATURE_ID = "error-memory-duplicate-cleanup-v16"
FILES_UNDER_TEST = [
...
def _test_single_saved_match_is_not_duplicate() -> None:
...
def _test_pending_candidate_deletes_single_saved_draft() -> None:
    with tempfile.TemporaryDirectory() as temp:
        root = _project_root(Path(temp))
        ...
        assert result is not None, "pending plus one saved draft is a duplicate pair"
        assert result.deleted_lesson_ids == ("lesson-pending-family",)
        assert _ids(root) == set(), "saved draft duplicate must disappear from Lessons"


def _test_selected_saved_candidate_deleted_when_family_duplicate_exists() -> None:
    with tempfile.TemporaryDirectory() as temp:
        root = _project_root(Path(temp))
        candidate = _lesson(
        ...
        assert "lesson-batch20-batch19-runner-help-duplicate-public-symbols-v1" in result.deleted_lesson_ids
...
def main() -> int:
    ...
    _test_single_saved_match_is_not_duplicate()
    _test_pending_candidate_deletes_single_saved_draft()
    _test_selected_saved_candidate_deleted_when_family_duplicate_exists()
