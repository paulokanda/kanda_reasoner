...
FEATURE_MARKER = "architecture-review-large-file-refactor-workbench-project-support-boundary-v2"
...
def _validate_distinct_tool_and_project(sandbox: Path) -> None:
    """Prove another project never inherits the KANDA tool slug or support root."""
    ...
    assert support_root != sandbox / "kanda_reasoner_show_project_to_AI"
    assert preview_runs_root(project_root) == (
        support_root / "large_file_refactor_workbench" / "preview"
    )
    assert daily_work_root(project_root) == sandbox / "other_project_delete_after_daily_work"
    assert shadow_runs_root(project_root) == (
        daily_work_root(project_root) / "large_file_refactor_shadow"
    )
