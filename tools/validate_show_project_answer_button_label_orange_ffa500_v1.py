"""Validate show-project-answer-button-label-orange-ffa500-v1."""
...
FEATURE_ID = "show-project-answer-button-label-orange-ffa500-v1"
...
def validate_ui(root: Path) -> None:
    """Validate that only the button text label color is set to the requested orange."""
    path = root / UI_REL
    ...
    color_index = text.index('QColor("' + ORANGE_HEX + '")', set_color_index)
    ...
    assert_true(
        button_index < palette_index < set_color_index < color_index < set_palette_index < add_index,
    ...
    assert_true(ORANGE_HEX in text, "Button label color must use exact requested hex.")
    assert_true(OLD_ORANGE_HEX not in text, "Old International Orange hex must not remain in touched UI file.")
...
def validate_zip_contract(patch_root: Path) -> None:
    ...
    expected = {
        ...
        UI_REL,
        "tools/validate_show_project_answer_button_label_orange_ffa500_v1.py",
    }
    actual = {
