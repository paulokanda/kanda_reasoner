...
FEATURE_ID = "local-ai-first-time-help-v1r2"
...
def main() -> int:
    ...
    local_title_index = tool_specs.index('step_title="Local AI"')
    local_spec_start = tool_specs.rfind("ToolSpec(", 0, local_title_index)
    next_spec_start = tool_specs.find(
        "\n    ToolSpec(",
        local_title_index,
    )
    require(local_spec_start >= 0, "Local AI ToolSpec start not found.")
    require(next_spec_start >= 0, "Local AI ToolSpec end not found.")
    local_spec = tool_specs[local_spec_start:next_spec_start]
    require(
