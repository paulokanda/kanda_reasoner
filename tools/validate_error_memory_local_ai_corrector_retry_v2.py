# project-path: tools/validate_error_memory_local_ai_corrector_retry_v2.py
...
TOUCHED_FILES = [
    ...
    "tools/validate_error_memory_local_ai_corrector_retry_v2.py",
]
...
def _validate_model_resolution() -> None:
    ...
    display = service.resolve_local_ai_model("QN3 Coder 30B", FakeRegistry())
    fallback = service.resolve_local_ai_model("missing-model", FakeRegistry())
    candidates = service.get_local_ai_model_candidates(service.AUTO_LOCAL_AI_MODEL_LABEL, FakeRegistry())
    ...
    _assert(display == "qwen3-coder:30b", "display model label did not resolve")
    _assert(fallback == "qwen3-coder:30b", "missing model did not fall back safely")
    _assert(candidates == ["qwen3-coder:30b", "qwen2.5-coder:7b"], "auto candidates wrong")
...
def _validate_prompt_builder() -> None:
    ...
    for marker in [
        ...
        "Include superseded_by every time",
        "validation_evidence must be a JSON list",
    ]:
        _assert(marker in text, "prompt missing marker: " + marker)


def _validate_response_parser() -> None:
    """Validate strict marker parsing, normalization, and baseline checks."""
    ...
    normalizable = dict(lesson)
    normalizable.pop("superseded_by")
    normalizable["validation_evidence"] = "Expected marker: VALIDATION OK."
    normalizable["regression_check"] = dict(
        normalizable["regression_check"],
        command="python scripts\\validate_patch_zip.py E:\\kanda\\patch.zip",
    )
    normalized = validator.parse_one_lesson_block(validator.format_lesson_block(normalizable))
    _assert(normalized.lesson["superseded_by"] == "", "superseded_by was not defaulted")
    _assert(isinstance(normalized.lesson["validation_evidence"], list), "evidence not list")
    _assert("\\" not in normalized.lesson["regression_check"]["command"], "slashes not normalized")
    _assert("Missing superseded_by" in normalized.warning, "normalization warning missing")
...
def _validate_ai_retry_service() -> None:
    """Validate invalid first response is retried with validation feedback."""
    ...
    def fake_chat(messages: list[dict[str, str]], **kwargs: Any) -> tuple[str, str]:
        ...
        if len(calls) == 1:
            return validator_text(first), "qwen3-coder:30b"
        return validator_text(lesson), "qwen3-coder:30b"

    def validator_text(payload: dict[str, Any]) -> str:
        validator = importlib.import_module("kanda_reasoner_app.error_memory_gui._ai_response_validator")
        return validator.format_lesson_block(payload)
    ...
    try:
        result = corrector.correct_error_memory_lesson_with_local_ai(
            intake_text="INSTALL ERROR: sample",
            editor_text="",
            project_root="E:/kanda_reasoner",
    ...
    _assert(result.ok, "retry service did not return success")
    _assert(len(calls) == 2, "service did not retry exactly once")
    _assert("previous answer failed" in calls[1][-1]["content"], "retry feedback missing")
...
def main() -> int:
    ...
    _validate_ai_retry_service()
    _validate_header_delegates_to_corrector()
    print("VALIDATION OK: error-memory-local-ai-corrector-retry-v2")
    return 0
