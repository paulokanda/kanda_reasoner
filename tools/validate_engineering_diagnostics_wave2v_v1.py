# project-path: tools/validate_engineering_diagnostics_wave2v_v1.py
"""Brick Wall validator for Engineering Diagnostics Wave 2V."""
...
def _check_hashes(root: Path, values: dict[str, str], marker: str) -> None:
    validator_relative = "tools/validate_engineering_diagnostics_wave2v_v1.py"
...
def _run_historical_functional_contracts(root: Path) -> None:
    ...
    output = run_wave2v_command([sys.executable, str(wave2m)], cwd=root, markers=("Ran 4 tests", "OK"))
    _require("FAILED" not in output, "WAVE2M_FUNCTIONAL_TEST_FAILURE")
    print("WAVE2M FUNCTIONAL TEST SUITE: PASS")
    output = run_wave2v_command([sys.executable, str(wave2l)], cwd=root, markers=("Ran 3 tests", "OK"))
    _require("FAILED" not in output, "WAVE2L_FUNCTIONAL_TEST_FAILURE")
    print("WAVE2L CURRENT PERSISTENCE FUNCTIONAL CONTRACT: PASS")
    print("WAVE2U FROZEN PREDECESSOR FUNCTIONAL CONTRACT: PASS")


def _run_architecture(root: Path) -> None:
    from tools.engineering_diagnostics_wave2v_architecture_gate import validate_engineering_diagnostics_wave2v_architecture_non_regression
    from tools.engineering_diagnostics_wave2v_validation_runtime import run_wave2v_command
    output = run_wave2v_command(
...
def _write_evidence(root: Path, lines: list[str]) -> Path:
    ...
    target = boundary.active_project_support_root / "project_validation_evidence" / "engineering_diagnostics_wave2v" / "engineering_diagnostics_wave2v_v1.txt"
...
def main() -> int:
    ...
    from tools.engineering_diagnostics_wave2v_public_boundary_gate import validate_engineering_diagnostics_wave2v_public_boundary
    from tools.engineering_diagnostics_wave2v_validation_runtime import run_wave2v_focused_tests
    ...
    _require(_validator_hash() == VALIDATOR_CANONICAL_SHA256, "WAVE2V_VALIDATOR_CANONICAL_HASH_MISMATCH")
    print("WAVE2V VALIDATOR CANONICAL HASH: PASS")
    _check_hashes(root, _INSTALLED_HASHES, "WAVE2V INSTALLED HASHES: PASS")
    _check_hashes(root, _PROTECTED_HASHES, "WAVE2N THROUGH WAVE2U PROTECTED HASHES: PASS")
    validate_engineering_diagnostics_wave2v_public_boundary(root)
    _run_historical_functional_contracts(root)
    run_wave2v_focused_tests(root)
    ...
    _require(_optional_hash(database) == database_before, "WAVE2V_LIVE_DIAGNOSTIC_DATABASE_MUTATED")
    _require(_tree_hash(freeze_memory) == freeze_before, "WAVE2V_LIVE_FREEZE_MEMORY_MUTATED")
    _check_hashes(root, _INSTALLED_HASHES, "WAVE2V SOURCE MUTATION EXECUTED: NO")

    lines = [
        "PACKAGE REVISION: " + PACKAGE_REVISION,
        "FOCUSED PUBLIC CONTRACT TESTS: 180/180 PASS",
        "WAVE2U FROZEN PREDECESSOR FUNCTIONAL CONTRACT: PASS",
        "ENGINEERING SAFETY REAL QT TWO-LEVEL REGISTRATION: PASS",
        "FULL AUDIT REAL QT CONTROLS PRESERVED: PASS",
        "FULL AUDIT REAL QT SONAR BEHAVIOR PRESERVED: PASS",
        "PONTUAL AUDIT REAL QT PAGE PRESERVED: PASS",
        "PONTUAL AUDIT REAL QT ASYNC BEHAVIOR PRESERVED: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT RUN BUTTON: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT CANCEL BUTTON: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT SONAR START: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT SONAR CANCEL STATE: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT SONAR TERMINAL STOP: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT PUBLIC RUN STATE: PASS",
        "PONTUAL ENGINEERING DIAGNOSTICS REAL QT COOPERATIVE CANCEL: PASS",
        "FULL AUDIT DRILL-THROUGH PONTUAL TARGET: PASS",
        "WAVE2V GUI 25000 ROW INITIALIZATION MS: " + f"{rows_ms:.1f}",
        "WAVE2V GUI FILTER UPDATE MS: " + f"{filter_ms:.1f}",
        "WAVE2V GUI PANEL CREATION MS: " + f"{panel_ms:.1f}",
        "WAVE2V GUI HIGH-VOLUME PERFORMANCE: PASS",
        "WAVE2V NEW ARCHITECTURE ISSUES: 0",
        "WAVE2V TOUCHED PATH ARCHITECTURE ISSUES: 0",
        "PONTUAL AUDIT PRODUCT FILES MODIFIED: 0",
