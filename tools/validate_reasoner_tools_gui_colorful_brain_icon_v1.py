# project-path: tools/validate_reasoner_tools_gui_colorful_brain_icon_v1.py
"""Validate the KANDA Reasoner colorful brain application icon patch."""
...
FEATURE_ID = "reasoner-tools-gui-colorful-brain-icon-v1"
...
def _validate_python_sources(root: Path) -> None:
    """Validate the source-level application and window icon contracts."""
...
def _validate_icon_asset(root: Path) -> None:
    """Validate the exact colorful brain ICO and its multi-size inventory."""
    icon_path = root / ICON
    if not icon_path.is_file():
        raise AssertionError("COLORFUL_BRAIN_ICON_MISSING")
...
def _validate_qt_icon_runtime(root: Path, allow_missing_pyside6: bool) -> None:
    """Load the installed ICO through Qt using an offscreen application."""
...
def _validate_patch_zip(patch_zip: Path) -> None:
    """Confirm that the patch ZIP carries the exact validated payload files."""
    expected = {
        "payload/" + LAUNCH.as_posix(),
        "payload/" + ICON.as_posix(),
        "payload/tools/validate_reasoner_tools_gui_colorful_brain_icon_v1.py",
    }
    ...
    print("PATCH_ICON_PAYLOAD: PASS")


def main() -> int:
    """Run the focused static, asset, Qt, and package validations."""
    ...
    _validate_icon_asset(root)
    _validate_qt_icon_runtime(root, args.allow_missing_pyside6)
    if args.patch_zip:
