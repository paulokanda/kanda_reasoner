#!/usr/bin/env python3
"""Validate the compact Docstring Assistant layout and corrected draft-action order."""
...
def validate_static(root: Path) -> None:
    ...
    require(required_draft_order in layout, "After Correction draft-action order is incorrect")
    ...
    print("DOCSTRING_VISIBLE_DRAFTS_RIGHT_OF_STOP: PASS")
    print("DOCSTRING_UNDO_BULK_REPLACES_VISIBLE_POSITION: PASS")
...
def validate_runtime(root: Path) -> None:
    ...
    draft_rows = [item[1] for item in draft_column.items if item[0] == "layout"]
    require(len(draft_rows) == 2, "draft actions are not packed into two rows")
    first_draft_row = [item[1] for item in draft_rows[0].items if item[0] == "widget"]
    second_draft_row = [item[1] for item in draft_rows[1].items if item[0] == "widget"]
    require(
        first_draft_row == [
        ...
        "Generate Visible Drafts is not immediately right of Stop AI Drafts",
    )
    require(
        second_draft_row == [
        ...
        "Undo Last Bulk Drafts did not replace Generate Visible Drafts position",
    )
    ...
    print("DOCSTRING_DRAFT_ACTION_ORDER_RUNTIME: PASS")
...
def validate_real_qt(root: Path) -> None:
    ...
    try:
        ...
        stop_button = window._review_stop_ai_drafts_button
        visible_button = window._review_generate_visible_drafts_button
        undo_bulk_button = window._review_undo_bulk_drafts_button
        generate_all_button = window._review_generate_all_drafts_button
        stop_center = stop_button.mapTo(after_group, stop_button.rect().center())
        visible_center = visible_button.mapTo(after_group, visible_button.rect().center())
        undo_center = undo_bulk_button.mapTo(after_group, undo_bulk_button.rect().center())
        all_center = generate_all_button.mapTo(after_group, generate_all_button.rect().center())
        require(abs(stop_center.y() - visible_center.y()) <= 3,
                "Generate Visible Drafts is not on the Stop AI Drafts row")
        require(visible_center.x() > stop_center.x(),
                "Generate Visible Drafts is not right of Stop AI Drafts")
        require(abs(undo_center.y() - all_center.y()) <= 3,
                "Undo Last Bulk Drafts is not on the Generate All Drafts row")
        require(undo_center.x() < all_center.x(),
                "Undo Last Bulk Drafts did not take the former visible-drafts position")
        require(undo_center.y() > visible_center.y(),
                "Undo Last Bulk Drafts was not moved to the next row")
        ...
        print("REAL_QT_DOCSTRING_VISIBLE_DRAFTS_RIGHT_OF_STOP: PASS")
        print("REAL_QT_DOCSTRING_UNDO_BULK_POSITION: PASS")
...
def main() -> int:
    ...
    print("VALIDATION OK: docstring-assistant-compact-columns-v1r1")
    return 0
