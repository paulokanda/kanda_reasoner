# project-path: tools/validate_error_memory_duplicate_cleanup_v19.py
"""Focused validation for Error Memory duplicate cleanup v19."""
...
FEATURE_ID = "error-memory-duplicate-cleanup-v19"
FILES_UNDER_TEST = [
...
def _project_root(base: Path) -> Path:
    """Return an isolated Error Memory root hint for Windows and POSIX.

    On Windows, a temp project named kanda_reasoner resolves external
    Error Memory storage to E:/kanda_reasoner_show_project_to_AI, which is
    the real user project store on the same drive.  Use an explicit
    *_show_project_to_AI hint under the temp directory so the store remains
    isolated and the validator never reads or deletes real Lessons.
    """
...
def _assert_isolated_store(root: Path, temp_base: Path) -> None:
    ...
    temp_resolved = temp_base.resolve(strict=False)
    try:
        lessons_dir.relative_to(temp_resolved)
    except ValueError as exc:
        raise AssertionError(
            "validator Error Memory store escaped temp directory: "
            + str(lessons_dir)
        ) from exc
...
def main() -> int:
    ...
    assert DUPLICATE_CLEANUP_GUARD_VERSION == "v19_windows_isolated_validator_same_cleanup_contract"
    ...
    assert "v19_windows_isolated_validator_same_cleanup_contract" in guard_text
