"""Validate show-project-clean-2sec-2xenter-button-freeze-evidence-v2."""
...
FEATURE_ID = "show-project-clean-2sec-2xenter-button-freeze-evidence-v2"
...
VALIDATOR = "validate_show_project_clean_2sec_2xenter_button_freeze_evidence_v2.py"
...
def validate_freeze_script(patch_root: Path) -> None:
    """Validate freeze script performs real evidence merge, not marker-only output."""
    freeze = read_text(patch_root, "FREEZE.ps1")
    required = [
        ...
        "--patch-zip",
        "--evidence-file",
        "KANDA_VALIDATION_EVIDENCE_FILE",
        "KANDA_PATCH_ZIP",
        "validation_evidence_for_freeze.txt",
        "python $MergeScript",
    ]
    for item in required:
        assert_true(item in freeze, "FREEZE.ps1 missing real merge fragment: " + item)
    assert_true('Write-Host "FREEZE_HINT_EVIDENCE_MERGE_OK:' not in freeze, "FREEZE.ps1 must not fake the evidence merge marker.")
    assert_true('Write-Host "FREEZE_HINT_EVIDENCE_MERGE_OK' not in freeze, "FREEZE.ps1 must not fake the evidence merge marker.")
    assert_true("Freeze evidence merge failed" in freeze, "FREEZE.ps1 must fail when merge helper fails.")


def validate_patch_payload(patch_root: Path) -> None:
    """Validate exact patch package boundaries and scripts."""
    expected = {
        "INSTALL.ps1",
        "VALIDATE.ps1",
        "FREEZE.ps1",
    ...
    validate_freeze_script(patch_root)
    hint = json.loads(read_text(patch_root, "KANDA_FREEZE_HINT.json"))
    assert_true(hint.get("feature_id") == FEATURE_ID, "Freeze hint feature_id mismatch.")
    protected = "\n".join(str(item) for item in hint.get("protected_behavior") or [])
    assert_true("merge_freeze_validation_evidence.py" in protected, "Freeze hint must protect real evidence merge.")
