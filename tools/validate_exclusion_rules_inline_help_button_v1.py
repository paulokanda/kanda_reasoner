...
FEATURE_ID = "exclusion-rules-inline-help-button-v1"
...
def main() -> int:
    ...
    require(
        'self.exclusion_help_button = QPushButton("Help")' in tab,
        "Embedded Exclusion Rules Help button is missing.",
    )
    require(
        "self.exclusion_help_button.clicked.connect(" in tab
        and "self._open_exclusion_rules_help" in tab,
        "Embedded Help button connection is missing.",
    )
    require(
        "open_help_document_for_legacy_catalog(" in tab
        and '"exclusion_rules.json"' in tab
        and 'window_title="Help - Exclusion Rules"' in tab,
        "Embedded Help button does not use the canonical help route.",
    )
    ...
    require(scope_widget_marker in ui, "Scope-label layout marker missing.")
    require(help_widget_marker in ui, "Embedded Help layout marker missing.")
    require(
        ui.index(help_widget_marker) > ui.index(scope_widget_marker),
        "Embedded Help button is not positioned after the Scope label.",
    )
    print("EXCLUSION_RULES_INLINE_HELP_AFTER_SCOPE: PASS")
    print("EXCLUSION_RULES_INLINE_HELP_CANONICAL_ROUTE: PASS")
