...
from kanda_reasoner_app.error_memory_gui._lesson_library_lifecycle_mixin import (
    ErrorMemoryLessonLibraryLifecycleMixin,
)
from kanda_reasoner_app.error_memory_gui._mcard_machine_mixin import ErrorMemoryCardMachineMixin
...
from kanda_reasoner_app.error_memory.paths import resolve_second_prompt_files_root
...
class ErrorMemoryTab(ErrorMemoryLessonLibraryLifecycleMixin, ErrorMemoryCardMachineMixin, ErrorMemoryProjectPathsMixin, ErrorMemoryTableDraftMixin, ErrorMemoryIntakeActionsMixin, QWidget):
    """Error Memory lesson-library GUI with AI-assisted editor tools."""
    ...
    def __init__(self) -> None:
        ...
        self._build_ui()
        self.ERROR_MEMORY_ROW_KIND_ROLE = ERROR_MEMORY_ROW_KIND_ROLE
        self.ERROR_MEMORY_PENDING_PATH_ROLE = ERROR_MEMORY_PENDING_PATH_ROLE
        self._configure_mcard_ui()
        self._refresh_paths()
    ...
    def _build_ui(self) -> None:
        ...
        self.error_memory_info_button = QPushButton('Info')
        self.error_memory_info_button.setObjectName('error_memory_storage_info_button')
        self.error_memory_info_button.setToolTip(
            'Show Error Memory storage provenance and whether the current store is durable or a disposable Portable smoke copy.'
        )
        ...
        self.send_zip_errors_button = QPushButton('Send Error Lesson')
        self.send_zip_errors_button.setStyleSheet('color: #FF8C00; font-weight: bold;')
        self.send_zip_errors_button.setToolTip(
            'Copy the runtime-safe Error Memory delivery prompt. Source/IDE may use '
            'the self-contained ZIP workflow; Portable uses native marker-wrapped intake.'
        )
        self.project_selection_status_label = QLabel(
            "Error Memory lesson handling is separate from Machine-Card/MCard lifecycle. "
            "A selected Project is optional context only."
        )
        ...
        self.project_selection_status_label.hide()
        ...
        path_action_row.addWidget(self.error_memory_info_button)
        ...
        help_label = QLabel(
            'Paste raw error/context, import an AI-created formatted Error Lesson ZIP, '
            'or paste AI formatted JSON. In-app ZIP import is passive and does not '
            'launch a standalone Python interpreter. Operation phase is metadata stored '
            'inside the formatted lesson JSON, not a manual chooser.'
        )
        help_label.setWordWrap(True)
        intake_layout.addWidget(help_label)
        self.raw_error_edit = QPlainTextEdit()
        self.raw_error_edit.setPlaceholderText('Raw error, draft text, or formatted AI lesson appears here. Every parsed lesson admitted into the tab is normalized and saved in Lessons as Draft. Memorize Error promotes that same active-ready Draft to active. Mark Draft is optional for intentional draft persistence or demotion.')
        ...
        self.import_zip_button.setToolTip(
            'Native in-app lesson import. It does not require or launch an external '
            'python.exe from the Portable _internal folder.'
        )
        ...
        self.receive_formulary_button.setToolTip("Paste AI's formatted Error Memory JSON block; KANDA admits it into Lessons as Draft and loads it into the intake window and Error Editor. Memorize Error promotes that same active-ready Draft to active.")
        ...
        self.memorize_error_button.setToolTip('Single human draft-to-active promotion/commit action. Incoming lessons are Draft automatically. If the Draft is active-ready, Memorize Error saves it as active; otherwise it stays Draft and reports what is missing.')
        self.import_zip_button.setToolTip('Import an AI-created formatted Error Lesson ZIP/JSON/text file into Lessons as Draft and load it into the intake window and Error Editor. Memorize Error promotes that same active-ready Draft to active.')
        ...
        self.received_preview_edit.setPlaceholderText('After Paste error formatted from AI, Import Error Lesson ZIP, or pending intake admission, the lesson is saved in Lessons as Draft and appears here with status Draft. Edit if needed, then click Memorize Error to promote that same active-ready Draft to active. Mark Draft is optional for intentional draft persistence.')
        ...
        self.delete_button.setToolTip(
            'Delete the selected lesson from the Error Memory lesson library. '
            'Undo Delete is available until another delete.'
        )
        self.export_errors_button.setToolTip('Export the complete Error Memory lesson library as plain JSON.')
        self.import_errors_button.setToolTip('Import another Error Memory library file and merge only unique lessons.')
        ...
        self.deprecate_button = QPushButton('Retire')
        ...
        self.mark_draft_button.setToolTip('Optional: persist or return the selected lesson to draft. New incoming lessons already enter the tab as Draft; Mark Draft is not required before Memorize Error.')
        self.deprecate_button.setToolTip('Retire the selected lesson while retaining its lesson identity in the library.')
        ...
        self.lessons_table.setToolTip('Lessons stored in the Error Memory lesson library. To memorize is the transport queue; once loaded into the work surface, the candidate is shown as Draft until Memorize Error promotes it to active.')
        ...
        self.send_zip_errors_button.clicked.connect(
            lambda: copy_send_zip_errors_prompt_to_clipboard(self)
        )
        ...
        self.deprecate_button.clicked.connect(lambda: self._set_selected_lesson_status('retired'))
    ...
    def showEvent(self, event) -> None:
        """Refresh the Error Memory lesson library when the tab becomes visible."""
