"""
Daily AI Structural Awareness Engine - v12.4c
============================================
Changes from v12.4b:
  - Phase 7 (AI bundle write) is now in its own isolated try/except block.
    If any of phases 1-6 fail, compilation stops cleanly and the error is
    shown. If only phase 7 fails, a specific warning dialog shows exactly
    what path was attempted and why it failed — no more silent drops.
  - Belt-and-suspenders: config["ai_bundle"].parent.mkdir() called
    immediately before write_atomic in phase 7.
  - Added "🗂 Save Bundle Now" button — writes the bundle from the last
    successful compile without re-running the full pipeline.
...
class StateVectorCompiler(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        btn_bundle = QPushButton("🗂  Save Bundle Now")
        btn_bundle.setFixedHeight(42)
        ...
        btn_bundle.clicked.connect(self._save_bundle_now)

        btn_row.addWidget(btn_compile, 4)
        btn_row.addWidget(btn_force, 1)
        btn_row.addWidget(btn_bundle, 1)
    ...
    def _compile_paste_only(self, raw: str) -> None:
        ...
        name   = self.json_name_edit.text().strip() or "state_vector"
        name   = name.replace(".json", "")
        # Re-read bundle path from edit field directly
        bundle_text = self.ai_bundle_edit.text().strip()
        ai_bundle   = Path(bundle_text) if bundle_text else None
