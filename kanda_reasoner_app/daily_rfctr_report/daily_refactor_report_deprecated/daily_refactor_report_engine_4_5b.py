...
class StateVectorCompiler(QMainWindow):
    ...
    def _compile_full(self, raw: str, force: bool = False) -> None:
        ...
        # ── Rebuild config fresh from all current field values ────
        # This guarantees Step 2b bundle path override is always honoured,
        # even if _refresh_config was skipped or ran before Step 2b was set.
        folder = self._json_folder or self._domain_root
        name   = self.json_name_edit.text().strip() or self._domain_root.name
        name   = name.replace(".json", "")
        # Re-read bundle path from edit field directly — source of truth
        bundle_text = self.ai_bundle_edit.text().strip()
        ...
        self._config = config  # keep in sync
