"""
Daily AI Structural Awareness Engine - v12.6
============================================
Changes from v12.5:
  - AI bundle path edit is now editable (users can type a path directly).
  - Bundle path field and "Save Bundle Now" button are disabled when no file is selected.
  - After writing the bundle, the app verifies that the file actually exists.
  - Added tooltips to clarify that bundle requires a selected file.
...
class StateVectorCompiler(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Structural State Compiler  v12.6")
        self.resize(900, 680)
    ...
    def _build_state_vector_from_selected_file(self) -> Dict[str, Any]:
        """
        Build a canonical state vector template automatically from the selected .py file.
        This is used when Step 1 has a file but Step 3 is empty.
        """
        ...
        try:
            source = self._selected_file.read_text(encoding="utf-8", errors="replace")
            tree = ast.parse(source)
        ...
        summary = (
            f"Initial state vector auto-generated from selected file {file_name}. "
            f"This establishes a canonical starting point for report creation or increment."
        )
        ...
        return {
            ...
            "forward_projection": {
                ...
                "suggested_next_action": "Review the auto-generated state vector and compile to create or increment the report."
            },
            "architectural_snapshot": {
