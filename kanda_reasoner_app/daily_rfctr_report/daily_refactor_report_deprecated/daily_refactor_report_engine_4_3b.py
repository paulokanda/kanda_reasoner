"""
Daily AI Structural Awareness Engine - v12.2
============================================
Changes from v12.1:
  - Separate "AI Bundle output path" field added to UI (Step 2b).
    ai_upload_bundle.json can now be written to any folder independently
    of the main report folder.  Defaults to the report folder when left blank.
  - build_config_named() and phase_ai_bundle() updated to accept the
    optional ai_bundle_path override.
  - Full file-layout docstring added to build_config_named() explaining
    where every output file is stored and why.
  - engine_version tag corrected to "v12.2" in bundle metadata.
  - All v12.1 logic preserved.

OUTPUT FILE LAYOUT
==================
Everything is written into one report sub-folder auto-created under the
output folder chosen in Step 2.  The only exception is ai_upload_bundle.json,
which can be redirected to a separate path via the "AI Bundle Path" field.

    <output_folder>/<prefix>_report/
    │
    ├── <prefix>_current_state.json      ← latest compiled state vector
    ├── <prefix>_state_history.json      ← rolling window of last 7 vectors
    ├── <prefix>_daily_refactor_report.json  ← single file to give the AI
...
====================
  ...
  ai_upload_bundle.json   — the single file to upload to the AI; contains
...
def build_config_named(
    domain_root:    Optional[Path], # Now explicitly Optional
    output_folder:  Path,
    json_name:      str,
    ...
    json_name      : stem used for all filenames, e.g. "my_report"
                     → report folder : <output_folder>/my_report_report/
                     → state file    : my_report_current_state.json
    ai_bundle_path : optional override for the ai_upload_bundle.json
    ...
    if json_name:
        ...
        prefix = domain_root.name
    else:
        prefix = "manual_compile"
...
def phase_persistence(config: Dict, merged: Dict) -> None:
    # 1. Attempt to load existing history. If missing, return empty list.
    history = safe_load_with_recovery(config["history"], config["log"]) or []

    # 2. Add the new pasted/processed data
    history.append(merged)

    # 3. Maintain the rolling window (last 7 entries)
    history = history[-ROLLING_WINDOW:]

    # 4. Atomic write (Creates the file if it doesn't exist)
...
def phase_ai_bundle(config: Dict) -> None:
    derived = config["root"] / "derived"
    log = config["log"]
    bundle_path = Path(config["ai_bundle"])

    # Ensure the directory exists (Path B safety)
    bundle_path.parent.mkdir(parents=True, exist_ok=True)

    # Load what we have. If derived files are missing (Paste-only mode),
    # _load_with_status returns empty dicts and "missing" tags.
    ...
    integrity = hashlib.sha256(json.dumps(core, sort_keys=True).encode("utf-8")).hexdigest()

    bundle = {
        "metadata": {
            "generated_at":   datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
            "engine_version": "v12.2", # Updated version tag
    ...
    # Atomic write ensures the file is created or overwritten correctly
...
class StateVectorCompiler(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Structural State Compiler  v12.2")
        self.resize(900, 680)
    ...
    def _build_ui(self) -> None:
        ...
        ag.setToolTip(
            "When set, ai_upload_bundle.json is written to this exact path instead of\n"
            "the default <prefix>_report/ folder.  Useful when you want to keep the\n"
            "AI context file in a dedicated 'AI uploads' folder separate from the\n"
            "engine's internal report files."
        )
        al = QHBoxLayout(ag)
        ...
        self.ai_bundle_edit.setPlaceholderText(
            "Leave blank → ai_upload_bundle.json saved inside <prefix>_report/ …"
        )

        btn_ai_bundle_file   = QPushButton("💾  Set Bundle Path…")
        btn_ai_bundle_file.setToolTip("Choose the exact file path for ai_upload_bundle.json")
        ...
        self.status_bar.showMessage(
            "Step 1: select domain  →  Step 2: name/load JSON  →  Step 3: paste full canonical vector & Compile"
        )
    ...
    def _choose_ai_bundle_path(self) -> None:
        """Let user pick the exact file path for ai_upload_bundle.json."""
        default_name = "ai_upload_bundle.json"
    ...
    def _refresh_config(self) -> None:
        """
        Rebuilds config. Step 2 (Folder/Name) is now the primary driver.
        Step 1 (Domain) is now an optional data source.
        """
        # Resolve Folder: User-chosen folder > Domain Root > User Home
        folder = self._json_folder or (self._domain_root if self._domain_root else Path.home())

        # Resolve Name: User-chosen name > Domain Name > "state_vector"
        name = self.json_name_edit.text().strip()
        if not name:
            name = self._domain_root.name if self._domain_root else "manual_compile"

        name = name.replace(".json", "")
        self._json_name = name

        # This function (v12.2+) handles creating the paths
        self._config = build_config_named(
            domain_root=self._domain_root,  # Can be None now
    ...
    def _compile_force(self) -> None:
        """Force full extraction + analysis, bypassing the domain cache."""
        self._compile(force=True)


    def _compile(self, force: bool = False) -> None:
        """
        Dual-mode compiler. If Step 1 is empty, creates 'manual_compile' report.
        """
        self._refresh_config()  # This now calls the updated build_config_named

        if self._config is None:
            QMessageBox.warning(self, "Config Error", "Step 2 Configuration failed.")
            return
        ...
        if not raw:
            QMessageBox.warning(self, "Empty Input", "Please paste JSON in Step 3.")
            return

        try:
            # --- PHASE 1: Parse Paste ---
            parsed = json.loads(raw)
            vector = parsed[0] if isinstance(parsed, list) else parsed
            validate_schema(vector)
            merged = recompute_fields(vector)

            # --- PHASE 2: Branching Extraction ---
            if self._domain_root:
                # MODE: File Scan
                phase_lock(config)
                derived_data, was_skipped = phase_extraction(config, self._domain_root, force=force)
                analysis_outputs = phase_analysis(config, self._domain_root, derived_data, was_skipped)
                merged.update(analysis_outputs)
            else:
                # MODE: Manual Paste
                self.status_bar.showMessage("Manual Mode: Skipping file scan...")

            # --- PHASE 3: Persist & Increment ---
            # This creates the .json files in Path B if they don't exist
            phase_persistence(config, merged)

            # --- PHASE 4: ALWAYS CREATE BUNDLE ---
            # This was previously hidden inside the 'if self._domain_root' block.
            # Moving it here ensures the manual_compile report is created.
            self.status_bar.showMessage("Building AI Bundle Report...")
            phase_ai_bundle(config)

            # --- PHASE 5: UI Reset ---
            self.text_area.clear()
            self.status_bar.showMessage(f"✓ Report created: {config['ai_bundle'].name}")
