"""
Daily AI Structural Awareness Engine - v12.0
============================================
Single-file edition — all helper logic inlined, no external imports.

Changes from v11.0:
  - Expanded canonical JSON structure to capture MAXIMUM information from
    each code period analysed (aligns with CANONICAL_DAILY_REFACTOR prompt v2.0+)
  - structural_change: added files_modified, functions_added/modified/removed,
    classes_added/modified, lines_added, lines_removed
  - complexity_state: added cyclomatic_complexity_delta, nesting_depth_max,
    coupling_degree
  - governance_state: added breaking_changes, backward_compatible,
    test_coverage_impact
  - refactor_pressure: added technical_debt_notes
  - pattern_dynamics: added design_patterns_detected, anti_patterns_detected
  - forward_projection: added next_likely_hotspot, suggested_next_action
  - architectural_snapshot: added dependencies_introduced/removed, imports_added,
    constants_added, global_state_mutations, integration_points_touched,
    localization_changes, side_effects_introduced, exception_handling_changes,
    docstrings_added, type_hints_added, async_changes, summary_narrative
  - CANONICAL_KEYS updated and CANONICAL_SUB_KEYS enforced per sub-object
  - validate_schema() updated to enforce all new sub-keys
  - recompute_fields() updated to enforce all new enforcement rules
  - Placeholder text updated to show full new structure
  - All other logic (lock, backup rotation, corruption recovery, diff viewer,
    archive retention, AI upload bundle, integrity hash) fully preserved.
"""
...
class StateVectorCompiler(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Structural State Compiler  v12.0")
        self.resize(900, 680)
