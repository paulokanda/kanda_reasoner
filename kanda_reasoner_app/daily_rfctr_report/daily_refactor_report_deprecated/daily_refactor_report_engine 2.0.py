...
def build_config(domain_root: Path) -> Dict[str, Any]:
    """
    Derive all paths from a selected domain root folder.
    Output lives in  <domain_root>/daily_refactor_report/
    """
    prefix     = domain_root.name          # use folder name as prefix
    report_dir = domain_root / "daily_refactor_report"
    ...
    return {
        "prefix":  prefix,
        "root":    report_dir,
        "current": report_dir / "current_state.json",
        "history": report_dir / "state_history.json",
...
class StateVectorCompiler(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        dg = QGroupBox("Domain (select any .py file inside the target folder)")
        dl = QHBoxLayout(dg)

        self.domain_edit = QLineEdit()
        self.domain_edit.setReadOnly(True)
        self.domain_edit.setPlaceholderText("No domain selected — click Browse…")
        ...
        # ── State vector JSON input ──────────────────────────────
        sv_group = QGroupBox("State Vector JSON  (paste here, then click Compile)")
        ...
        self.status_bar.showMessage(
            "Ready — select a domain folder, paste your state vector JSON, then Compile."
        )
    ...
    def _browse_domain(self) -> None:
        """
        User picks any .py file inside the target domain folder.
        The engine uses that file's parent directory as domain_root.
        """
        ...
        self._config      = build_config(domain_root)

        self.domain_edit.setText(str(domain_root))
        self.domain_label.setText(
            f"Domain: {domain_root.name}   "
            f"│  Output: {self._config['root']}"
        )
        self.domain_label.setStyleSheet("color: #2ecc71;")
        self.status_bar.showMessage(
            f"Domain set: {domain_root.name}  →  output in daily_refactor_report/"
        )
    ...
    def _compile(self) -> None:
        if self._domain_root is None or self._config is None:
            QMessageBox.warning(
                self, "No Domain",
                "Please select a domain folder first (Browse button)."
            )
            return
