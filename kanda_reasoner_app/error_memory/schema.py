...
"""Lightweight schema helpers for Error Memory lessons."""
...
def write_schema_if_missing(selected_project_root: str | Path) -> Path:
    """Write the local JSON schema file when missing."""
    path = resolve_lesson_schema_path(selected_project_root)
    path.parent.mkdir(parents=True, exist_ok=True)
    if not path.exists():
        path.write_text(json.dumps(LESSON_SCHEMA, indent=2, sort_keys=True), encoding="utf-8")
    return path
...
def validate_lesson_shape(lesson: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if str(lesson.get("status", "")) not in {"draft", "active", "deprecated", "superseded"}:
    ...
    if str(lesson.get("status", "")).strip().lower() == "active":
        failures.extend(reason for reason in active_ready_missing_reasons(lesson) if reason not in failures)
