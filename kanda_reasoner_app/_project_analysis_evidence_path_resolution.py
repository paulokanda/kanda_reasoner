# project-path: kanda_reasoner_app/_project_analysis_evidence_path_resolution.py
"""Path-resolution helpers for project-analysis evidence artifacts."""
...
PROJECT_STRUCTURE_3D_STATE_DIR = "project_structure_3d"
...
def _canonical_show_project_root(output_root: Path) -> Path:
    """Return the drive-root sibling when a show-project root is nested."""
    if not output_root.name.endswith(SHOW_PROJECT_TO_AI_SUFFIX):
        return output_root

    base_name = output_root.name[: -len(SHOW_PROJECT_TO_AI_SUFFIX)].strip()
    parent_slug = make_safe_slug(output_root.parent.name)
    if base_name and make_safe_slug(base_name) == parent_slug:
        return _folder_name_for_slug(output_root.parent, parent_slug)

    return output_root


def _canonical_show_project_root_for_legacy_child(path: Path) -> Path:
    """Return the external show-project root for an in-source legacy child."""
    return _folder_name_for_slug(path.parent, make_safe_slug(path.parent.name))


def _canonical_show_project_root_for_legacy_state_child(path: Path) -> Path:
    """Return the external show-project root for a nested in-source state child."""
    return _folder_name_for_slug(path.parent.parent, make_safe_slug(path.parent.parent.name))
...
def show_project_to_ai_root_from_hint(project_root: str | Path) -> Path:
    ...
    if root.name in state_children:
        ...
        if parent.name in {PROJECT_ERROR_MEMORY_DIR, PROJECT_FREEZE_AFTER_UPDATE_DIR} and parent.parent.name.endswith(SHOW_PROJECT_TO_AI_SUFFIX):
            return _canonical_show_project_root(parent.parent)
        if parent.name in {PROJECT_ERROR_MEMORY_DIR, PROJECT_FREEZE_AFTER_UPDATE_DIR}:
            return _canonical_show_project_root_for_legacy_state_child(root)
    ...
    if root.name in {PROJECT_ERROR_MEMORY_DIR, PROJECT_FREEZE_AFTER_UPDATE_DIR} and root.parent.name.endswith(SHOW_PROJECT_TO_AI_SUFFIX):
        return _canonical_show_project_root(root.parent)
    ...
    if root.name in show_children:
        return _canonical_show_project_root_for_legacy_child(root)

    if root.name.endswith(SHOW_PROJECT_TO_AI_SUFFIX):
        return _canonical_show_project_root(root)
