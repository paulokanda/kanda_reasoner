...
from PySide6.QtWidgets import QApplication, QComboBox, QDialog, QFileDialog, QGridLayout, QGroupBox, QHBoxLayout, QLabel, QLineEdit, QMessageBox, QPushButton, QRadioButton, QTextEdit, QVBoxLayout, QWidget
from kanda_reasoner_app.freeze_after_update.contract import ensure_freeze_after_update_box, generate_freeze_after_update_ai_files, inspect_freeze_after_update_box, preview_freeze_entry, refresh_freeze_exposure, refresh_ai_compliance_context, validate_freeze_entry_preview, write_confirmed_freeze_entry
from kanda_reasoner_app.freeze_hint_intake import build_freeze_form_inputs_from_latest_hint, mark_latest_freeze_hint_used
...
def _local_ai_response_looks_like_prompt_echo(response_text: str) -> bool:
    ...
    echo_markers = ['You are a specialist in KANDA Reasoner', 'Context:\nI just implemented', 'Task:\nReview and correct', 'Critical rules:', 'Return format - strict copy/paste contract', 'Copy/paste-ready answer shape:', 'Current auto-filled form JSON']
...
def _validate_local_ai_form_against_heuristic(candidate: dict, heuristic: dict) -> tuple[bool, list[str]]:
    ...
    forbidden_echo_markers = ['You are a specialist in KANDA Reasoner', 'Return format - strict copy/paste contract', 'Copy/paste-ready answer shape', 'Current auto-filled form JSON']
    ...
    line_preserve_fields = ['validated_files', 'generated_files', 'protected_paths', 'do_not_regress_rules', 'validation_evidence_summary']
...
class LocalFreezeAIFormularyRunner:
    ...
    def run(self) -> tuple[bool, str, str]:
        ...
        try:
            ...
            if not model_name:
                raise RuntimeError('No local Ollama model is available. Use Heuristics mode or refresh/install a local model.')
            ai_class = _load_local_ai_class(LOCAL_AI_MODULE, LOCAL_AI_CLASS)
            ai = ai_class()
            response_text = ai.chat(_build_local_ai_freeze_form_messages(self._inputs, self._project_root), model=model_name, temperature=0.05, max_tokens=2600, use_cache=False)
...
class FreezeAfterUpdateTab(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        intro = QLabel('Prepare or execute the project-local freeze / AI-compliance flow after a validated update. Normal freezing happens locally through preview and human confirmation. External AI review export remains available as an advanced fallback. Frozen memory stays inside the actual project.')
        ...
        status_help = QLabel('Check Box Status inspects the project-local freeze box and reports whether it is valid, missing, or incomplete. Create / Repair Box creates missing safe folder structure without writing frozen memory.')
        ...
        staged_help = QLabel('Click Prepare first. The log will show exactly what will be read, written, refreshed, and not touched. Nothing changes until Do it.')
        ...
        local_freeze_help = QLabel('Create a local freeze entry from this tab. The form now defaults to Local AI auto-fill when a local model is available, with a safe heuristic fallback. It immediately builds a preview and then waits for human confirmation before writing.')
    ...
    def move_project_root_controls_to_layout(self, target_layout: QHBoxLayout, insert_index: int | None=None) -> None:
        """Move freeze path controls into the outer LOADED/Source row."""
        if self._project_root_controls_moved:
            return
        controls = [self.project_root_header_label, self.project_root_edit, self.search_project_button, self.box_folder_button, self.external_ai_review_folder_button, self.help_button]
    ...
    def _copy_last_freeze_snippet(self) -> None:
        ...
        show_auto_close_action_window(self, title='Get Last Freeze', message='Latest frozen feature snippet copied to clipboard.')
    ...
    def _copy_all_frozen_snippets(self) -> None:
        ...
        show_auto_close_action_window(self, title='Get All Frozen', message='All frozen feature snippets copied to clipboard.')
    ...
    def _refresh_derived_paths(self) -> None:
        ...
        if paths is None:
            ...
            self.external_ai_review_folder_button.setToolTip('Select a project root before opening the external AI review folder')
            return
    ...
    def _show_result(self, result: FreezeAfterUpdateResult) -> None:
        ...
        if result.box_root is not None:
            ...
            self.external_ai_review_folder_button.setToolTip('Open external AI review folder: ' + str(result.box_root / 'files_to_send_ai'))
        ...
        if result.output_zip is not None:
            ...
            self.external_ai_review_folder_button.setToolTip('Open external AI review folder: ' + str(result.output_zip.parent))
        ...
        if result.output_instruction is not None:
            ...
            self.external_ai_review_folder_button.setToolTip('Open external AI review folder: ' + str(result.output_instruction.parent))
    ...
    def _build_heuristic_local_freeze_inputs(self, project_root: Path) -> dict:
        ...
        validation evidence.
        """
        fallback_inputs = {'feature_title': 'Current validated feature - replace with exact feature title', 'primary_box': 'Replace with the primary box for the current feature', 'box_type': 'Replace with the current box type', 'validated_files': '', 'generated_files': '', 'protected_paths': 'project_freeze_after_update/frozen_features_memory/', 'do_not_regress_rules': '\n'.join(['Do not freeze without current feature validation evidence.', 'Preserve the current feature behavior validated by the user.', 'Keep project-specific frozen memory under <project>_show_project_to_AI/project_freeze_after_update/frozen_features_memory.', 'Do not store project-specific frozen memory inside project_freeze_ledger.', 'Do not replace current feature data with stale legacy workflow data.']), 'validation_evidence_summary': '', 'known_warnings': 'Starter draft only. Replace placeholders with the current validated feature data. No validation evidence has been inferred or invented.', 'planned_next_step': 'Replace placeholders with current feature evidence, then Preview Freeze Entry before Confirm and Write.', 'notes': 'Auto-filled by the Freeze Feature After Update tab as a safe current-feature starter. This draft intentionally avoids stale legacy freeze-workflow titles, paths, and validation markers.'}
        try:
            return build_freeze_form_inputs_from_latest_hint(project_root, fallback_inputs)
        except Exception as exc:
            recovered = dict(fallback_inputs)
            recovered['known_warnings'] = (str(recovered.get('known_warnings', '')).strip() + ' Freeze hint intake was unavailable, so the safe starter draft was kept. Error: ' + str(exc)).strip()
            return recovered
    ...
    def _open_local_freeze_entry_dialog(self) -> None:
        ...
        intro = QLabel('Local freeze entry workflow: choose Heuristics or Local AI. Heuristics is the default because it is deterministic and validated. Local AI remains optional for testing and can improve the heuristic draft only when it passes quality gates. Preview is read-only; Confirm and Write still requires human confirmation.')
        intro.setWordWrap(True)
        ...
        validation_evidence_edit.setPlaceholderText('Paste validation output. Must include a marker such as VALIDATION OK, INSTALL OK, py_compile passed, validator passed, or STATUS: IN_SYNC.')
        ...
        for editor in [validated_files_edit, generated_files_edit, protected_paths_edit, do_not_regress_edit, validation_evidence_edit, known_warnings_edit, planned_next_step_edit, notes_edit]:
        ...
        def parse_ai_formulary_response(text: str) -> dict:
            ...
            allowed = {'feature_title', 'primary_box', 'box_type', 'validated_files', 'generated_files', 'protected_paths', 'do_not_regress_rules', 'validation_evidence_summary', 'known_warnings', 'planned_next_step', 'notes'}

            def normalize_ai_text(raw_text: str) -> str:
                return (raw_text or '').replace('\ufeff', '').replace('\u200b', '').replace('\u200c', '').replace('\u200d', '').replace('\xa0', ' ').strip()
            ...
            if not parsed_payloads:
                raise ValueError('Could not find a usable freeze-form JSON object in the AI answer. The receiver searched marker payloads, fenced JSON, balanced JSON objects, and broad fallback slices.')
            ...
            for key in allowed:
                ...
                if isinstance(value, list):
                    cleaned[key] = '\n'.join((normalize_common_ai_markdown_damage(str(item).strip()) for item in value if str(item).strip()))
    ...
    def _show_what_to_say_window(self) -> None:
        ...
        if not what_to_say_path.exists():
            QMessageBox.warning(self, 'Instruction file missing', f'The instruction file does not exist yet.\n\nThe deprecated external AI review export button has been removed. Use Show Project to AI artifacts for full external review.\n\nExpected path:\n{what_to_say_path}')
            return
        try:
    ...
    def _render_help_html(self) -> str:
        ...
        return '<!doctype html>\n<html>\n<head>\n<meta charset="utf-8">\n<style>\nbody {\n    font-family: "Segoe UI", "Aptos", "Calibri", sans-serif;\n    font-size: 14px;\n    line-height: 1.55;\n    color: #243043;\n    background: #fffaf2;\n    margin: 0;\n    padding: 0;\n}\n.page { padding: 18px 22px 26px 22px; }\nh1 { color: #16213e; font-size: 26px; margin: 0 0 8px 0; }\nh2 { color: #0f766e; font-size: 20px; margin-top: 26px; border-bottom: 2px solid #99f6e4; padding-bottom: 4px; }\nh3 { color: #7c3aed; font-size: 16px; margin-top: 18px; }\n.hero { background: #fff3c4; border: 1px solid #facc15; border-radius: 12px; padding: 12px 14px; margin: 10px 0 18px 0; }\n.card { background: #ffffff; border: 1px solid #dbeafe; border-left: 6px solid #3b82f6; border-radius: 10px; padding: 12px 14px; margin: 12px 0; }\n.good { border-left-color: #10b981; }\n.warn { border-left-color: #f59e0b; background: #fffbeb; }\n.danger { border-left-color: #ef4444; background: #fff1f2; }\n.step { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 8px 10px; margin: 8px 0; }\ncode { background: #e0f2fe; color: #075985; border-radius: 5px; padding: 2px 5px; font-family: "Cascadia Mono", "Consolas", monospace; }\nul { margin-top: 6px; }\nstrong { color: #111827; }\n.small { color: #64748b; font-size: 13px; }\n</style>\n</head>\n<body>\n<div class="page">\n<h1>Freeze Feature After Update - Practical Help</h1>\n<div class="hero">\n<strong>Start here:</strong> after changing a project and validating the change, use this tab to freeze locally first. The deprecated project-local external AI review ZIP button has been removed; full external review now uses Show Project to AI artifacts.\n</div>\n\n<h2>1. Box status and repair</h2>\n<div class="card good">\n<h3>Check Box Status</h3>\n<p>Inspects the selected project and reports whether its freeze-after-update box is valid, missing, or incomplete.</p>\n<h3>Create / Repair Box</h3>\n<p>Creates missing safe freeze folder structure, including project-local freeze memory folders, without writing a frozen entry.</p>\n<p>It is safe because it creates or repairs structure only. It does not bypass Preview, Confirm and Write, or human confirmation.</p>\n</div>\n\n<h2>2. Local freeze is the normal path</h2>\n<div class="card">\n<p><strong>Freeze Feature After Update</strong> is the project safety memory system.</p>\n<p>When a feature is validated and should not be broken later, use <strong>Local Freeze Entry</strong>, preview it, and then confirm/write locally.</p>\n<p>Future AI sessions read freeze context from Show Project to AI startup artifacts and frozen feature memory snippets, not from the old deprecated files_to_send_ai ZIP generator.</p>\n</div>\n\n<h2>3. Useful copy buttons</h2>\n<div class="card good">\n<h3>Get Last Freeze</h3>\n<p>Copies the latest frozen feature entry as an AI-ready snippet.</p>\n<h3>Get All Frozen</h3>\n<p>Copies all frozen feature entries as an AI-ready snippet.</p>\n</div>\n\n<h2>4. Host-row folder buttons</h2>\n<div class="card">\n<h3>Project Root</h3>\n<p>The editable active project folder. Example: <code>E:\\kanda_reasoner</code>.</p>\n<h3>Box Folder</h3>\n<p>Opens <code>&lt;project&gt;\\project_freeze_after_update</code>.</p>\n<h3>External AI Review Folder</h3>\n<p>Opens the external review folder for inspection when it exists, but the deprecated export button is no longer part of the normal UI.</p>\n</div>\n\n<h2>5. Best workflow</h2>\n<div class="card good">\n<ol>\n<li>Select project root.</li>\n<li>Click <strong>Check Box Status</strong>.</li>\n<li>If incomplete, click <strong>Create / Repair Box</strong>.</li>\n<li>After a patch validates, click <strong>Local Freeze Entry</strong>.</li>\n<li>Review the preview.</li>\n<li>Click <strong>Confirm and Write Freeze Entry</strong> only after human review.</li>\n<li>Use <strong>Get Last Freeze</strong> or <strong>Get All Frozen</strong> when you need to paste freeze context to AI.</li>\n</ol>\n</div>\n\n<h2>6. Protected behavior</h2>\n<div class="card danger">\n<ul>\n<li><strong>Preview Freeze Entry</strong> remains read-only.</li>\n<li><strong>Confirm and Write</strong> still requires explicit human confirmation.</li>\n<li><strong>Ignore this Freeze</strong> never writes frozen memory.</li>\n<li>Project-specific frozen memory stays under <code>&lt;project&gt;_show_project_to_AI\\project_freeze_after_update\\frozen_features_memory</code>.</li>\n<li>Do not store project-specific frozen memory inside <code>project_freeze_ledger</code>.</li>\n</ul>\n</div>\n\n<p class="small">Box rule: <code>project_freeze_ledger</code> is KANDA Reasoner&#39;s blueprint generator box. <code>project_freeze_after_update</code> is the project-specific freeze memory/output box.</p>\n</div>\n</body>\n</html>\n'
