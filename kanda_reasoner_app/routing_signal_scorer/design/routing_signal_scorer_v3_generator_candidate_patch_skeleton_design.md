...
This milestone is design only, schema only, and skeleton evidence only. It
creates the metadata skeleton that a possible future generator-candidate patch
...
The scope is `generator_candidate_patch_skeleton_only`.

The current skeleton state is `not_skeletoned`, with effect
`no_effect_schema_only_not_skeletoned`.

## Required prior milestones

A future skeleton can only be considered after the v3 closure shield, disabled
...
- Current skeleton state: `not_skeletoned`.
