...
This milestone defines the shape of a future actual human decision record. It does not record a real human decision and does not authorize a generator candidate.

## Purpose

The previous milestone created a human decision intake schema. This milestone adds the next boundary: the exact static shape a future decision-recording patch would have to satisfy before any generator candidate could be reviewed.

The boundary is intentionally inert. It exists to prevent phrases such as "approved", "go next", or "record the decision" from becoming generator authorization.
...
## Allowed future recorded decision values
...
The current record effect is always:

```text
no_effect_schema_only_not_recorded
```

## Future work boundary

A future real decision recording requires a separate governed, validated, and frozen patch.
