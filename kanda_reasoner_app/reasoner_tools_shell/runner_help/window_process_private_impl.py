...
def _start_runtime_trace_process(self) -> None:
    ...
    process_args = [str((tool_root / CANONICAL_PACKAGE_NAME / 'reasoner_runtime_collector' / 'runtime_runner.py').resolve())]
...
def _on_process_finished_without_web_ai_enrichment(self, exit_code: int, _exit_status) -> None:
    ...
    if self._process is not None:
        stdout_text = bytes(self._process.readAllStandardOutput()).decode('utf-8', errors='replace').strip()
        stderr_text = bytes(self._process.readAllStandardError()).decode('utf-8', errors='replace').strip()
    if exit_code != 0:
        ...
        _safe_log_runtime_error(source_symbol='CollectorRunnerWindow._on_process_finished', message=f'{failed_stage} failed', payload={'traceback': stderr_text or stdout_text or 'Unknown child-process error', 'stage': failed_stage, 'exit_code': exit_code})
        show_error_copy_close_window(self, title='Collector error', message=stderr_text or stdout_text or 'Unknown child-process error')
        self._process = None
        return
    try:
    ...
    if self._active_stage == 'Generating AI context bundle':
        ...
        try:
            ...
            _zip_json_impl.write_second_prompt_status(build_dir, status='running', step='zipping generated files before final publish', project_root=project_root_path, final_output_dir=final_dir, details=['The final second_prompt_files folder remains untouched until ZIP export succeeds.', 'ZIP files are created inside the temporary build folder first.'])
            ...
            show_error_copy_close_window(self, title='Collector ZIP preparation error', message=str(exc))
            return
...
def _on_process_error(self, _process_error) -> None:
    ...
    show_error_copy_close_window(self, title='Collector error', message=f'{failed_stage} process could not start.')
    _safe_log_runtime_error(source_symbol='CollectorRunnerWindow._on_process_error', message=f'{failed_stage} process could not start.', payload={'stage': failed_stage})
    self._process = None
...
def _start_complete_json_enrichment_process(self) -> None:
    ...
    _tab4_apply_second_prompt_build_env(self, env)
    _tab4_apply_qprocess_env(self._process, env, project_root)
    process_args = ['-m', CANONICAL_PACKAGE_NAME + '.reasoner_context_collector.complete_json_web_ai_enrichment', '--root', self._pending_project_root, '--complete-json', self._pending_output_json, '--compact']
...
def _start_ai_context_bundle_process(self) -> None:
    ...
    _tab4_apply_second_prompt_build_env(self, env)
    _tab4_apply_qprocess_env(self._process, env, project_root)
    process_args = ['-m', CANONICAL_PACKAGE_NAME + '.reasoner_context_bundle', '--root', self._pending_project_root, '--write', '--compact', '--skip-validation']
...
def _run_collector(self) -> None:
    ...
    try:
        ...
        QMessageBox.warning(self, 'Invalid path', 'Project root must be a project folder, not a broad drive root.\n' + str(exc))
        return
    if not project_root_path.is_dir():
        QMessageBox.warning(self, 'Invalid path', 'Project root does not exist:\n' + str(project_root_path))
        return
    ...
    if cleanup_result.get('errors'):
        QMessageBox.warning(self, 'Cleanup failed', 'Generated ZIP artifact cleanup failed before source archive:\n' + str(cleanup_result.get('errors')))
        return
    ...
    _zip_json_impl.write_second_prompt_status(build_output_dir, status='running', step='starting lightweight map generation', project_root=project_root_path, final_output_dir=final_output_dir, details=['Temporary build folder was cleared before this run.', 'Removed old build-folder items: ' + str(removed_count), 'Previous final delivery remains available until publish succeeds.', 'Hybrid mode skips old heavy complete.json and active_snapshot.json generation.'])
    self._append_log('Prepared second_prompt_files_building output folder; removed existing items: ' + str(removed_count))
