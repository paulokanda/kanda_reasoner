...
def external_readme_text(context: ProjectContext, part_size_mb: int) -> str:
    ...
    return "\n".join(
        [
            ...
            "3. " + slug + "__ai_handoff_upload*.zip, in numeric order if split. This is the zipped Project JSON handoff package.",
            "4. Inside the JSON handoff ZIP, read UPLOAD_README.txt first, then ai_briefing, routing_manifest, bundle_manifest, patch_safety_routes, file_manifest, exclusion_rules when routed, source_archive_manifest, and validation_state.",
            ...
            "Reusable Error Lessons belong to the KANDA Reasoner Tool-owned Error Memory library, including the Portable Error Memory tab.",
            "A lesson may record this Project as its origin without becoming Project-owned Error Memory.",
            "Do not expect or require project-specific Error Memory files in second_prompt_files or inside the Project handoff ZIP.",
            "When an external AI chat needs Error Memory context, provide the Tool-owned Error Memory export separately from the Error Memory workflow.",
            "",
            ...
            "   Upload with source_archive parts when exact reconstruction needs PNG assets; parts use ZIP_STORED and split by the selected size cap.",
            "",
...
def package_readme_text(
    ...
    return "\n".join(
        [
            ...
            "For ai_handoff_upload packages, read UPLOAD_README.txt first, then ai_briefing, routing_manifest, bundle_manifest, patch_safety_routes, file_manifest, exclusion_rules when routed, source_archive_manifest, and validation_state.",
            "Reusable Error Lessons are Tool-owned Error Memory and are not a Project handoff package family; provide Tool-owned Error Memory context separately when needed.",
