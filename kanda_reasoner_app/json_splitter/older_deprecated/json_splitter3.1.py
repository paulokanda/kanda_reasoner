...
class SplitterWorker(QThread):
    ...
    def run(self):
        try:
            ...
            total = len(entries)
            if total == 0:
                self.finished.emit(False, f"❌ No entries found under {split_label}.", self.filepath)
                return
            ...
            self.finished.emit(True, f"🎉 Done! {total_chunks} file(s) created.", self.filepath)

        except json.JSONDecodeError as e:
            self.finished.emit(False, f"❌ Invalid JSON: {e}", self.filepath)
        except Exception as e:
            import traceback
            self.finished.emit(False, f"❌ Unexpected error: {e}\n{traceback.format_exc()}", self.filepath)
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # ---- Output directory + delete checkbox (in a vertical layout) ----
        out_vertical = QVBoxLayout()
        ...
        # New checkbox
        self.delete_tmp_check = QCheckBox("Delete original .tmp file after splitting")
        self.delete_tmp_check.setToolTip("Only files ending with .tmp will be removed on success")
        out_vertical.addWidget(self.delete_tmp_check)

        root.addLayout(out_vertical)
