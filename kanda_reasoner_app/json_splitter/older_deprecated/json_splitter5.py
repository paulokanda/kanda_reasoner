...
class SplitterWorker(QThread):
    ...
    def _run_split(self) -> None:
        ...
        for index, chunk_entries in enumerate(chunks, start=1):
            ...
            # NOVO: Aviso mais detalhado para parte com única entrada
            if size_bytes > MAX_FILE_BYTES:
                if len(chunk_entries) == 1:
                    key = chunk_entries[0][0]
                    if isinstance(key, str) and path_parts:  # dicionário com caminho definido
                        suggested = f"{plan.target_path}.{key}" if plan.target_path else key
                        self.log.emit(
                            f"Warning: Part {index} contains a single entry '{key}' of size {size_bytes/1024/1024:.2f} MB, "
    ...
    def _build_chunks(
        ...
        if mode == "parts":
            ...
            # MODIFICADO: Listar as chaves das entradas grandes
            large_entries = [i for i, sz in enumerate(entry_sizes) if sz > MAX_FILE_BYTES]
            if large_entries:
                # Tenta extrair as chaves (se for dict)
                large_keys = []
                for i in large_entries:
                    ...
                    if isinstance(key, str):
                        ...
                        large_keys.append(f"[{key}]")  # para listas, mostra índice
                self.log.emit(
                    f"Warning: {len(large_entries)} entry/entries exceed {MAX_FILE_BYTES//1024} KB: {', '.join(large_keys)}. "
                    f"Parts containing them will be larger than the limit. "
                    f"To split these entries, target a deeper path inside them (e.g., '{path_parts + [large_keys[0]] if path_parts else large_keys[0]}' if applicable)."
                )
            ...
            if large_entries:
                ...
                for i in large_entries:
                    ...
                    if isinstance(key, str):
                        ...
                        large_keys.append(f"[{key}]")
                self.log.emit(
                    f"Warning: {len(large_entries)} entry/entries exceed the specified {value} KB: {', '.join(large_keys)}. "
...
class JsonSplitterWindow(QMainWindow):
    ...
    def _inspect_json(self) -> None:
        ...
        # NOVO: Calcular e mostrar os tamanhos dos contêineres
