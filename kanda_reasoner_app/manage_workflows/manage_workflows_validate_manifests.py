# project-path: kanda_reasoner_app/manage_workflows/manage_workflows_validate_manifests.py
"""Validate the workflow manager helper manifest contract."""
...
ORIGIN = Path(_STAGED_PACKAGE_DIR) / "manage_workflows" / "manage_workflows.py"
MANIFEST = Path(_STAGED_PACKAGE_DIR) / "manage_workflows" / "manage_workflows_help.json"
HELP_FOLDER = Path(_STAGED_PACKAGE_DIR) / "manage_workflows" / "manage_workflows_help"
...
def main() -> int:
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    print("workflow manager helper manifest ok")
    return 0
