...
def apply_ruff_correction_preview(
    ...
    *,
    ...
    """Write a support-side proposal receipt without mutating Project source."""
    paths = resolve_ruff_correction_paths(project_root)
    ...
    if str(confirm_token or "") != record.confirm_token:
        raise RuffCorrectionApplyError(
            "RUFF_CORRECTION_CONFIRMATION_TOKEN_MISMATCH"
        )
    ...
    _validate_source_and_payload(root, payload_root, record)
    transaction_id = _transaction_id(
        safe_id,
        record.manifest_identity_sha256,
    )
    proposed_files = tuple(item.relative_path for item in record.files)
    return _write_receipt(
        ...
        changed_files=(),
        validation_markers=(
            "RUFF_CORRECTION_PREVIEW_INTEGRITY: PASS",
            "RUFF_CORRECTION_PROJECT_SOURCE_WRITE: PROPOSAL_ONLY",
        ),
        error="",
        metadata={
            "proposal_only_marker": PROJECT_REPAIR_PROPOSAL_ONLY_MARKER,
...
def _apply_payload(
    ...
    """Fail closed because active Project source application is unsupported."""
    del root, payload_root, record
    raise RuffCorrectionApplyError(PROJECT_REPAIR_PROPOSAL_ONLY_MARKER)
...
def _rollback_from_backups(
    ...
    """Fail closed because rollback may not mutate Project source."""
    del root, backup_root, source_before
    raise RuffCorrectionApplyError(PROJECT_REPAIR_PROPOSAL_ONLY_MARKER)
