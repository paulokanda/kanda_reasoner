# project-path: kanda_reasoner_app/reasoner_tools_gui_shell/ignore_rules_tab_help/prefs_io.py
...
class IgnoreRulesPrefsMixin:
    ...
    def _write_prefs(self, data: dict) -> None:
        ...
        self.prefs_path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding='utf-8')

    def _get_project_rules_from_prefs(self, data: dict) -> dict[str, list[str]] | None:
        ...
        if isinstance(project_rules, dict):
            ...
            if isinstance(rules, dict):
                return {'folders': list(rules.get('folders', [])), 'files': list(rules.get('files', [])), 'extensions': list(rules.get('extensions', []))}
        legacy_rules = data.get('ignore_rules', {})
        if self._project_key == '__global__' and isinstance(legacy_rules, dict):
            return {'folders': list(legacy_rules.get('folders', [])), 'files': list(legacy_rules.get('files', [])), 'extensions': list(legacy_rules.get('extensions', []))}
        if self._project_key != '__global__' and self._is_reasoner_project_root() and isinstance(legacy_rules, dict) and (not isinstance(project_rules, dict)):
            return {'folders': list(legacy_rules.get('folders', [])), 'files': list(legacy_rules.get('files', [])), 'extensions': list(legacy_rules.get('extensions', []))}
        return None
    ...
    def _save_rules(self) -> None:
        ...
        extensions = [self.ext_list.item(i).text() for i in range(self.ext_list.count())]
        ...
        try:
            ...
            show_error_copy_close_window(self, title='Error', message=f'Failed to save rules: {exc}')
