...
def is_transitional_copy_filename(filename: str) -> bool:
    ...
    return " - copy" in normalized or normalized.startswith("copy of ")


def is_transitional_path(path_text: str) -> bool:
...
def parse_module_source(path: Path, text: str) -> tuple[ast.Module, list[str]]:
    ...
    for warning in caught:
        if issubclass(warning.category, SyntaxWarning):
            ...
            if message:
                syntax_warnings.append(message)

    return tree, syntax_warnings
...
def scan_module(
    ...
    path: Path,
    top_names: set[str],
) -> tuple[ModuleInfo, list[str]]:
    text = read_text(path)
    tree, syntax_warnings = parse_module_source(path, text)
...
def is_relaxed_warning_path(path_text: str, filename: str) -> bool:
    normalized = str(path_text).replace("\\", "/").lstrip("./").lower()
    lowered_filename = filename.lower()

    if normalized.startswith("temp/"):
        return True
    if normalized.startswith("developer_tools/"):
        return True
    if normalized.startswith("original_backups/"):
        return True
    if normalized.startswith("_phase3_tmp_project/"):
        return True
    if "/older/" in normalized:
        return True
    if "/backup/" in normalized or "/backups/" in normalized:
        return True
    if is_transitional_path(normalized):
        return True
    if is_transitional_copy_filename(filename):
        return True
    if "to restore if needed" in lowered_filename:
        return True
...
def validate_modules(root: Path, modules: dict[str, ModuleInfo]) -> list[ValidationIssue]:
    ...
    for module in modules.values():
        path = module.path
        relaxed_warning_path = is_relaxed_warning_path(path, module.filename)

        if (
            not module.docstring
            and not is_test_path(path)
            and not module.is_init
            and not relaxed_warning_path
        ):
...
def scan_project(root: Path) -> tuple[dict[str, ModuleInfo], list[ValidationIssue], dict[str, Any], dict[Path, str]]:
    ...
    scan_issues: list[ValidationIssue] = []

    for path in iter_python_files(root):
        rel_path = str(path.relative_to(root)).replace("\\", "/")
        try:
            info, syntax_warnings = scan_module(root, path, top_names)
            ...
            level = (
                "warning"
                if is_relaxed_warning_path(rel_path, path.name)
                else "error"
            )
            scan_issues.append(
                ValidationIssue(
        ...
        for warning_text in syntax_warnings:
            scan_issues.append(
                ValidationIssue(
                    "warning",
                    "SYNTAX_WARNING",
    ...
    issues = scan_issues + validate_modules(root, modules)
