...
_AI_REVIEW_GUI_MODULE = _architecture_gui_importlib.import_module(
    _STAGED_PACKAGE_NAME
    + ".manage_architecture.ai_review.gui_integration"
)
install_tab1_ai_review_controls = (
    _AI_REVIEW_GUI_MODULE.install_tab1_ai_review_controls
)
            ...
            with contextlib.redirect_stdout(buffer), contextlib.redirect_stderr(buffer):
                if hasattr(module, "run"):
                    ...
                    raise AttributeError(
                        "manage_architecture.py must expose either run(root, mode) or main()."
                    )
...
class ArchitectureManagerWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._script_host_label: QLabel | None = None


        self._root_path_edit = QLineEdit(DEFAULT_PROJECT_ROOT)
        self._root_path_edit.setMinimumWidth(300)
    ...
    def _build_ui(self) -> None:
        ...
        root_row.addWidget(QLabel("Mode"))
        root_row.addWidget(self._mode_combo)
        root_row.addWidget(self._strict_write_checkbox)
    ...
    def move_script_selector_to_layout(
        self,
        destination_layout,
        insert_index: int | None = None,
    ) -> None:
        """Move the Tab 1 worker script selector into the host source row."""
        if self._script_selector_moved_to_host:
            return

        self._script_host_label = QLabel("Worker Script:")
        self._script_host_label.setStyleSheet(
            "color: #0B3D91; font-weight: bold; padding-left: 4px;"
        )

        self._script_path_edit.setParent(None)
        self._browse_script_btn.setParent(None)

        if insert_index is None:
            destination_layout.addSpacing(12)
            destination_layout.addWidget(self._script_host_label, 0)
            destination_layout.addWidget(self._script_path_edit, 0)
            destination_layout.addWidget(self._browse_script_btn, 0)
        else:
            destination_layout.insertSpacing(insert_index, 12)
            destination_layout.insertWidget(insert_index + 1, self._script_host_label, 0)
            destination_layout.insertWidget(insert_index + 2, self._script_path_edit, 0)
            destination_layout.insertWidget(insert_index + 3, self._browse_script_btn, 0)

        self._script_selector_moved_to_host = True
    ...
    def run_mode(self, mode: str) -> None:
        ...
        if not root_path.exists():
            QMessageBox.critical(self, "Invalid project root", f"Project root not found:\n{root_path}")
            return
        ...
        self._worker = ArchitectureRunWorker(
    ...
    def show_mode_help(self) -> None:
        help_text = (
            "Validate\n"
            "- checks the project tree for duplicate public symbols, helper-folder rule violations, "
            "docstring mismatches, and oversized modules.\n\n"
            "Diff\n"
            "- previews the changes that would be written to __init__.py files, "
            "ARCHITECTURE.md, and architecture_manifest.json.\n\n"
            "Scan\n"
            "- prints the full manifest JSON describing the project structure.\n\n"
            "Write\n"
            "- writes architecture_manifest.json, minimal __init__.py facades, "
            "and ARCHITECTURE.md, but only when validation has no errors."
        )
    ...
    def save_output(self) -> None:
        path, _ = QFileDialog.getSaveFileName(
            self,
            "Save output",
            str(Path.cwd() / "architecture_manager_output.txt"),
            "Text Files (*.txt)",
        )
        if not path:
            return
