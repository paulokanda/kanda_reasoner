...
<!-- PATCH_VALIDATE_FREEZE_ERROR_MEMORY_ROUTINE_GUARDRAIL_HOOK_V1_START -->
## Patch/Validate/Freeze/Error Memory Routine Hook

For any freezeable KANDA patch or recovery patch, and for any repair after
Show Project to AI generation, handoff ZIP creation, patch delivery,
validation, freeze evidence merge, freeze-form review, or Error Memory intake
fails, apply or request `patch_validate_freeze_error_memory_routine_blueprint`
before producing patch, install, validate, freeze, freeze-form, or Error Memory
output.
...
Do not claim `ZIP CONTRACT: PASS`, `VALIDATION OK`, `STATUS: IN_SYNC`, or
freeze evidence merge success unless those markers were actually produced by
the relevant local or sandbox validation step. Do not add nonessential marker
requirements to freeze code when the patch contract has already validated the
feature-specific markers.
<!-- PATCH_VALIDATE_FREEZE_ERROR_MEMORY_ROUTINE_GUARDRAIL_HOOK_V1_END -->
