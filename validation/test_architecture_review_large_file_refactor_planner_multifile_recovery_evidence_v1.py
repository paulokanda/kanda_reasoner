# project-path: validation/test_architecture_review_large_file_refactor_planner_multifile_recovery_evidence_v1.py
"""Validation for Multi-File Recovery Evidence v1."""
...
from kanda_reasoner_app.manage_architecture.large_file_refactor_planner.source_apply_multifile_recovery_evidence import (
...
from kanda_reasoner_app.manage_architecture.large_file_refactor_planner.source_apply_multifile_recovery_evidence_formatting import (
    format_source_apply_multifile_recovery_evidence,
)
...
FEATURE_MARKER = "VALIDATION OK: architecture-review-large-file-refactor-planner-multifile-recovery-evidence-v1"
PREVIOUS_MARKERS = [
...
def _make_recovered_context(tmp: Path, *, include_per_file_evidence: bool = False) -> tuple[Path, Path, Path, Path]:
    ...
    rollback = build_source_apply_rollback_recovery(
        str(post_apply_manifest),
        active_project_root=str(project_root),
        rollback_confirmation=SOURCE_APPLY_ROLLBACK_RECOVERY_TOKEN,
    )
    rollback_manifest = write_source_apply_rollback_recovery_manifest(rollback)
    if include_per_file_evidence:
        data = json.loads(rollback_manifest.read_text(encoding="utf-8"))
        data["per_file_backup_evidence"] = {
            item: {"backup_snapshot_path": str(preview_root / "per_file_backups" / Path(item).name)}
            for item in data.get("non_target_written_files", [])
        }
        rollback_manifest.write_text(json.dumps(data, indent=2), encoding="utf-8")
    return project_root, target, preview_root, rollback_manifest
...
def test_multifile_evidence_blocks_automated_removal_without_per_file_backup() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        project_root, _target, _preview_root, rollback_manifest = _make_recovered_context(Path(temp_text))
        result = build_source_apply_multifile_recovery_evidence(
            str(rollback_manifest),
            active_project_root=str(project_root),
            evidence_confirmation=SOURCE_APPLY_MULTIFILE_RECOVERY_EVIDENCE_TOKEN,
        )
        assert result.status == "blocked"
        assert result.non_target_written_files
        assert result.non_target_removal_enabled is False
        assert result.automated_multifile_recovery_enabled is False
        assert "PER_FILE_BACKUP_EVIDENCE_MISSING_FOR_NON_TARGET_FILES" in result.blockers
        manifest = write_source_apply_multifile_recovery_evidence_manifest(result)
        data = json.loads(manifest.read_text(encoding="utf-8"))
        assert data["loose_preview_artifacts_used_as_source_of_truth"] is False
        assert data["import_rewrite_rollback_enabled"] is False
        assert "SOURCE APPLY MULTI-FILE RECOVERY" in format_source_apply_multifile_recovery_evidence(result)
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def test_per_file_evidence_ready_still_does_not_remove_files() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        project_root, _target, _preview_root, rollback_manifest = _make_recovered_context(Path(temp_text), include_per_file_evidence=True)
        result = build_source_apply_multifile_recovery_evidence(
            str(rollback_manifest),
            active_project_root=str(project_root),
            evidence_confirmation=SOURCE_APPLY_MULTIFILE_RECOVERY_EVIDENCE_TOKEN,
        )
        assert result.status == "multifile_recovery_evidence_ready"
        assert result.per_file_backup_evidence_present is True
        assert result.non_target_removal_enabled is False
        manifest = write_source_apply_multifile_recovery_evidence_manifest(result)
        data = json.loads(manifest.read_text(encoding="utf-8"))
        assert data["automated_multifile_recovery_enabled"] is False
        for item in result.non_target_written_files:
            assert Path(item).exists()
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def test_project_reference_non_target_blocks_evidence() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        project_root, _target, _preview_root, rollback_manifest = _make_recovered_context(Path(temp_text), include_per_file_evidence=True)
        ref_dir = project_root / ".project_reference"
        ref_dir.mkdir()
        ref_file = ref_dir / "leak.py"
        ref_file.write_text("VALUE = 3\n", encoding="utf-8")
        data = json.loads(rollback_manifest.read_text(encoding="utf-8"))
        data["non_target_written_files"] = [str(ref_file)]
        data["per_file_backup_evidence"] = {str(ref_file): {"backup_snapshot_path": str(ref_file)}}
        rollback_manifest.write_text(json.dumps(data, indent=2), encoding="utf-8")
        result = build_source_apply_multifile_recovery_evidence(
            str(rollback_manifest),
            active_project_root=str(project_root),
...
def test_missing_evidence_token_blocks() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        project_root, _target, _preview_root, rollback_manifest = _make_recovered_context(Path(temp_text), include_per_file_evidence=True)
        result = build_source_apply_multifile_recovery_evidence(str(rollback_manifest), active_project_root=str(project_root), evidence_confirmation="")
        assert result.status == "blocked"
        assert "MULTIFILE_RECOVERY_EVIDENCE_TOKEN_MISSING_OR_INVALID" in result.blockers
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def main() -> int:
    test_multifile_evidence_blocks_automated_removal_without_per_file_backup()
    test_per_file_evidence_ready_still_does_not_remove_files()
    test_project_reference_non_target_blocks_evidence()
    test_missing_evidence_token_blocks()
