"""Validate startup artifact read-order guard v8."""
...
def main() -> int:
    ...
    require("CANONICAL_HELPERS = r\n" not in generator_text, "Broken dangling helper assignment remains in generator.")
    require('after = content.find("\n", end_index + len(end_phrase))' not in generator_text, "Broken interpreted newline string remains in generator.")
    require('after = content.find("\\n", end_index + len(end_phrase))' in generator_text, "Escaped newline search is not installed in generator.")
    ...
    require("content = add_read_order_block(content, filename)" in generator_text, "tell_AI output is not forced through generated read-order notice.")
    require("# STARTUP_ARTIFACT_READ_ORDER_GUARD_V8 canonical helpers BEGIN" in generator_text, "Generator does not contain v8 canonical helpers.")
    require("# STARTUP_ARTIFACT_READ_ORDER_GUARD_V7 canonical helpers BEGIN" not in generator_text, "Broken v7 helper block remains in generator.")
    require("def strip_startup_artifact_read_order_block(" in generator_text, "Generator does not replace stale read-order blocks.")
    require("required.add(READ_BEFORE_ANY_STARTUP_ARTIFACT_FILENAME)" in generator_text, "Generator does not treat old ZIPs missing the read-order notice as stale output.")
    require("except (ValueError, KeyError) as exc:" in generator_text, "Generator does not catch KeyError as stale/invalid generated ZIP output.")
    ...
    require("STATUS: IN_SYNC" in sync_output or "ENSURE SYNC RESULT: IN_SYNC_AFTER_SYNC" in sync_output, "Startup sync did not report in-sync status.")
    ...
    require(sorted_names[:3] == normal_required_files, "Generated normal startup file sort order is not expected: " + str(sorted_names[:3]))
    ...
    print("VALIDATION OK: startup-artifact-read-order-guard-v8")
