"""Validate startup artifact read-order guard v9."""
...
OPTIONAL_STEP = (
    "4. zz_read_only_if_modifying_startup_delivery.md - optional. Read only when modifying "
    "startup delivery. If this file is missing, pass and continue normal startup."
)
...
def assert_read_order_text(text: str, label: str) -> None:
    ...
    require(OPTIONAL_STEP in text, f"Maintenance optional/pass step is not exact in {label}.")
    require("optional" in lower, f"Maintenance file is not marked optional in {label}.")
    require("if this file is missing, pass" in lower, f"Maintenance missing-pass rule not explicit in {label}.")
    require("continue normal startup" in lower, f"Normal startup continuation not explicit in {label}.")
    require("read only when modifying startup delivery" in lower, f"Maintenance read-only condition not explicit in {label}.")
...
def inspect_zip(path: Path, label: str) -> None:
    with zipfile.ZipFile(path, "r") as z:
        ...
        if label == PROMPT_LIBRARY_ZIP:
            ...
            require("ACTIVE_PROMPTS/02_prompt_routing_and_indexing/prompt_navigation_index.md" in names, "prompt_library.zip missing prompt_navigation_index.md.")


def assert_no_known_corruption(text: str) -> None:
    require("CANONICAL_HELPERS = r\n" not in text, "Broken dangling helper assignment remains in generator.")
    require(not re.search(r're\.finditer\(r"\n', text), "Broken raw regex string with physical newline remains in generator.")
    require(not re.search(r'content\.find\("\n",', text), "Broken content.find string with physical newline remains in generator.")
    require('matches = list(re.finditer("\\\\n# (?!STARTUP DELIVERY READ ORDER)", content))' in text, "Safe escaped regex newline helper is not installed.")
    require('after = content.find("\\n", end_index + len(end_phrase))' in text, "Safe escaped content.find newline helper is not installed.")
...
def main() -> int:
    ...
    assert_no_known_corruption(generator_text)
    require("# STARTUP_ARTIFACT_READ_ORDER_GUARD_V9 canonical helpers BEGIN" in generator_text, "v9 helper marker not installed.")
    ...
    require(OPTIONAL_STEP in generator_text, "Generator does not contain the exact optional maintenance step.")
    require("content = add_read_order_block(content, filename)" in generator_text, "tell_AI_read_before_all.md is not generated with the read-order guard.")
    require("except (ValueError, KeyError) as exc:" in generator_text, "Old ZIP missing notice is not handled as stale/regenerate.")
    require("OPTIONAL: {MODIFY_STARTUP_DELIVERY_FILENAME} is missing" in generator_text, "Missing maintenance file is not optional in --check.")

    sync_output = run_checked([sys.executable, str(generator), "--ensure-sync", "--yes"], cwd=workspace)
    require("STATUS: IN_SYNC" in sync_output, "Startup delivery is not in sync after ensure-sync.")

    output_dir = default_first_prompt_output_dir(root)
    require(output_dir.exists(), f"first_prompt_files output dir missing: {output_dir}")

    tell_path = output_dir / TELL_FILE
    startup_zip = output_dir / STARTUP_ZIP
    prompt_zip = output_dir / PROMPT_LIBRARY_ZIP
    require(tell_path.exists(), f"Missing {TELL_FILE}.")
    require(startup_zip.exists(), f"Missing {STARTUP_ZIP}.")
    require(prompt_zip.exists(), f"Missing {PROMPT_LIBRARY_ZIP}.")
    ...
    inspect_zip(startup_zip, STARTUP_ZIP)
    inspect_zip(prompt_zip, PROMPT_LIBRARY_ZIP)
    assert_missing_maintenance_passes(generator, workspace, output_dir)

    print("VALIDATION OK: startup-artifact-read-order-guard-v9")
