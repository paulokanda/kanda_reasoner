# project-path: validation/test_project_aa_boxed_dynamic_local_ai_run_analysis_v1.py
"""Validate boxed Project A&A dynamic local-AI Run Analysis."""
...
FEATURE_ID = "project-aa-boxed-dynamic-local-ai-run-analysis-v1"
...
def _exercise_project_qa_runner(root: Path) -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="project_qa_boxed_runner_"))
...
def _exercise_publish_retention(root: Path) -> None:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="project_aa_boxed_publish_"))
    try:
        project_root = temp_root / "sample_project"
        project_root.mkdir()
