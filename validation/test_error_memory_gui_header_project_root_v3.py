"""Validation for Error Memory GUI header project-root placement v3."""
...
def _receive_block(status: str = "active") -> str:
    payload = {
        ...
        "regression_check": {
            ...
            "command": "python validation\\test_error_memory_gui_header_project_root_v3.py",
            "expected_marker": "VALIDATION OK: error-memory-gui-header-project-root-v3",
            "required_before_freeze": False,
        },
        "source_patch_zip": "kanda_error_memory_gui_header_project_root_v3_patch.zip",
        "install_command_summary": "Installer stages ZIP from project drive root and extracts only from the staged ZIP.",
        "validation_command_summary": "Validation checks Error Memory title/header placement and formatted-only memorize contract.",
        "notes": "Formatted active-ready lesson used to validate Memorize Error contract after header relocation.",
    }
    return ERROR_LESSON_JSON_BEGIN + "\n" + json.dumps(payload, indent=2) + "\n" + ERROR_LESSON_JSON_END + "\n"


def test_project_root_header_above_intake_without_loaded_context_label() -> None:
    source = _source()
    title_index = source.index('title = QLabel("Error Memory")')
    root_index = source.index('root_row.addWidget(QLabel("Project Root:"))')
    splitter_index = source.index('splitter = QSplitter(Qt.Horizontal)')
    intake_index = source.index('QGroupBox("AI-assisted error lesson intake")')
    right_index = source.index("# RIGHT COLUMN")
    _assert(title_index < root_index < splitter_index < intake_index < right_index, "Title and Project Root row must be above the splitter and intake group")
    _assert('self.loaded_project_context_label' not in source, "Loaded project context label must be deleted")
    _assert('Loaded project context' not in source, "Loaded project context text must be deleted")
    ...
    _assert('title.setStyleSheet("font-size: 16px; font-weight: bold;")' in source, "Error Memory title must be bold")
    _assert('self.project_root_value_label = QLineEdit(str(self._project_root))' in source, "Project root path must use a QLineEdit field")
    _assert('self.project_root_value_label.setReadOnly(True)' in source, "Project root field must be read-only")
    ...
    _assert('self.error_memory_path_label' not in source, "Separate EM path label should not remain below the header")
    _assert('self.second_prompt_path_label' not in source, "Separate Second Prompt path label should not remain below the header")
...
def test_manual_operation_phase_and_raw_draft_remain_removed() -> None:
    ...
    _assert("fallback_operation_phase=\"unknown\"" in source, "Operation phase should come from AI JSON metadata or inference, not a manual chooser")


def test_memorize_error_is_formatted_active_ready_only() -> None:
    source = _source()
...
def main() -> None:
    test_project_root_header_above_intake_without_loaded_context_label()
    test_manual_operation_phase_and_raw_draft_remain_removed()
    test_memorize_error_is_formatted_active_ready_only()
    test_current_intake_builds_active_ready_formatted_lesson()
    test_current_intake_downgrades_incomplete_json_to_draft()
    print("VALIDATION OK: error-memory-gui-header-project-root-v3")
