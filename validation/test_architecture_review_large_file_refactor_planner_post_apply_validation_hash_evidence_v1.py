# project-path: validation/test_architecture_review_large_file_refactor_planner_post_apply_validation_hash_evidence_v1.py
"""Validation for Post-Apply Validation + Hash/Evidence Check v1."""
...
from kanda_reasoner_app.manage_architecture.large_file_refactor_planner.post_apply_validation_formatting import (
...
FEATURE_MARKER = "VALIDATION OK: architecture-review-large-file-refactor-planner-post-apply-validation-hash-evidence-v1"
PREVIOUS_MARKERS = [
...
def _make_applied_context(tmp: Path) -> tuple[Path, Path, Path, Path, Path, str, str]:
    ...
    manifest = write_guarded_source_apply_execution_manifest(execution)
    after_hash = hashlib.sha256(target.read_bytes()).hexdigest()
    assert after_hash != before_hash
    return project_root, target, preview_root, manifest, backup, before_hash, after_hash
...
def test_post_apply_validation_passes_after_guarded_execution() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        tmp = Path(temp_text)
        project_root, _target, preview_root, execution_manifest, backup, before_hash, after_hash = _make_applied_context(tmp)
        backup_text = backup.read_text(encoding="utf-8")
        result = build_post_apply_validation(
            str(execution_manifest),
            active_project_root=str(project_root),
            validation_confirmation=POST_APPLY_VALIDATION_TOKEN,
        )
        assert result.status == "post_apply_validation_passed"
        assert result.source_content_hash_before == before_hash
        assert result.source_content_hash_after_actual == after_hash
        assert result.source_hash_transition_verified is True
        assert result.backup_snapshot_verified_unchanged is True
        assert result.written_files_verified is True
        assert result.module_size_gate_passed is True
        assert result.syntax_gate_passed is True
        manifest = write_post_apply_validation_manifest(result)
        ...
        assert "POST-APPLY VALIDATION" in format_post_apply_validation(result)
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def test_missing_validation_token_blocks_without_mutation() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        tmp = Path(temp_text)
        project_root, target, _preview_root, execution_manifest, _backup, _before, _after = _make_applied_context(tmp)
        before_text = target.read_text(encoding="utf-8")
        result = build_post_apply_validation(str(execution_manifest), active_project_root=str(project_root), validation_confirmation="")
        assert result.status == "blocked"
        assert "POST_APPLY_VALIDATION_TOKEN_MISSING_OR_INVALID" in result.blockers
        write_post_apply_validation_manifest(result)
        assert target.read_text(encoding="utf-8") == before_text
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def test_backup_snapshot_change_blocks() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        tmp = Path(temp_text)
        project_root, _target, _preview_root, execution_manifest, backup, _before, _after = _make_applied_context(tmp)
        ...
        assert result.status == "blocked"
        assert "BACKUP_SNAPSHOT_HASH_CHANGED" in result.blockers
        shutil.rmtree(_daily_root_for_test(project_root), ignore_errors=True)


def test_project_reference_written_file_blocks() -> None:
    with tempfile.TemporaryDirectory() as temp_text:
        tmp = Path(temp_text)
        project_root, _target, _preview_root, execution_manifest, _backup, _before, _after = _make_applied_context(tmp)
        data = json.loads(execution_manifest.read_text(encoding="utf-8"))
        ref_dir = project_root / ".project_reference"
        ref_dir.mkdir()
        ref_file = ref_dir / "bad.py"
        ref_file.write_text("VALUE = 3\n", encoding="utf-8")
        data["written_files"] = [str(ref_file)]
        execution_manifest.write_text(json.dumps(data, indent=2), encoding="utf-8")
        result = build_post_apply_validation(
            str(execution_manifest),
            active_project_root=str(project_root),
            validation_confirmation=POST_APPLY_VALIDATION_TOKEN,
...
def main() -> int:
    test_post_apply_validation_passes_after_guarded_execution()
    test_missing_validation_token_blocks_without_mutation()
    test_backup_snapshot_change_blocks()
    test_project_reference_written_file_blocks()
