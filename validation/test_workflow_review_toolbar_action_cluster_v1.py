"""Validate Workflow Review toolbar action cluster v1."""
...
FEATURE_ID = "workflow-review-toolbar-action-cluster-v1"
...
def assert_toolbar_action_cluster() -> None:
    window = read(WINDOW)
    ...
    require(
        ...
        "mode action cluster is not added to the toolbar",
    )
    require(
        "toolbar.addWidget(self._run_options_toolbar_widget)" in window,
        "run options cluster is not added to the toolbar",
    )
    require(
        ...
        "mode actions must be left of run options in the toolbar",
    )
    require(
        'self._run_button = QPushButton("Run Selected Mode")' in window,
        "Run Selected Mode button is not in readable source",
    )
    require(
        "mode_action_toolbar_layout.addWidget(self._run_button)" in window,
        "Run Selected Mode button is not in the toolbar action cluster",
    )
    require(
        "self._mode_quick_buttons: dict[str, QPushButton] = {}" in window,
        "quick mode button registry is missing",
    )
    require(
    ...
    require(
        ...
        "quick mode buttons are not added to the toolbar action cluster",
    )
    require(
        'self._mode_help_toolbar_button = QPushButton("What do these modes do?")'
        in window,
        "What do these modes do button is not in the run options toolbar widget",
    )
    require(
        "run_options_toolbar_layout.addWidget(self._mode_help_toolbar_button)"
        in window,
        "mode help button is not added to the run options toolbar widget",
    )
    require(
        "run_options_toolbar_layout.addWidget(self._strict_write_checkbox)"
        in window,
        "confirm-before-write checkbox is not kept in run options widget",
    )
    require(
        window.index("run_options_toolbar_layout.addWidget(self._strict_write_checkbox)")
        < window.index("run_options_toolbar_layout.addWidget(self._mode_help_toolbar_button)"),
        "mode help button must be right of the checkbox inside run options",
    )


def assert_body_row_removed() -> None:
    window = read(WINDOW)
    ...
    require('QAction("> Run", self)' not in window, "duplicate toolbar Run action remains")
    require('QAction("Mode Help", self)' not in window, "duplicate Mode Help action remains")
...
def assert_behavior_still_connected() -> None:
    ...
    require(
        "self._mode_help_toolbar_button.clicked.connect(self.show_mode_help)" in window,
        "mode help button no longer calls show_mode_help",
    )
    require(
        "if mode == \"write\" and self._strict_write_checkbox.isChecked():" in window,
        "write confirmation gate was changed or removed",
    )
    require(
        "self._run_button.setEnabled(False)" in window,
        "run button is no longer disabled while work runs",
    )
    require(
        "self._run_button.setEnabled(True)" in window,
        "run button is no longer re-enabled after work finishes",
    )
...
def assert_freeze_hint_staging_contract() -> None:
    hint_path = (
        ...
        / "freeze_hint_intake"
        / "workflow-review-toolbar-action-cluster-v1__KANDA_FREEZE_HINT.json"
    )
    if hint_path.exists():
...
def main() -> int:
    ...
    assert_toolbar_action_cluster()
    assert_body_row_removed()
