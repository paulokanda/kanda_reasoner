...
FIXTURE_PROJECT_PREFIX = "kanda_v6_fixture_"


def make_fixture_project_root(tmp: str, label: str) -> Path:
    """Return a unique test project root that is safe to clean on Windows.

    On Windows, the production resolver intentionally places
    <project>_show_project_to_AI at the drive root. Test projects therefore
    need unique guarded names so any cleanup target is clearly a disposable
    fixture, not a real user project-support root.
    """
    safe_label = "".join(ch if ch.isalnum() or ch == "_" else "_" for ch in label.lower()).strip("_")
    return Path(tmp) / (FIXTURE_PROJECT_PREFIX + safe_label + "_" + uuid.uuid4().hex[:12])


def remove_isolated_show_root_for_fixture(project_root: Path) -> None:
    """Remove only a uniquely named fixture show-project root.

    The resolver uses drive-root placement on Windows, so parent equality with
    project_root.parent is not a valid safety check there. The safety boundary
    is the generated fixture name: it must start with kanda_v6_fixture_ and end
    with _show_project_to_AI.
    """
    show_root = project_analysis_evidence_root(project_root)
    expected_name = project_root.name + "_show_project_to_AI"
    assert_true(show_root.name == expected_name, "refusing to clean unexpected show-project fixture root")
    assert_true(show_root.name.startswith(FIXTURE_PROJECT_PREFIX), "refusing to clean non-fixture show-project root")
    assert_true(show_root.name.endswith("_show_project_to_AI"), "refusing to clean non-show-project root")
    assert_true(len(show_root.name) > len(FIXTURE_PROJECT_PREFIX + "_show_project_to_AI") + 8, "refusing to clean suspiciously short fixture root")
    if show_root.exists():
...
def test_resolver_and_lifecycle() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_root_") as tmp:
        project_root = make_fixture_project_root(tmp, "sample_project")
        project_root.mkdir()
        show_root = project_analysis_evidence_root(project_root)
        freeze_root = analysis_project_freeze_after_update_dir(project_root)
        paths = build_paths(project_root)

        assert_true(show_root.name.startswith(FIXTURE_PROJECT_PREFIX + "sample_project_"), "wrong show-project fixture root name")
        assert_true(show_root.name.endswith("_show_project_to_AI"), "wrong show-project root suffix")
        ...
        remove_isolated_show_root_for_fixture(project_root)
...
def test_legacy_fallback_and_clean_migration() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_migrate_") as tmp:
        project_root = make_fixture_project_root(tmp, "project_with_legacy")
        project_root.mkdir()
        remove_isolated_show_root_for_fixture(project_root)
...
def test_conflict_does_not_merge() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_conflict_") as tmp:
        project_root = make_fixture_project_root(tmp, "project_with_conflict")
        project_root.mkdir()
        legacy_root = write_sample_legacy_freeze(project_root)
        canonical_root = analysis_project_freeze_after_update_dir(project_root)
...
def test_cleanup_preserves_persistent_children() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_cleanup_") as tmp:
        project_root = make_fixture_project_root(tmp, "project_cleanup")
        project_root.mkdir()
        show_root = project_analysis_evidence_root(project_root)
        freeze_root = analysis_project_freeze_after_update_dir(project_root)
...
def test_startup_freeze_context_uses_external_root() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_startup_") as tmp:
        project_root = make_fixture_project_root(tmp, "project_startup")
        project_root.mkdir()
...
def test_local_freeze_writer_preview_root_adapter() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_contract_") as tmp:
        ...
        try:
            project_root = make_fixture_project_root(tmp, "contract_project")
            project_root.mkdir()
            ...
            assert_true(preview.get("project_root") == str(owner_root), "preview project_root should be engine-facing owner root")
            assert_true(preview.get("freeze_state_owner_root") == str(owner_root), "preview should expose external owner root")
            assert_true(preview.get("selected_project_root") == str(project_root.resolve(strict=False)), "preview should preserve selected source project root")
            assert_true(preview.get("display_project_root") == str(project_root.resolve(strict=False)), "preview should expose display project root")

            validation = validate_freeze_entry_preview(project_root, preview)
            assert_true(validation.get("ok") is True, "adapter did not prevent project_root mismatch: " + str(validation.get("errors")))
            assert_true(validation.get("project_root") == str(owner_root), "validation project_root should be engine-facing owner root")
            assert_true(validation.get("freeze_state_owner_root") == str(owner_root), "validation should expose external owner root")
            assert_true(validation.get("selected_project_root") == str(project_root.resolve(strict=False)), "validation should preserve selected source project root")
            ...
            assert_true(write_result.get("project_root") == str(owner_root), "write project_root should be engine-facing owner root")
            assert_true(write_result.get("freeze_state_owner_root") == str(owner_root), "write should expose external owner root")
            assert_true(write_result.get("selected_project_root") == str(project_root.resolve(strict=False)), "write should preserve selected source project root")
            remove_isolated_show_root_for_fixture(project_root)
        finally:
...
def test_tool_logic_never_lands_in_freeze_state() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="freeze_external_boundary_") as tmp:
        project_root = make_fixture_project_root(tmp, "project_boundary")
        project_root.mkdir()
        canonical_root = analysis_project_freeze_after_update_dir(project_root)
...
def test_freeze_hint_accepts_validation_evidence_aliases() -> None:
    """Validate v6 freeze hints feed mandatory freeze-form fields."""
    with tempfile.TemporaryDirectory(prefix="freeze_hint_v6_") as tmp:
        project_root = make_fixture_project_root(tmp, "hint_project")
        project_root.mkdir()
        hint = {
            ...
            "box_type": "governed architecture/path migration correction",
            "validated_files": [
            ...
            "do_not_touch_summary": [
                "Freeze hint sidecars must populate validated_files.",
                "Freeze hint sidecars must populate validation_evidence_summary.",
            ],
            "validation_evidence": [
        ...
        record = save_freeze_hint_record(project_root, hint)
        form = record.get("form_inputs") or {}
        assert_true("kanda_reasoner_app/freeze_hint_intake/contract.py" in str(form.get("validated_files") or ""), "validated_files not populated from hint")
        assert_true("VALIDATION OK: freeze-state-external-root-migration-v6" in str(form.get("validation_evidence_summary") or ""), "validation evidence alias not normalized")
        assert_true("STATUS: IN_SYNC" in str(form.get("validation_evidence_summary") or ""), "sync marker missing from normalized evidence")
        loaded = load_latest_freeze_hint_record(project_root)
        assert_true(loaded.get("ok") is True, "saved v6 hint did not load")
        state = resolve_freeze_hint_autofill_state(project_root, {})
        assert_true(state.get("confirm_write_enabled") is True, "v6 hint should enable confirm-write after validation evidence")
        assert_true(state.get("mandatory_fields_status") == "complete", "mandatory fields not complete")
        assert_true(state.get("validation_evidence_status") == "recognized", "validation evidence not recognized")
        remove_isolated_show_root_for_fixture(project_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    tests = [
        ...
        test_freeze_hint_accepts_validation_evidence_aliases,
    ...
    print("VALIDATION OK: freeze-state-external-root-migration-v6")
