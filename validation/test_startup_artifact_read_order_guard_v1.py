"""Validate startup artifact read-order guard v1.

This validation intentionally regenerates the startup delivery so the test checks
what future AI chats will receive, not just source text.
"""
...
FEATURE_ID = "startup-artifact-read-order-guard-v1"
...
def inspect_zip(path: Path, label: str) -> None:
    with zipfile.ZipFile(path, "r") as z:
        ...
        require(NOTICE_FILE in names, f"{label} missing {NOTICE_FILE}.")
...
def main() -> int:
    ...
    run_checked([sys.executable, "-m", "py_compile", str(generator)], cwd=root)
    ...
    expected_files = [STARTUP_ZIP, PROMPT_LIBRARY_ZIP, TELL_FILE, MAINTENANCE_FILE]
    for name in expected_files:
        require((output_dir / name).exists(), f"Generated delivery file missing: {name}")
    ...
    require(sorted_names[:4] == expected_files, "Generated file sort order is not the expected Windows-friendly order: " + str(sorted_names[:4]))

    tell_text = read_text(output_dir / TELL_FILE)
    maintenance_text = read_text(output_dir / MAINTENANCE_FILE)
    require(tell_text.startswith("# " + EXPECTED_MARKER), "tell_AI_read_before_all.md does not start with the read-order guard.")
    require(maintenance_text.startswith("# " + EXPECTED_MARKER), "maintenance file does not start with the read-order guard.")
    ...
    print("VALIDATION OK: startup-artifact-read-order-guard-v1")
