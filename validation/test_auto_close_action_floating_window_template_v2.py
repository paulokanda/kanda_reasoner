"""Validate the silent auto-close action floating window template patch."""
...
FEATURE_ID = "auto-close-action-floating-window-template-v2"
...
REQUIRED_TEMPLATE_STRINGS = [
    ...
    "Sound policy:",
    "This template is intentionally silent.",
]

REQUIRED_BLUEPRINT_STRINGS = [
    "Silent Auto-Close Action Floating Window Blueprint",
    "This template is silent by default.",
    "must not use `QMessageBox`, `QApplication.beep`",
    "Closing is silent and must not play the Windows alert sound.",
...
def assert_not_contains(path: Path, forbidden_strings: list[str]) -> None:
    """Ensure forbidden strings do not exist in a file."""
    text = read_text(path)
    for needle in forbidden_strings:
        if needle in text:
            raise AssertionError(f"forbidden text found in {path}: {needle}")
...
def main() -> int:
    ...
    assert_not_contains(TEMPLATE_FILE, FORBIDDEN_TEMPLATE_STRINGS)
    ...
    if "setAutoDefault(False)" not in template_text or "setDefault(False)" not in template_text:
        raise AssertionError("OK button must not use default dialog-button behavior")
