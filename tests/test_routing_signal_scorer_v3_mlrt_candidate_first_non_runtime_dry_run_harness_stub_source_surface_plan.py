...
DOC = MLRT / "MLRT_18_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_SOURCE_SURFACE_PLAN.md"
README = MLRT / "README.md"
MANIFEST = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "box_manifest.json"
FEATURE_ID = "routing_signal_scorer_v3_mlrt_candidate_first_non_runtime_dry_run_harness_stub_source_surface_plan_v1"
ALLOWED_LAB_PY = {
...
def test_mlrt18_docs_exist() -> None:
    assert DOC.exists()
    assert README.exists()
    text = read(DOC)
    assert "MLRT-18 Candidate First Non-Runtime Dry-Run Harness Stub Source Surface Plan v1" in text
    assert FEATURE_ID in text


def test_mlrt18_is_source_surface_planning_only() -> None:
    text = read(DOC)
    required = [
        "documentation-only first non-runtime dry-run harness stub source-surface planning milestone",
        "still not source code and not a harness implementation",
        ...
        "does not implement a static interface",
...
def test_mlrt18_records_roadmap_progression() -> None:
    text = read(DOC)
    ...
    assert "MLRT-19 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Contract Plan" in text


def test_mlrt18_defines_source_surface_planning_groups() -> None:
    text = read(DOC)
    for heading in [
...
def test_mlrt18_preserves_forbidden_boundaries() -> None:
    text = read(DOC)
    forbidden_phrases = [
...
def test_mlrt18_positive_label_is_planning_only() -> None:
    text = read(DOC)
    assert "MLRT_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_SOURCE_SURFACE_PLAN_READY_FOR_STATIC_INTERFACE_CONTRACT_PLANNING_ONLY" in text
...
def test_mlrt18_has_no_python_implementation_files() -> None:
...
def test_mlrt18_does_not_create_forbidden_implementation_paths() -> None:
    forbidden_paths = [
...
def test_box_manifest_records_mlrt18_source_surface_plan() -> None:
    data = json.loads(read(MANIFEST))
    prefix = "mlrt_candidate_first_non_runtime_dry_run_harness_stub_source_surface_plan"
    assert data[f"{prefix}_feature_id"] == FEATURE_ID
    assert data[f"{prefix}_doc"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_18_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_SOURCE_SURFACE_PLAN.md"
    assert data[f"{prefix}_readme"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/README.md"
    assert data[f"{prefix}_test"] == "tests/test_routing_signal_scorer_v3_mlrt_candidate_first_non_runtime_dry_run_harness_stub_source_surface_plan.py"
    assert data[f"{prefix}_documentation_only"] is True
    assert data[f"{prefix}_source_surface_planning_only"] is True
    assert data[f"{prefix}_static_interface_contract_planning_only"] is False
    ...
    assert data[f"{prefix}_positive_label"] == "MLRT_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_SOURCE_SURFACE_PLAN_READY_FOR_STATIC_INTERFACE_CONTRACT_PLANNING_ONLY"
    assert data[f"{prefix}_next_safe_milestone"] == "MLRT-19 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Contract Plan after MLRT-18 freeze with FREEZE_MEMORY_STATUS OK"


def main() -> None:
    test_mlrt18_docs_exist()
    test_mlrt18_is_source_surface_planning_only()
    test_mlrt18_records_roadmap_progression()
    test_mlrt18_defines_source_surface_planning_groups()
    test_mlrt18_preserves_forbidden_boundaries()
    test_mlrt18_positive_label_is_planning_only()
    test_mlrt18_has_no_python_implementation_files()
    test_lab_python_surface_remains_exactly_three_files()
    test_mlrt18_does_not_create_forbidden_implementation_paths()
    test_box_manifest_records_mlrt18_source_surface_plan()
    print(
        "CONTRACT_TEST_OK: MLRT-18 Candidate First Non-Runtime Dry-Run Harness Stub Source Surface Plan v1, "
        "immutable governed documentation-only harness stub source-surface planning after MLRT-17 freeze with FREEZE_MEMORY_STATUS OK, "
        "confirms roadmap progress and defines future non-authoritative harness stub source-surface planning groups for source-surface identity, "
        "allowed source-surface shape, import policy, output envelope doctrine, abort doctrine, no-implementation doctrine, no-reliability-claim doctrine, "
        ...
        "no Python source files created, no source surface created, no static interface created, no executable validators, no schema code, no contract schema created, "
        "no boundary checker created, no harness created, no runner created, no harness stub files created, no implementation modules created, "
        ...
        "candidate first non-runtime dry-run harness stub source-surface plan remains non-authoritative, next safe milestone is MLRT-19 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Contract Plan"
    )
    print("SANDBOX_ROUTING_SIGNAL_SCORER_V3_MLRT_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_SOURCE_SURFACE_PLAN_V1_VALIDATION_OK")
