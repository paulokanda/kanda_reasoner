...
DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE10_READ_ONLY_ADVISORY_PANEL_RENDERER_MOUNT_FINAL_SAFETY_GATE_V1.md"
NEXT_DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE10_READ_ONLY_ADVISORY_PANEL_RENDERER_MOUNT_COMPLETION_HANDOFF_READINESS_V1.md"
...
REVIEW_GATE_DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE10_READ_ONLY_ADVISORY_PANEL_RENDERER_MOUNT_IMPLEMENTATION_RESULT_REVIEW_GATE_V1.md"

FEATURE_ID = "rss_ml_adv_phase10_read_only_advisory_panel_renderer_mount_final_safety_gate_v1"
REVIEWED_FEATURE_ID = "rss_ml_adv_phase10_read_only_advisory_panel_renderer_mount_implementation_result_review_gate_v1"
PREFIX = "ml_advisory_phase10_read_only_advisory_panel_renderer_mount_final_safety_gate_v1_"
IMPL_PREFIX = "ml_advisory_phase10_read_only_advisory_panel_renderer_mount_implementation_v1_"
REVIEW_PREFIX = "ml_advisory_phase10_read_only_advisory_panel_renderer_mount_implementation_result_review_gate_v1_"
...
def test_phase10_renderer_mount_final_safety_gate_accepts_only_safe_line_for_handoff() -> None:
    ...
    assert data[PREFIX + "planned_next_step"] == "Routing Signal Scorer ML Advisory-Signal Phase 10 Read-Only Advisory Panel Renderer Mount Completion Handoff v1"
    assert data[PREFIX + "new_real_prompt_selection_cases_added"] == 0
    assert data[PREFIX + "critical_boundary_error_budget"] == 0


def test_phase10_renderer_mount_final_safety_gate_updates_prior_state_without_authority() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
...
def test_phase10_renderer_mount_final_safety_gate_docs_define_completion_handoff_only() -> None:
    ...
    assert "not runtime UI mutation" in doc_text
...
if __name__ == "__main__":
    test_phase10_renderer_mount_final_safety_gate_accepts_only_safe_line_for_handoff()
    test_phase10_renderer_mount_final_safety_gate_updates_prior_state_without_authority()
    test_phase10_renderer_mount_final_safety_gate_docs_define_completion_handoff_only()
    print("VALIDATION OK: phase10 read-only advisory panel renderer mount final safety gate")
