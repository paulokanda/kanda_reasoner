...
from kanda_reasoner_app.reasoner_context_bundle import generate_ai_context_bundle  # noqa: E402
...
def _make_project() -> Path:
    ...
    _write_json(
        root / "project_analysis_evidence" / "json_complete" / "sample_project__complete.json",
        {"complete": True},
    )
    _write_json(
        root
        ...
        / "json_complete"
        / "sample_project__complete_runtime_trace.json",
        {"trace": True},
    )
    return root
...
def test_zip_ai_context_bundle_creates_foldered_zip_with_required_and_trace_files() -> None:
    ...
    assert zip_result["required_artifact_count"] == 6
    assert zip_result["optional_artifact_count"] == 1
    assert zip_result["artifact_count"] == 7
