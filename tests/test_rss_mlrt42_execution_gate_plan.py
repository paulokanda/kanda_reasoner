...
DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_42_EXECUTION_GATE_PLAN.md"
README = MLRT / "README.md"
MANIFEST = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "box_manifest.json"
FEATURE_ID = "rss_mlrt42_execution_gate_plan_v1"
FEATURE_TITLE = "Routing Signal Scorer MLRT-42 Execution Gate Plan v1"
...
NEXT_TITLE = "Routing Signal Scorer MLRT-43 Execution Gate Outcome Plan v1"
POS_LABEL = "RSS_MLRT42_READY_FOR_EXECUTION_GATE_OUTCOME_PLANNING_ONLY"
PREFIX = "rss_mlrt42_execution_gate_plan"
...
def test_mlrt42_docs_exist() -> None:
    assert DOC.exists()
    assert README.exists()
    assert MANIFEST.exists()


def test_mlrt42_uses_compact_freeze_safe_name() -> None:
    text = read(DOC)
...
def test_mlrt42_is_execution_gate_planning_only() -> None:
    text = read(DOC)
    required = [
        "Status: governed documentation-only execution-gate planning milestone.",
        "MLRT-42 is still not source code.",
        "It is not execution-gate execution.",
        "It is not an execution-gate outcome.",
        "It is not source creation approval.",
        "MLRT-42 only plans the future gate",
        "does not run that gate",
        "does not produce a gate outcome",
...
def test_mlrt42_boundaries_prevent_source_and_runtime() -> None:
    text = read(DOC)
    for item in [
...
def test_mlrt42_planned_source_path_remains_absent() -> None:
    assert PLANNED_SOURCE in read(DOC)
    assert not (ROOT / PLANNED_SOURCE).exists()


def test_mlrt42_required_evidence_is_safe() -> None:
    text = read(DOC)
    required = [
...
def test_mlrt42_rejection_reasons_are_comprehensive() -> None:
    text = read(DOC)
    reasons = [
...
def test_mlrt42_vocabulary_remains_non_authoritative() -> None:
    text = read(DOC)
    assert "controlled_minimal_source_file_creation_patch_execution_gate_ready_for_outcome_planning_only" in text
...
def test_mlrt42_manifest_records_planning_only_boundaries() -> None:
    manifest = read(MANIFEST)
    required = [
        FEATURE_ID,
        FEATURE_TITLE,
        "execution_gate_planning_only",
...
def test_mlrt42_has_no_python_implementation_files() -> None:
...
def test_mlrt42_readme_references_feature() -> None:
    readme = read(README)
    assert FEATURE_TITLE in readme
    assert FEATURE_ID in readme
    assert NEXT_TITLE in readme
    assert "Tests do not start at MLRT-42" in readme


if __name__ == "__main__":
    test_mlrt42_docs_exist()
    test_mlrt42_uses_compact_freeze_safe_name()
    test_mlrt42_is_execution_gate_planning_only()
    test_mlrt42_boundaries_prevent_source_and_runtime()
    test_mlrt42_planned_source_path_remains_absent()
    test_mlrt42_required_evidence_is_safe()
    test_mlrt42_rejection_reasons_are_comprehensive()
    test_mlrt42_vocabulary_remains_non_authoritative()
    test_mlrt42_manifest_records_planning_only_boundaries()
    test_mlrt42_has_no_python_implementation_files()
    test_lab_python_surface_remains_exactly_three_files()
    test_mlrt42_readme_references_feature()
    print(
        "CONTRACT_TEST_OK: MLRT-42 Execution Gate Plan v1, "
        "immutable governed documentation-only execution-gate planning after MLRT-41 freeze with FREEZE_MEMORY_STATUS OK, "
    ...
    print("SANDBOX_RSS_MLRT42_EXECUTION_GATE_PLAN_V1_VALIDATION_OK")
