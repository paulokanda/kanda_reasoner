...
def test_phase9_runtime_activation_completion_handoff_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "completion_handoff_closes_phase9_runtime_activation_as_dormant_envelope_only_phase10_renderer_mount_contract_next"
    ...
    assert data[PREFIX + "renderer_neutral"] is True
    ...
    assert data[PREFIX + "missing_view_model_behavior"] == "fail_open"
    assert data[PREFIX + "unsafe_view_model_behavior"] == "blocked_fail_open"
    for flag in REQUIRED_FALSE:
        assert data[PREFIX + flag] is False, flag


def test_readme_records_phase9_runtime_activation_completion_handoff() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 9 read-only advisory panel runtime activation completion handoff" in text
    assert "dormant, in-memory, renderer-neutral activation-envelope foundation" in text
    assert "does not render" in text
    assert "Phase 10 Read-Only Advisory Panel Renderer Mount Contract v1" in text


if __name__ == "__main__":
    test_phase9_runtime_activation_completion_handoff_manifest_gates()
    test_readme_records_phase9_runtime_activation_completion_handoff()
    print("VALIDATION OK: phase9 read-only advisory panel runtime activation completion handoff manifest gates")
