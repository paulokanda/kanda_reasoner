...
DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_63_ROUTER_PROMPT_SELECTION_FIXED_CASE_SET_STATIC_VALIDATION_PLAN.md"
...
PREV_DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_62_ROUTER_PROMPT_SELECTION_FIXED_CASE_SET_FORMAT_PLAN.md"
FEATURE_ID = "rss_mlrt63_router_prompt_selection_fixed_case_set_static_validation_plan_v1"
FEATURE_TITLE = "Routing Signal Scorer MLRT-63 Router Prompt Selection Fixed Case Set Static Validation Plan v1"
...
NEXT_TITLE = "Routing Signal Scorer MLRT-64 Router Prompt Selection Non-Runtime Offline Evaluation Harness Plan v1"
POS_LABEL = "RSS_MLRT63_PROMPT_SELECTION_FIXED_CASE_SET_STATIC_VALIDATION_PLANNED_NO_VALIDATOR_NO_CASE_SET"
PREFIX = "rss_mlrt63_router_prompt_selection_fixed_case_set_static_validation_plan"
...
def test_mlrt63_files_exist() -> None:
    assert DOC.exists()
...
def test_mlrt63_preserves_prompt_selection_goal_and_countdown() -> None:
    text = read(DOC)
    required = [
        "ML must be tested for helping prompt selection in router prompt logic",
        "router prompt-selection fixed case set static validation plan",
        "does **not** create a fixed case set",
        "does **not** create case files",
        "does **not** implement a validator",
        "does **not** run static validation",
        "does **not** execute cases",
        "does **not** score candidate output",
        "does **not** compare routes",
        "The critical boundary error budget remains `0`",
        "1 step to the testing harness",
        "2 steps to the first real ML prompt-selection test",
        "manifest_identity_check",
        "schema_compatibility_check",
        "case_index_shape_check",
        "expected_route_label_check",
        "accepted_alternate_route_check",
        "forbidden_route_check",
        "boundary_expectation_check",
        "critical_failure_check",
        "provenance_check",
        "checksum_freeze_check",
        "static_validation_output_check",
...
def test_mlrt63_only_allowed_python_surfaces_exist() -> None:
...
def test_mlrt63_stub_boundary_still_blocks_runtime_use() -> None:
    module = load_module(SOURCE, "mlrt63_stub_under_static_validation_plan_test")
...
def test_mlrt63_does_not_create_validators_cases_or_evaluation_outputs() -> None:
    text = read(DOC)
    forbidden_positive_claims = [
        "static validator is created",
        "static validation has passed",
    ...
    required_negative_claims = [
        "must not claim static validation passed",
        "fixed-case readiness",
...
def test_mlrt63_manifest_records_static_validation_plan_without_unlocking_testing() -> None:
    ...
    required = [
        ...
        POS_LABEL,
        NEXT_TITLE,
        "router_prompt_selection_fixed_case_set_static_validation_plan_only_no_validator_no_case_set_no_evaluation_no_runtime_authority",
    ]
    for item in required:
        assert item in manifest
    assert f'"{PREFIX}_router_prompt_selection_fixed_case_set_static_validation_planned": true' in manifest
    ...
    false_flags = [
        "static_validator_created",
        "static_validation_run_enabled",
        "static_validation_output_created",
...
def test_mlrt63_readme_references_static_validation_focus_and_next_milestone() -> None:
    readme = read(README)
    ...
    assert "router prompt-selection fixed case set static validation planning only" in readme
    assert "1 step to the testing harness" in readme
    assert "2 steps to the first real ML prompt-selection test" in readme
    assert "No static validator, fixed case set, prompt-selection cases, labels, candidate outputs, scoring, route comparison, or report is created" in readme
...
if __name__ == "__main__":
    test_mlrt63_files_exist()
    test_mlrt63_preserves_prompt_selection_goal_and_countdown()
    test_mlrt63_only_allowed_python_surfaces_exist()
    test_mlrt63_stub_boundary_still_blocks_runtime_use()
    test_mlrt63_does_not_create_validators_cases_or_evaluation_outputs()
    test_mlrt63_manifest_records_static_validation_plan_without_unlocking_testing()
    test_mlrt63_readme_references_static_validation_focus_and_next_milestone()
    print(
        "CONTRACT_TEST_OK: MLRT-63 Router Prompt Selection Fixed Case Set Static Validation Plan v1, "
        "defined the future static validation plan for router prompt-selection fixed case sets after MLRT-62 freeze; preserved "
        "the final goal that ML must be tested for helping prompt selection in router prompt logic; after this milestone there "
        "is 1 step to the testing harness and 2 steps to the first real ML prompt-selection test; no static validator, no static validation run, "
        "no validation output, no fixed case set, no case files, no prompt-selection cases, no labels, no candidate outputs, no scoring, "
        "no route comparison, no report generation, no registry write, no registry mutation, no training-data intake, no dataset creation, "
        "no model training, no model calibration, no model improvement, no controlled learning experiment, no case execution, no route authority, "
        "no prompt loading, no provider calls, no embeddings, no persistence, no runtime Pilot, no Copilot behavior, critical boundary error budget zero."
    )
    print("SANDBOX_RSS_MLRT63_ROUTER_PROMPT_SELECTION_FIXED_CASE_SET_STATIC_VALIDATION_PLAN_V1_VALIDATION_OK")
