...
FEATURE_ID = "rss_ml_adv_phase6_guarded_runtime_advisory_display_final_safety_gate_v1"
PREFIX = "ml_advisory_phase6_guarded_runtime_advisory_display_final_safety_gate_v1_"


def test_phase6_final_safety_gate_manifest_gates():
    manifest = json.loads(
        Path("kanda_reasoner_app/routing_signal_scorer/box_manifest.json")
        .read_text(encoding="utf-8")
    )
    assert manifest[PREFIX + "feature_id"] == FEATURE_ID
    assert manifest[PREFIX + "reviewed_feature_id"] == "rss_ml_adv_phase6_guarded_runtime_advisory_display_implementation_result_review_gate_v1"
    assert manifest[PREFIX + "display_payload_builder_reviewed"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_read_only"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_bounded"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_in_memory"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_fail_open"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_removable_noop"] is True
    assert manifest[PREFIX + "display_payload_builder_locked_final_selection_invisible"] is True
...
if __name__ == "__main__":
    test_phase6_final_safety_gate_manifest_gates()
    print("VALIDATION OK: phase6 guarded runtime advisory display final safety gate manifest gates")
