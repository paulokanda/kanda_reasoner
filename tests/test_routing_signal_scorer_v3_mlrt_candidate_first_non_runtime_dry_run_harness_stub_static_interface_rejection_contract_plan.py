...
DOC = MLRT / "MLRT_21_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_REJECTION_CONTRACT_PLAN.md"
README = MLRT / "README.md"
MANIFEST = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "box_manifest.json"
FEATURE_ID = "routing_signal_scorer_v3_mlrt_candidate_first_non_runtime_dry_run_harness_stub_static_interface_rejection_contract_plan_v1"
ALLOWED_LAB_PY = {
...
def test_mlrt21_docs_exist() -> None:
    assert DOC.exists()
    assert README.exists()
    text = read(DOC)
    assert "MLRT-21 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Rejection Contract Plan v1" in text
    assert FEATURE_ID in text


def test_mlrt21_answers_tests_do_not_start_yet_and_estimates_steps() -> None:
    text = read(DOC)
    assert "Real candidate/dry-run tests must not start at MLRT-21." in text
    assert "testing likely starts only after roughly six more governed milestones" in text
    assert "not expected before approximately MLRT-26 or MLRT-27" in text
...
def test_mlrt21_is_rejection_contract_planning_only() -> None:
    text = read(DOC)
    required = [
        "documentation-only first non-runtime dry-run harness stub static-interface rejection-contract planning milestone",
        "still not testing",
        "not source code, not executable rejection logic, not an executable rejection gate",
...
def test_mlrt21_records_roadmap_progression() -> None:
    text = read(DOC)
    ...
    assert "MLRT-22 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Test-Start Readiness Gate Plan" in text


def test_mlrt21_defines_rejection_contract_planning_groups() -> None:
    text = read(DOC)
    for heading in [
        "When tests start",
...
def test_mlrt21_preserves_forbidden_boundaries() -> None:
    text = read(DOC)
    forbidden_phrases = [
...
def test_mlrt21_positive_label_is_planning_only() -> None:
    text = read(DOC)
    assert "MLRT_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_REJECTION_CONTRACT_PLAN_READY_FOR_TEST_START_READINESS_GATE_PLANNING_ONLY" in text
...
def test_mlrt21_has_no_python_implementation_files() -> None:
...
def test_mlrt21_does_not_create_forbidden_implementation_paths() -> None:
    forbidden_paths = [
        ...
        "static_interface_files", "abort_contract", "abort_contract_code", "abort_gate", "rejection_contract", "rejection_contract_code", "rejection_gate",
...
def test_box_manifest_records_mlrt21_rejection_contract_plan() -> None:
    data = json.loads(read(MANIFEST))
    prefix = "mlrt_candidate_first_non_runtime_dry_run_harness_stub_static_interface_rejection_contract_plan"
    assert data[f"{prefix}_feature_id"] == FEATURE_ID
    assert data[f"{prefix}_doc"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_21_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_REJECTION_CONTRACT_PLAN.md"
    assert data[f"{prefix}_readme"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/README.md"
    assert data[f"{prefix}_test"] == "tests/test_routing_signal_scorer_v3_mlrt_candidate_first_non_runtime_dry_run_harness_stub_static_interface_rejection_contract_plan.py"
    assert data[f"{prefix}_documentation_only"] is True
    assert data[f"{prefix}_rejection_contract_planning_only"] is True
    ...
    assert data[f"{prefix}_positive_label"] == "MLRT_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_REJECTION_CONTRACT_PLAN_READY_FOR_TEST_START_READINESS_GATE_PLANNING_ONLY"
    assert data[f"{prefix}_next_safe_milestone"] == "MLRT-22 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Test-Start Readiness Gate Plan after MLRT-21 freeze with FREEZE_MEMORY_STATUS OK"
    assert data[f"{prefix}_testing_estimate"] == "not expected before approximately MLRT-26 or MLRT-27 if no new safety gap appears"


def main() -> None:
    test_mlrt21_docs_exist()
    test_mlrt21_answers_tests_do_not_start_yet_and_estimates_steps()
    test_mlrt21_is_rejection_contract_planning_only()
    test_mlrt21_records_roadmap_progression()
    test_mlrt21_defines_rejection_contract_planning_groups()
    test_mlrt21_preserves_forbidden_boundaries()
    test_mlrt21_positive_label_is_planning_only()
    test_mlrt21_has_no_python_implementation_files()
    test_lab_python_surface_remains_exactly_three_files()
    test_mlrt21_does_not_create_forbidden_implementation_paths()
    test_box_manifest_records_mlrt21_rejection_contract_plan()
    print(
        "CONTRACT_TEST_OK: MLRT-21 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Rejection Contract Plan v1, "
        "immutable governed documentation-only harness stub static-interface rejection-contract planning after MLRT-20 freeze with FREEZE_MEMORY_STATUS OK, "
        "confirms tests must not start yet, estimates testing is not expected before approximately MLRT-26 or MLRT-27 if no new safety gap appears, "
        "and defines future non-authoritative rejection-contract planning groups for test-start boundary, rejection-contract identity, rejection categories, "
        "rejection output doctrine, abort-contract dependency, test-start readiness dependency, import policy, no-implementation doctrine, no-testing doctrine, "
        "no-reliability-claim doctrine, positive planning label, forbidden authority fields, and next-milestone doctrine only, candidate reliability not validated, "
        "ML implementation not unlocked, no Python source files created, no rejection-contract code created, no abort-contract code created, no executable rejection gate created, "
        "no executable abort gate created, no source surface created, no static interface created, no static interface files created, no executable validators, "
        "no schema code, no contract schema created, no boundary checker created, no harness created, no runner created, no harness stub files created, "
        "no implementation modules created, no dry run executed, no dry-run outputs created, no candidate execution, no candidate outputs created, no case execution, "
        "no case scoring, no route comparison, no route authority, no prompt loading, no provider calls, no embeddings, no network calls, no subprocess calls, "
        ...
        "next safe milestone is MLRT-22 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Test-Start Readiness Gate Plan"
    )
    print("SANDBOX_ROUTING_SIGNAL_SCORER_V3_MLRT_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_REJECTION_CONTRACT_PLAN_V1_VALIDATION_OK")
