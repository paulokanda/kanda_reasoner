...
from kanda_reasoner_app.routing_signal_scorer.generator_candidate_human_decision_recording_patch_design import (
    GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_PATCH_DESIGN_FEATURE_ID,
    GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_PATCH_DESIGN_SCHEMA_VERSION,
...
class GeneratorCandidateHumanDecisionRecordingPatchDesignTests(unittest.TestCase):
    def test_contract_validates(self):
        result = validate_generator_candidate_human_decision_recording_patch_design_contract(
            build_generator_candidate_human_decision_recording_patch_design_contract()
        )
        self.assertTrue(result["ok"])
        self.assertEqual(result["schema_id"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_PATCH_DESIGN_FEATURE_ID)
        self.assertEqual(result["schema_version"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_PATCH_DESIGN_SCHEMA_VERSION)

    def test_current_state_is_not_created_and_not_recorded(self):
        result = validate_generator_candidate_human_decision_recording_patch_design_contract(
            build_generator_candidate_human_decision_recording_patch_design_contract()
        )
        self.assertEqual(result["current_human_decision_recording_patch_design_state"], "decision_recording_patch_design_not_created")
        ...
        self.assertFalse(result["human_decision_recording_patch_design_created"])
    ...
    def test_no_decision_or_approval_authority(self):
        ...
        self.assertFalse(result["real_human_decision_recorded_by_human_decision_recording_patch_design"])
    ...
    def test_activation_request_is_denied(self):
        result = classify_generator_candidate_human_decision_recording_patch_design_request(
    ...
    def test_schema_request_is_review_only(self):
        result = classify_generator_candidate_human_decision_recording_patch_design_request(
            "show decision recording patch design schema and evidence checklist for review only"
        )
        self.assertTrue(result["allowed_now"])
        self.assertEqual(result["permitted_output"], "human_decision_recording_patch_design_schema")
        self.assertFalse(result["human_decision_recording_patch_created"])
        self.assertFalse(result["generator_candidate_patch_authorized"])

    def test_missing_required_field_fails(self):
        contract = build_generator_candidate_human_decision_recording_patch_design_contract()
        contract.pop("current_recorded_decision_value")
        result = validate_generator_candidate_human_decision_recording_patch_design_contract(contract)
    ...
    def test_manifest_declares_feature(self):
        ...
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_patch_design_feature_id"],
            "routing_signal_scorer_v3_generator_candidate_human_decision_recording_patch_design_v1",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_patch_design_schema_version"],
            "3.37-generator-candidate-human-decision-recording-patch-design",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_patch_design_status"],
            "standard_library_only_generator_candidate_human_decision_recording_patch_design_no_decision_write_no_candidate_patch_no_generator_no_runtime_behavior_change",
        )
    ...
    def test_manifest_protected_characteristics(self):
        ...
        for expected in (
            "generator_candidate_human_decision_recording_patch_design",
            "generator_candidate_human_decision_recording_patch_design_only",
            "no_real_human_decision_from_human_decision_recording_patch_design",
            "no_decision_write_from_human_decision_recording_patch_design",
            "no_decision_commit_from_human_decision_recording_patch_design",
            "no_decision_finalization_from_human_decision_recording_patch_design",
            "no_decision_recording_patch_creation_from_human_decision_recording_patch_design",
            "no_approval_inference_from_human_decision_recording_patch_design",
            "no_candidate_patch_from_human_decision_recording_patch_design",
            "no_generator_authorization_from_human_decision_recording_patch_design",
            "no_artifact_generation_from_human_decision_recording_patch_design",
            "no_source_scanning_from_human_decision_recording_patch_design",
            "future_decision_recording_after_human_decision_recording_patch_design_requires_separate_governed_patch",
        ):
            self.assertIn(expected, chars)

    def test_contract_and_init_do_not_export_patch_design(self):
    ...
    def test_design_doc_states_no_decision_write(self):
        text = (
            BOX / "design" / "routing_signal_scorer_v3_generator_candidate_human_decision_recording_patch_design.md"
        ).read_text(encoding="utf-8")
        self.assertIn("schema-only decision-recording patch design", text)
