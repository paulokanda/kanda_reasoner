...
def test_phase9_runtime_activation_final_safety_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "final_safety_gate_closes_phase9_runtime_activation_as_dormant_envelope_only_no_renderer_no_mounted_panel_no_route_authority"
...
def test_readme_records_phase9_runtime_activation_final_safety_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 9 read-only advisory panel runtime activation final safety gate" in text
    assert "dormant in-memory read-only activation-envelope foundation" in text
    assert "does not activate a renderer" in text
    assert "mount a panel" in text
    assert "Phase 9 completion handoff" in text


if __name__ == "__main__":
    test_phase9_runtime_activation_final_safety_gate_manifest_gates()
    test_readme_records_phase9_runtime_activation_final_safety_gate()
    print("VALIDATION OK: phase9 read-only advisory panel runtime activation final safety gate manifest gates")
