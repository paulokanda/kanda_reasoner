...
DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_61_ROUTER_PROMPT_SELECTION_OFFLINE_EVALUATION_CASE_SCHEMA_PLAN.md"
...
PREV_DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_60_GOLD_REGISTRY_SCHEMA_PROPOSAL_PLAN.md"
FEATURE_ID = "rss_mlrt61_router_prompt_selection_offline_evaluation_case_schema_plan_v1"
FEATURE_TITLE = "Routing Signal Scorer MLRT-61 Router Prompt Selection Offline Evaluation Case Schema Plan v1"
...
NEXT_TITLE = "Routing Signal Scorer MLRT-62 Router Prompt Selection Fixed Case Set Format Plan v1"
POS_LABEL = "RSS_MLRT61_PROMPT_SELECTION_OFFLINE_CASE_SCHEMA_PLANNED_NO_CASES_NO_EVALUATION"
PREFIX = "rss_mlrt61_router_prompt_selection_offline_case_schema_plan"
...
def test_mlrt61_files_exist() -> None:
    assert DOC.exists()
...
def test_mlrt61_preserves_prompt_selection_goal_and_countdown() -> None:
    text = read(DOC)
    required = [
        "ML must be tested for helping prompt selection in router prompt logic",
        "router prompt-selection offline evaluation case schema",
        "define what a future offline prompt-selection evaluation case must contain",
        "does **not** create the schema file",
        ...
        "3 steps to the testing harness",
        "4 steps to the first real ML prompt-selection test",
        "case_id",
        "case_version",
        "user_request_text",
        "normalized_user_intent",
        "required_context_signals",
        "forbidden_context_assumptions",
...
def test_mlrt61_only_allowed_python_surfaces_exist() -> None:
...
def test_mlrt61_stub_boundary_still_blocks_runtime_use() -> None:
    module = load_module(SOURCE, "mlrt61_stub_under_case_schema_plan_test")
...
def test_mlrt61_does_not_create_cases_or_evaluation_outputs() -> None:
    text = read(DOC)
    ...
    required_negative_claims = [
        "must not claim schema implementation readiness",
        "case corpus readiness",
...
def test_mlrt61_manifest_records_case_schema_plan_without_unlocking_testing() -> None:
    ...
    required = [
        ...
        POS_LABEL,
        NEXT_TITLE,
        "router_prompt_selection_offline_case_schema_plan_only_no_schema_file_no_cases_no_evaluation_no_runtime_authority",
    ]
    for item in required:
        assert item in manifest
    assert f'"{PREFIX}_router_prompt_selection_offline_case_schema_planned": true' in manifest
    ...
    false_flags = [
        ...
        "offline_evaluation_case_schema_file_created",
        "offline_evaluation_case_validator_created",
        ...
        "prompt_selection_gold_records_created",
...
def test_mlrt61_readme_references_case_schema_focus_and_next_milestone() -> None:
    readme = read(README)
    ...
    assert "router prompt-selection offline evaluation case schema planning only" in readme
    assert "3 steps to the testing harness" in readme
    assert "4 steps to the first real ML prompt-selection test" in readme
    assert "No schema file, prompt-selection cases, fixed case set, labels, candidate outputs, scoring, route comparison, or report is created" in readme
...
if __name__ == "__main__":
    test_mlrt61_files_exist()
    test_mlrt61_preserves_prompt_selection_goal_and_countdown()
    test_mlrt61_only_allowed_python_surfaces_exist()
    test_mlrt61_stub_boundary_still_blocks_runtime_use()
    test_mlrt61_does_not_create_cases_or_evaluation_outputs()
    test_mlrt61_manifest_records_case_schema_plan_without_unlocking_testing()
    test_mlrt61_readme_references_case_schema_focus_and_next_milestone()
    print(
        "CONTRACT_TEST_OK: MLRT-61 Router Prompt Selection Offline Evaluation Case Schema Plan v1, "
        "defined the future offline evaluation case schema plan for router prompt selection after MLRT-60 freeze; preserved "
        "the final goal that ML must be tested for helping prompt selection in router prompt logic; after this milestone there "
        "are 3 steps to the testing harness and 4 steps to the first real ML prompt-selection test; no schema file, no prompt-selection "
        "cases, no fixed case set, no labels, no candidate outputs, no scoring, no route comparison, no report generation, no registry write, "
    ...
    print("SANDBOX_RSS_MLRT61_ROUTER_PROMPT_SELECTION_OFFLINE_EVALUATION_CASE_SCHEMA_PLAN_V1_VALIDATION_OK")
