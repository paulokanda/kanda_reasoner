...
def test_phase7_surface_wiring_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_contract_for_future_read_only_wiring_no_runtime_activation_no_route_authority"
...
def test_readme_records_result_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 7 guarded advisory surface wiring contract result review gate" in text
    assert "does not activate router/app/UI wiring" in text
    assert "canonical router final selection first" in text
    assert "no direct route effect" in text


if __name__ == "__main__":
    test_phase7_surface_wiring_review_gate_manifest_gates()
    test_readme_records_result_review_gate()
    print("VALIDATION OK: phase7 surface wiring contract result review gate manifest gates")
