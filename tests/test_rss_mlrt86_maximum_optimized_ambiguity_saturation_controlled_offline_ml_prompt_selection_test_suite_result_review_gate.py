...
SANDBOX_MARKER = 'SANDBOX_RSS_MLRT86_MAXIMUM_OPTIMIZED_AMBIGUITY_SATURATION_CONTROLLED_OFFLINE_ML_PROMPT_SELECTION_TEST_SUITE_RESULT_REVIEW_GATE_V1_VALIDATION_OK'
...
def test_mlrt86_review_gate_contract() -> None:
    data = manifest()
    doc = read(DOC)
    readme = read(README)
    ...
    assert data[f"{PREFIX}_mlrt85_cases_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt85_cases_passed"] == 64
    assert data[f"{PREFIX}_mlrt85_ambiguity_pairs_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt85_audit_families_reviewed"] == 8
    assert data[f"{PREFIX}_mlrt85_cases_per_family_reviewed"] == 8
    assert data[f"{PREFIX}_mlrt85_governed_offline_review_only_cases_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt85_containment_no_authority_cases_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt85_unique_case_ids_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt85_unique_user_requests_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt85_forbidden_selected_routes_reviewed"] == 0
    assert data[f"{PREFIX}_cumulative_controlled_offline_cases_passed"] == 474
    assert data[f"{PREFIX}_cumulative_controlled_offline_test_stages_reviewed"] == 11
    ...
    assert data[f"{PREFIX}_identified_next_correction"] == "maximum_optimized_state_transition_evidence_recognition_offline_suite"
...
def main() -> None:
    test_mlrt86_review_gate_contract()
    test_mlrt86_non_runtime_project_surface()
