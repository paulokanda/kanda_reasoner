...
def test_mlrt80_files_and_prerequisites_exist() -> None:
    assert DOC.exists()
...
def test_mlrt80_manifest_reviews_mlrt79_without_claiming_runtime() -> None:
    data = manifest()
    assert data[f"{PREFIX}_feature_id"] == FEATURE_ID
    assert data[f"{PREFIX}_previous_suite_feature_id"] == "rss_mlrt79_maximum_optimized_differential_drift_controlled_offline_ml_prompt_selection_test_suite_v1"
    assert data[f"{PREFIX}_previous_suite_freeze_required_before_install"] is True
    assert data[f"{PREFIX}_mlrt79_result_reviewed"] is True
    assert data[f"{PREFIX}_mlrt79_differential_drift_cases_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt79_differential_drift_cases_passed"] == 64
    assert data[f"{PREFIX}_mlrt79_audit_families_reviewed"] == 8
    assert data[f"{PREFIX}_mlrt79_cases_per_family_reviewed"] == 8
    assert data[f"{PREFIX}_mlrt79_differential_drift_pairs_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt79_differential_drift_pairs_passed"] == 32
    assert data[f"{PREFIX}_mlrt79_unique_case_ids_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt79_unique_user_requests_reviewed"] == 64
    assert data[f"{PREFIX}_mlrt79_governed_offline_review_only_stable_cases_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt79_containment_no_authority_stable_cases_reviewed"] == 32
    assert data[f"{PREFIX}_mlrt79_forbidden_selected_routes_reviewed"] == 0
    assert data[f"{PREFIX}_cumulative_controlled_offline_cases_passed"] == 282
    assert data[f"{PREFIX}_cumulative_controlled_offline_test_stages_reviewed"] == 8
    assert data[f"{PREFIX}_review_gate_new_real_cases_added"] == 0
    assert data[f"{PREFIX}_differential_drift_result_reviewed"] is True
    ...
    assert data[f"{PREFIX}_identified_next_correction"] == "maximum_optimized_regression_metamorphic_consistency_offline_suite"
...
def test_mlrt80_documentation_records_review_result_and_next_correction() -> None:
    doc = read(DOC)
    readme = read(README)
...
def test_mlrt80_preserves_exact_python_source_surface() -> None:
...
def main() -> None:
    test_mlrt80_files_and_prerequisites_exist()
    test_mlrt80_manifest_reviews_mlrt79_without_claiming_runtime()
    test_mlrt80_documentation_records_review_result_and_next_correction()
    test_mlrt80_preserves_exact_python_source_surface()
    print(f"VALIDATION OK: {FEATURE_ID}")
    print(f"CONTRACT_TEST_OK: {CONTRACT_SUMMARY}")
    print("SANDBOX_RSS_MLRT80_MAXIMUM_OPTIMIZED_DIFFERENTIAL_DRIFT_CONTROLLED_OFFLINE_ML_PROMPT_SELECTION_TEST_SUITE_RESULT_REVIEW_GATE_V1_VALIDATION_OK")
