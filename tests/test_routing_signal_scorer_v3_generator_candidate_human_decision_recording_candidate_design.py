...
class GeneratorCandidateHumanDecisionRecordingCandidateDesignTests(unittest.TestCase):
    def test_contract_validates(self):
        contract = build_generator_candidate_human_decision_recording_candidate_contract()
        result = validate_generator_candidate_human_decision_recording_candidate_contract(contract)
        self.assertTrue(result["ok"])
        self.assertEqual(result["schema_id"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_CANDIDATE_FEATURE_ID)
        self.assertEqual(result["schema_version"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_CANDIDATE_SCHEMA_VERSION)

    def test_current_state_is_not_created_and_not_recorded(self):
        result = validate_generator_candidate_human_decision_recording_candidate_contract(
            build_generator_candidate_human_decision_recording_candidate_contract()
        )
        self.assertEqual(result["current_human_decision_recording_candidate_state"], "decision_recording_candidate_not_created")
        self.assertEqual(result["current_recorded_decision_value"], "not_recorded")
        self.assertFalse(result["human_decision_recording_candidate_created"])

    def test_no_decision_or_approval_authority(self):
        result = validate_generator_candidate_human_decision_recording_candidate_contract(
            build_generator_candidate_human_decision_recording_candidate_contract()
        )
        self.assertFalse(result["real_human_decision_recorded_by_human_decision_recording_candidate"])
        ...
        self.assertTrue(result["requires_explicit_human_confirmation_to_prepare_recording_candidate"])
    ...
    def test_activation_request_is_denied(self):
        result = classify_generator_candidate_human_decision_recording_candidate_request(
            "continue approve now record decision create candidate patch and generate artifact with embeddings"
        )
    ...
    def test_schema_request_is_review_only(self):
        result = classify_generator_candidate_human_decision_recording_candidate_request(
            "show recording candidate schema and evidence checklist for review only"
        )
        self.assertTrue(result["allowed_now"])
        self.assertEqual(result["permitted_output"], "human_decision_recording_candidate_schema")
        self.assertFalse(result["human_decision_recording_candidate_created"])
        self.assertFalse(result["generator_candidate_patch_authorized"])
    ...
    def test_manifest_declares_feature(self):
        ...
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_candidate_design_feature_id"],
            "routing_signal_scorer_v3_generator_candidate_human_decision_recording_candidate_design_v1",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_candidate_schema_version"],
            "3.32-generator-candidate-human-decision-recording-candidate-design",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_candidate_design_status"],
    ...
    def test_contract_and_init_do_not_export_candidate(self):
    ...
    def test_design_doc_states_no_decision_write(self):
        text = (
            BOX / "design" / "routing_signal_scorer_v3_generator_candidate_human_decision_recording_candidate_design.md"
        ).read_text(encoding="utf-8")
        self.assertIn("schema-only decision-recording-candidate", text)
