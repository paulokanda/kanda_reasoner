...
FEATURE_ID = "routing_signal_scorer_v3_mlrt_candidate_non_runtime_dry_run_readiness_gate_plan_v1"
DOC = MLRT / "MLRT_6_CANDIDATE_NON_RUNTIME_DRY_RUN_READINESS_GATE_PLAN.md"
...
def test_mlrt6_docs_exist() -> None:
    assert README.exists()
    assert DOC.exists()


def test_mlrt6_is_dry_run_readiness_gate_planning_only() -> None:
    text = read(DOC)
    assert FEATURE_ID in text
    assert "does not perform a dry run" in text
    assert "does not create dry-run readiness records" in text
    assert "does not create dry-run protocol records" in text
    assert "does not read static review outcome gates as live evidence" in text
    ...
    assert "There is no candidate reliability validation in MLRT-6 and there is no ML implementation unlock" in text
    assert "MLRT-7 - Candidate Non-Runtime Dry-Run Protocol Plan" in text


def test_mlrt6_defines_readiness_gate_groups() -> None:
    text = read(DOC)
    assert "Dry-run readiness gate identity and traceability doctrine" in text
    assert "Required static review outcome-gate reference doctrine" in text
    assert "Non-runtime dry-run scope doctrine" in text
    assert "Candidate package eligibility doctrine" in text
    assert "Dry-run protocol precondition doctrine" in text
    assert "Critical boundary blocker doctrine" in text
    assert "Human-escalation doctrine" in text
    assert "Dry-run protocol planning label doctrine" in text
    assert "Dry-run readiness non-authority doctrine" in text
    assert "Dry-run readiness forbidden authority fields" in text


def test_mlrt6_preserves_forbidden_boundaries() -> None:
    text = read(DOC)
    ...
    assert "Neither MLRT-5 nor MLRT-6 validates candidate reliability." in text


def test_mlrt6_positive_label_is_planning_only() -> None:
    text = read(DOC)
    assert "MLRT_DRY_RUN_READINESS_READY_FOR_DRY_RUN_PROTOCOL_PLANNING_ONLY" in text
    assert "does not mean that dry-run readiness has been performed" in text
    assert "does not mean readiness records exist" in text
    assert "does not mean a dry-run protocol exists" in text
...
def test_mlrt6_has_no_python_implementation_files() -> None:
...
def test_mlrt6_does_not_create_forbidden_implementation_paths() -> None:
    forbidden_paths = [
        "candidate",
        "candidate_package",
        "candidate_packages",
...
def test_box_manifest_records_mlrt6_boundary() -> None:
    data = json.loads(read(MANIFEST))
    prefix = "mlrt_candidate_non_runtime_dry_run_readiness_gate_plan"
    assert data[f"{prefix}_feature_id"] == FEATURE_ID
    assert data[f"{prefix}_doc"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_6_CANDIDATE_NON_RUNTIME_DRY_RUN_READINESS_GATE_PLAN.md"
    assert data[f"{prefix}_readme"] == "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/README.md"
    assert data[f"{prefix}_test"] == "tests/test_routing_signal_scorer_v3_mlrt_candidate_non_runtime_dry_run_readiness_gate_plan.py"
    assert data[f"{prefix}_documentation_only"] is True
    assert data[f"{prefix}_dry_run_readiness_gate_planning_only"] is True
    ...
    assert data[f"{prefix}_positive_label"] == "MLRT_DRY_RUN_READINESS_READY_FOR_DRY_RUN_PROTOCOL_PLANNING_ONLY"
    assert data[f"{prefix}_next_safe_milestone"] == "MLRT-7 Candidate Non-Runtime Dry-Run Protocol Plan after MLRT-6 freeze with FREEZE_MEMORY_STATUS OK"


def main() -> None:
    test_mlrt6_docs_exist()
    test_mlrt6_is_dry_run_readiness_gate_planning_only()
    test_mlrt6_defines_readiness_gate_groups()
    test_mlrt6_preserves_forbidden_boundaries()
    test_mlrt6_positive_label_is_planning_only()
    test_mlrt6_has_no_python_implementation_files()
    test_lab_python_surface_remains_exactly_three_files()
    test_mlrt6_does_not_create_forbidden_implementation_paths()
    test_box_manifest_records_mlrt6_boundary()
    print(
        "CONTRACT_TEST_OK: MLRT-6 Candidate Non-Runtime Dry-Run Readiness Gate Plan v1, "
        "immutable governed documentation-only dry-run readiness gate planning after MLRT-5 freeze with FREEZE_MEMORY_STATUS OK, "
        "defines future non-authoritative dry-run readiness gate groups for identity, static review outcome-gate references, non-runtime dry-run scope, candidate package eligibility, dry-run protocol preconditions, critical boundary blockers, human escalation, dry-run protocol planning labels, non-authority doctrine, forbidden authority fields, and next-milestone doctrine only, "
        ...
        "no dry-run readiness review performed, no dry-run readiness records created, no dry-run protocol records created, no dry run executed, no dry-run outputs created, "
        ...
        "critical boundary error budget zero, candidate dry-run readiness gate remains non-authoritative, "
        "next safe milestone is MLRT-7 Candidate Non-Runtime Dry-Run Protocol Plan"
    )
    print("SANDBOX_ROUTING_SIGNAL_SCORER_V3_MLRT_CANDIDATE_NON_RUNTIME_DRY_RUN_READINESS_GATE_PLAN_V1_VALIDATION_OK")
