...
HANDOFF_DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/ml_advisory_signal/ML_ADVISORY_PHASE9_READ_ONLY_ADVISORY_PANEL_RUNTIME_ACTIVATION_COMPLETION_HANDOFF_V1.md"
NEXT_DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/ml_advisory_signal/ML_ADVISORY_PHASE10_READ_ONLY_ADVISORY_PANEL_RENDERER_MOUNT_CONTRACT_READINESS_V1.md"
...
FEATURE_ID = "rss_ml_adv_phase9_read_only_advisory_panel_runtime_activation_completion_handoff_v1"
FINAL_GATE_ID = "rss_ml_adv_phase9_read_only_advisory_panel_runtime_activation_final_safety_gate_v1"
IMPLEMENTATION_ID = "rss_ml_adv_phase9_read_only_advisory_panel_runtime_activation_implementation_v1"
PREFIX = "ml_advisory_phase9_read_only_advisory_panel_runtime_activation_completion_handoff_v1_"
FINAL_PREFIX = "ml_advisory_phase9_read_only_advisory_panel_runtime_activation_final_safety_gate_v1_"
...
def test_phase9_runtime_activation_completion_handoff_closes_phase9_only() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "feature_id"] == FEATURE_ID
    assert data[PREFIX + "closed_feature_id"] == FINAL_GATE_ID
    assert data[PREFIX + "closed_implementation_feature_id"] == IMPLEMENTATION_ID
    assert data[PREFIX + "completion_handoff_only"] is True
    assert data[PREFIX + "phase9_runtime_activation_line_complete"] is True
    assert data[PREFIX + "phase9_closed_as_dormant_activation_envelope_foundation_only"] is True
    assert data[PREFIX + "visible_ml_integration_complete"] is False
    assert data[PREFIX + "phase10_renderer_mount_contract_required_next"] is True
    assert data[PREFIX + "accepted_dormant_envelope_only"] is True
    assert data[PREFIX + "accepted_in_memory_only"] is True
    assert data[PREFIX + "accepted_bounded"] is True
    ...
    assert data[PREFIX + "planned_next_step"] == "Routing Signal Scorer ML Advisory-Signal Phase 10 Read-Only Advisory Panel Renderer Mount Contract v1"
    assert data[PREFIX + "new_real_prompt_selection_cases_added"] == 0
    assert data[PREFIX + "critical_boundary_error_budget"] == 0


def test_phase9_runtime_activation_completion_handoff_preserves_final_gate_and_impl() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[FINAL_PREFIX + "feature_id"] == FINAL_GATE_ID
    assert data[FINAL_PREFIX + "phase9_runtime_activation_line_closed_as_dormant_envelope_only"] is True
    assert data[FINAL_PREFIX + "accepted_no_renderer_activation"] is True
    assert data[FINAL_PREFIX + "accepted_no_mounted_panel"] is True
    assert data[FINAL_PREFIX + "accepted_no_route_authority"] is True
    assert data[IMPL_PREFIX + "feature_id"] == IMPLEMENTATION_ID
...
def test_phase9_runtime_activation_completion_handoff_docs_define_phase10_as_contract_next() -> None:
    ...
    assert "closes the Phase 9 read-only advisory panel runtime activation line" in handoff_text
    assert "does not complete visible ML integration" in handoff_text
    assert "Phase 10 renderer/mount contract" in handoff_text
    assert "not a route authority change" in next_text
    assert "must not implement the actual visible runtime mount" in next_text
    assert "does not activate a renderer" in final_text
    assert "renderer adapter remains" in impl_text


if __name__ == "__main__":
    test_phase9_runtime_activation_completion_handoff_closes_phase9_only()
    test_phase9_runtime_activation_completion_handoff_preserves_final_gate_and_impl()
    test_phase9_runtime_activation_completion_handoff_docs_define_phase10_as_contract_next()
    print("VALIDATION OK: phase9 read-only advisory panel runtime activation completion handoff")
