...
def test_phase11_host_binding_completion_handoff_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "completion_handoff_closes_phase11_host_binding_descriptor_line_no_actual_host_binding_no_ui_mutation_no_route_authority"
    ...
    assert data[PREFIX + "phase10_renderer_mount_descriptor_lineage_only"] is True
    assert data[PREFIX + "bounded_host_bound_sections_only"] is True
...
def test_readme_records_phase11_completion_handoff_and_phase12_boundary() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 11 read-only advisory panel host binding completion handoff" in text
    assert "closes the Phase 11 host-binding descriptor line" in text
    assert "Actual runtime app-host visibility still requires a separate governed runtime app-host visibility contract" in text
    assert "Phase 12 Read-Only Advisory Panel Runtime App-Host Visibility Contract v1" in text


if __name__ == "__main__":
    test_phase11_host_binding_completion_handoff_manifest_gates()
    test_readme_records_phase11_completion_handoff_and_phase12_boundary()
    print("VALIDATION OK: phase11 read-only advisory panel host binding completion handoff manifest gates")
