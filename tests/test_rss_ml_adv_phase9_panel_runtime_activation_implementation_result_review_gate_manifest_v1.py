...
def test_phase9_runtime_activation_implementation_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_phase9_runtime_activation_implementation_no_renderer_no_mounted_panel_no_route_authority"
...
def test_readme_records_phase9_runtime_activation_implementation_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 9 read-only advisory panel runtime activation implementation result review gate" in text
    assert "guarded in-memory read-only activation envelope builder" in text
    assert "does not activate a renderer" in text
    assert "does not activate a renderer, mount a panel, mutate UI, wire runtime telemetry" in text
    assert "route authority" in text


if __name__ == "__main__":
    test_phase9_runtime_activation_implementation_review_gate_manifest_gates()
    test_readme_records_phase9_runtime_activation_implementation_review_gate()
    print("VALIDATION OK: phase9 read-only advisory panel runtime activation implementation result review gate manifest gates")
