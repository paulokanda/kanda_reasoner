"""Direct public import tests for the Tab 1 activity indicator."""
...
from kanda_reasoner_app.manage_architecture.ai_review.running_indicator import (  # noqa: E402
...
def test_indicator_is_hidden_when_idle_and_visible_when_running() -> None:
    """The indicator is not an idle blue widget when no work is running."""
    indicator = Tab1ActivityIndicator()

    assert indicator.widget().visible is False

    indicator.start_heuristic("validate")
    assert indicator.widget().visible is True

    indicator.finish_success("validate finished")
    assert indicator.widget().visible is False
...
def test_public_install_contract_sets_window_attribute() -> None:
    ...
    indicator = install_tab1_activity_indicator(window, layout)

    assert indicator is getattr(window, "_tab1_activity_indicator")
...
if __name__ == "__main__":
    ...
    print("Tab 1 running indicator public contract tests passed.")
