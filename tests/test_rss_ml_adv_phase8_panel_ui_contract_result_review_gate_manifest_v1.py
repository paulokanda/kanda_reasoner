...
def test_phase8_panel_ui_contract_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_panel_ui_contract_no_runtime_panel_activation_no_route_authority"
...
def test_readme_records_phase8_panel_ui_contract_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 8 read-only advisory panel UI contract result review gate" in text
    assert "contract-only, read-only, telemetry-only" in text
    assert "does not accept runtime panel activation" in text
    assert "no route authority" in text or "route authority" in text


if __name__ == "__main__":
    test_phase8_panel_ui_contract_review_gate_manifest_gates()
    test_readme_records_phase8_panel_ui_contract_review_gate()
    print("VALIDATION OK: phase8 read-only advisory panel UI contract result review gate manifest gates")
