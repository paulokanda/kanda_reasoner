...
PREFIX = "ml_advisory_phase13_read_only_advisory_panel_passive_visibility_activation_contract_result_review_gate_v1_"
CONTRACT_PREFIX = "ml_advisory_phase13_read_only_advisory_panel_passive_visibility_activation_contract_v1_"


def test_phase13_passive_visibility_activation_contract_result_review_gate_manifest():
    manifest = json.loads(Path("kanda_reasoner_app/routing_signal_scorer/box_manifest.json").read_text(encoding="utf-8"))
    assert manifest[PREFIX + "feature_id"] == "rss_ml_adv_phase13_read_only_advisory_panel_passive_visibility_activation_contract_result_review_gate_v1"
    assert manifest[PREFIX + "feature_title"] == "Routing Signal Scorer ML Advisory-Signal Phase 13 Read-Only Advisory Panel Passive Visibility Activation Contract Result Review Gate v1"
    assert manifest[PREFIX + "reviewed_feature_id"] == "rss_ml_adv_phase13_read_only_advisory_panel_passive_visibility_activation_contract_v1"
    ...
    assert manifest[PREFIX + "accepted_future_passive_visibility_activation_conditions_defined_only"] is True
    ...
    assert manifest[PREFIX + "accepted_bounded_passive_visibility_input_output"] is True
    assert manifest[PREFIX + "accepted_read_only_passive_visibility_slot_requirements"] is True
    assert manifest[PREFIX + "accepted_removable_noop_activation"] is True
    ...
    assert manifest[PREFIX + "planned_next_step"] == "Routing Signal Scorer ML Advisory-Signal Phase 13 Read-Only Advisory Panel Passive Visibility Activation Implementation v1"
    assert manifest[CONTRACT_PREFIX + "contract_result_review_gate_completed"] is True
    assert manifest[CONTRACT_PREFIX + "contract_result_review_gate_feature_id"] == "rss_ml_adv_phase13_read_only_advisory_panel_passive_visibility_activation_contract_result_review_gate_v1"
    assert manifest[CONTRACT_PREFIX + "implementation_required_next"] is True
    assert manifest[CONTRACT_PREFIX + "actual_passive_visibility_activation_enabled"] is False
    assert manifest[CONTRACT_PREFIX + "passive_visibility_slot_registration_enabled"] is False
    assert manifest[CONTRACT_PREFIX + "passive_visibility_slot_mutation_enabled"] is False
    assert manifest[CONTRACT_PREFIX + "visible_ml_integration_complete"] is False


if __name__ == "__main__":
    test_phase13_passive_visibility_activation_contract_result_review_gate_manifest()
    print("VALIDATION OK: phase13 read-only advisory panel passive visibility activation contract result review gate manifest gates")
