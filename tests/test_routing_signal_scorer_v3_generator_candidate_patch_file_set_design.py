...
MODULE = BOX / "generator_candidate_patch_file_set_design.py"
DESIGN_DOC = BOX / "design" / "routing_signal_scorer_v3_generator_candidate_patch_file_set_design.md"
...
class RoutingSignalScorerV3GeneratorCandidatePatchFileSetDesignTests(unittest.TestCase):
    def test_contract_validates_and_authorizes_nothing(self) -> None:
        contract = build_generator_candidate_patch_file_set_contract()
        result = validate_generator_candidate_patch_file_set_contract(contract)
        self.assertTrue(result["ok"], result["errors"])
        self.assertEqual(result["current_file_set_state"], "not_file_set_defined")
        self.assertFalse(result["real_human_decision_recorded_by_file_set"])
    ...
    def test_contract_identity_and_effect_are_schema_only(self) -> None:
        contract = build_generator_candidate_patch_file_set_contract()
        self.assertEqual(contract["schema_id"], GENERATOR_CANDIDATE_PATCH_FILE_SET_FEATURE_ID)
        self.assertEqual(contract["schema_version"], GENERATOR_CANDIDATE_PATCH_FILE_SET_SCHEMA_VERSION)
        self.assertTrue(contract["declared_generator_candidate_patch_file_set_only"])
        self.assertEqual(contract["file_set_scope"], "generator_candidate_patch_file_set_only")
        self.assertEqual(contract["current_file_set_state"], "not_file_set_defined")
        self.assertEqual(contract["current_file_set_effect"], "no_effect_schema_only_not_file_set_defined")
    ...
    def test_missing_required_field_fails_validation(self) -> None:
        ...
        mutated.pop("required_file_set_sections")
        result = validate_generator_candidate_patch_file_set_contract(mutated)
    ...
    def test_forbidden_output_field_fails_validation(self) -> None:
        ...
        result = validate_generator_candidate_patch_file_set_contract(mutated)
        self.assertFalse(result["ok"])
        self.assertTrue(any("forbidden file_set field" in error for error in result["errors"]))
    ...
    def test_empty_required_lists_fail_validation(self) -> None:
        contract = build_generator_candidate_patch_file_set_contract()
        mutated = dict(contract)
        mutated["required_file_set_sections"] = []
        result = validate_generator_candidate_patch_file_set_contract(mutated)
        self.assertFalse(result["ok"])
        self.assertTrue(any("required_file_set_sections missing" in error for error in result["errors"]))

    def test_classification_allows_schema_view_only(self) -> None:
        allowed = classify_generator_candidate_patch_file_set_request(
            "show the candidate patch file-set schema sections declarations"
        )
        self.assertTrue(allowed["allowed_now"])
        self.assertEqual(allowed["permitted_output"], "generator_candidate_patch_file_set_schema")
        self.assertEqual(allowed["current_file_set_state"], "not_file_set_defined")
    ...
    def test_classification_blocks_candidate_patch_and_generation_requests(self) -> None:
        blocked = classify_generator_candidate_patch_file_set_request(
    ...
    def test_manifest_public_contract_and_runtime_paths_are_safe(self) -> None:
        ...
        self.assertEqual(
            manifest["generator_candidate_patch_file_set_design_feature_id"],
            GENERATOR_CANDIDATE_PATCH_FILE_SET_FEATURE_ID,
        )
        self.assertEqual(
            manifest["generator_candidate_patch_file_set_schema_version"],
            GENERATOR_CANDIDATE_PATCH_FILE_SET_SCHEMA_VERSION,
        )
