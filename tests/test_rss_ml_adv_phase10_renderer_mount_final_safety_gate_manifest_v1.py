...
def test_phase10_renderer_mount_final_safety_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "final_safety_gate_only_closes_phase10_renderer_mount_line_for_completion_handoff_no_runtime_ui_mutation_no_route_authority"
...
def test_readme_records_phase10_renderer_mount_final_safety_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 10 read-only advisory panel renderer mount final safety gate" in text
    assert "safe prerequisite for a completion handoff" in text
    assert "does not introduce runtime UI mutation" in text
    assert "must not grant route authority" in text


if __name__ == "__main__":
    test_phase10_renderer_mount_final_safety_gate_manifest_gates()
    test_readme_records_phase10_renderer_mount_final_safety_gate()
    print("VALIDATION OK: phase10 read-only advisory panel renderer mount final safety gate manifest gates")
