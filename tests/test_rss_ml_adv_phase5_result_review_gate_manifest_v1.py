...
PREFIX = 'ml_advisory_phase5_real_adapter_boundary_result_review_gate_v1_'
...
REVIEWED_FEATURE_ID = 'rss_ml_adv_phase5_offline_real_adapter_boundary_contract_v1'
...
def test_manifest_registers_phase5_review_gate_and_next_step():
    ...
    assert manifest[PREFIX + 'phase'] == 'phase_5_real_adapter_boundary_result_review_gate'
    assert manifest[PREFIX + 'status'] == 'review_gate_only_no_adapter_execution_no_runtime_ml_no_route_authority_no_router_prompt_logic_change'
    assert manifest[PREFIX + 'doc'].endswith('ML_ADVISORY_PHASE5_OFFLINE_REAL_ADAPTER_BOUNDARY_RESULT_REVIEW_GATE_V1.md')
    assert manifest[PREFIX + 'phase5_candidate_readiness_doc'].endswith('ML_ADVISORY_PHASE5_OFFLINE_REAL_ADAPTER_CANDIDATE_CONTRACT_READINESS_V1.md')


def test_manifest_forbids_all_runtime_adapter_and_authority_capabilities():
    manifest = load_manifest()
    false_suffixes = [
        'mlrt_113_created',
        'real_ml_enabled',
...
def test_manifest_validation_tests_are_local_to_review_gate():
    ...
    assert tests == [
        'tests/test_rss_ml_adv_phase5_result_review_gate_v1.py',
        'tests/test_rss_ml_adv_phase5_result_review_gate_manifest_v1.py',
    ]


if __name__ == '__main__':
    test_manifest_registers_phase5_review_gate_and_next_step()
    test_manifest_forbids_all_runtime_adapter_and_authority_capabilities()
    test_manifest_validation_tests_are_local_to_review_gate()
    print('VALIDATION OK: rss_ml_adv_phase5_real_adapter_boundary_result_review_gate_v1_manifest')
