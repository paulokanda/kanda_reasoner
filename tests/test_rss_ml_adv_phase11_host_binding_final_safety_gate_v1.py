...
DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE11_READ_ONLY_ADVISORY_PANEL_HOST_BINDING_FINAL_SAFETY_GATE_V1.md"
NEXT_DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE11_READ_ONLY_ADVISORY_PANEL_HOST_BINDING_COMPLETION_HANDOFF_READINESS_V1.md"
...
REVIEW_GATE_DOC = ROOT / "kanda_reasoner_app" / "routing_signal_scorer" / "ml_advisory_signal" / "ML_ADVISORY_PHASE11_READ_ONLY_ADVISORY_PANEL_HOST_BINDING_IMPLEMENTATION_RESULT_REVIEW_GATE_V1.md"

FEATURE_ID = "rss_ml_adv_phase11_read_only_advisory_panel_host_binding_final_safety_gate_v1"
REVIEWED_FEATURE_ID = "rss_ml_adv_phase11_read_only_advisory_panel_host_binding_implementation_result_review_gate_v1"
PREFIX = "ml_advisory_phase11_read_only_advisory_panel_host_binding_final_safety_gate_v1_"
IMPL_PREFIX = "ml_advisory_phase11_read_only_advisory_panel_host_binding_implementation_v1_"
REVIEW_PREFIX = "ml_advisory_phase11_read_only_advisory_panel_host_binding_implementation_result_review_gate_v1_"
...
def test_phase11_host_binding_final_safety_gate_accepts_only_safe_line_for_handoff() -> None:
    ...
    assert data[PREFIX + "accepted_removable_noop_host_binding_descriptor"] is True
    ...
    assert data[PREFIX + "planned_next_step"] == "Routing Signal Scorer ML Advisory-Signal Phase 11 Read-Only Advisory Panel Host Binding Completion Handoff v1"
    assert data[PREFIX + "new_real_prompt_selection_cases_added"] == 0
    assert data[PREFIX + "critical_boundary_error_budget"] == 0


def test_phase11_host_binding_final_safety_gate_updates_prior_state_without_authority() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
...
def test_phase11_host_binding_final_safety_gate_docs_define_completion_handoff_only() -> None:
...
if __name__ == "__main__":
    test_phase11_host_binding_final_safety_gate_accepts_only_safe_line_for_handoff()
    test_phase11_host_binding_final_safety_gate_updates_prior_state_without_authority()
    test_phase11_host_binding_final_safety_gate_docs_define_completion_handoff_only()
    print("VALIDATION OK: phase11 read-only advisory panel host binding final safety gate")
