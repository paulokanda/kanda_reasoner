...
def test_phase10_renderer_mount_contract_result_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_phase10_renderer_mount_contract_no_actual_mount_no_ui_mutation_no_route_authority"
...
def test_readme_records_phase10_renderer_mount_contract_result_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 10 read-only advisory panel renderer mount contract result review gate" in text
    assert "not visible ML integration" in text
    assert "not actual renderer activation" in text
    assert "Phase 10 Read-Only Advisory Panel Renderer Mount" in text


if __name__ == "__main__":
    test_phase10_renderer_mount_contract_result_review_gate_manifest_gates()
    test_readme_records_phase10_renderer_mount_contract_result_review_gate()
    print("VALIDATION OK: phase10 read-only advisory panel renderer mount contract result review gate manifest gates")
