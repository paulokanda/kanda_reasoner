...
class GeneratorCandidateHumanDecisionRecordingFinalizationClosureShieldTests(unittest.TestCase):
    def test_contract_validates(self):
        contract = build_generator_candidate_human_decision_recording_finalization_closure_shield_contract()
        result = validate_generator_candidate_human_decision_recording_finalization_closure_shield_contract(contract)
        self.assertTrue(result["ok"])
        self.assertEqual(result["schema_id"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_FINALIZATION_CLOSURE_SHIELD_FEATURE_ID)
        self.assertEqual(result["schema_version"], GENERATOR_CANDIDATE_HUMAN_DECISION_RECORDING_FINALIZATION_CLOSURE_SHIELD_SCHEMA_VERSION)

    def test_current_state_is_closed_and_not_recorded(self):
        result = validate_generator_candidate_human_decision_recording_finalization_closure_shield_contract(
            build_generator_candidate_human_decision_recording_finalization_closure_shield_contract()
        )
        self.assertEqual(result["current_human_decision_recording_finalization_closure_shield_state"], "decision_recording_finalization_closure_shield_closed")
    ...
    def test_no_decision_or_approval_authority(self):
        ...
        self.assertFalse(result["real_human_decision_recorded_by_human_decision_recording_finalization_closure_shield"])
        ...
        self.assertTrue(result["requires_explicit_human_confirmation_to_open_future_decision_recording_patch"])
    ...
    def test_activation_request_is_denied(self):
        result = classify_generator_candidate_human_decision_recording_finalization_closure_shield_request(
    ...
    def test_schema_request_is_review_only(self):
        result = classify_generator_candidate_human_decision_recording_finalization_closure_shield_request(
            "show finalization closure shield schema and evidence checklist for review only"
        )
        self.assertTrue(result["allowed_now"])
        self.assertEqual(result["permitted_output"], "human_decision_recording_finalization_closure_shield_schema")
        self.assertFalse(result["human_decision_recording_patch_created"])
        self.assertFalse(result["generator_candidate_patch_authorized"])
    ...
    def test_manifest_declares_feature(self):
        ...
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_finalization_closure_shield_feature_id"],
            "routing_signal_scorer_v3_generator_candidate_human_decision_recording_finalization_closure_shield_v1",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_finalization_closure_shield_schema_version"],
            "3.36-generator-candidate-human-decision-recording-finalization-closure-shield",
        )
        self.assertEqual(
            manifest["generator_candidate_human_decision_recording_finalization_closure_shield_status"],
    ...
    def test_manifest_protected_characteristics(self):
        ...
        for expected in (
            ...
            "no_approval_inference_from_human_decision_recording_finalization_closure_shield",
    ...
    def test_contract_and_init_do_not_export_closure_shield(self):
    ...
    def test_design_doc_states_no_decision_write(self):
        text = (
            BOX / "design" / "routing_signal_scorer_v3_generator_candidate_human_decision_recording_finalization_closure_shield.md"
        ).read_text(encoding="utf-8")
        self.assertIn("schema-only decision-recording-finalization closure shield", text)
