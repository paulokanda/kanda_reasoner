...
def test_phase12_visibility_implementation_result_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_phase12_runtime_app_host_visibility_implementation_for_final_safety_gate_no_actual_visibility_no_ui_mutation_no_route_authority"
    ...
    assert data[PREFIX + "runtime_panel_mount_side_effects_enabled"] is False
    assert data[PREFIX + "runtime_ui_mutation_enabled"] is False
    assert data[PREFIX + "runtime_telemetry_surface_wiring_enabled"] is False
    for flag in REQUIRED_FALSE:
        assert data[PREFIX + flag] is False, flag


def test_readme_records_phase12_visibility_implementation_result_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 12 read-only advisory panel runtime app-host visibility implementation result review gate" in text
    assert "not actual runtime app-host visibility side effects" in text
    assert "does not mutate runtime UI" in text
    assert "Final Safety Gate v1" in text


if __name__ == "__main__":
    test_phase12_visibility_implementation_result_review_gate_manifest_gates()
    test_readme_records_phase12_visibility_implementation_result_review_gate()
    print("VALIDATION OK: phase12 read-only advisory panel runtime app-host visibility implementation result review gate manifest gates")
