...
def test_phase11_host_binding_implementation_result_review_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "review_gate_only_accepts_phase11_host_binding_implementation_for_final_safety_gate_no_actual_host_binding_no_ui_mutation_no_route_authority"
...
def test_readme_records_phase11_host_binding_implementation_result_review_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 11 read-only advisory panel host binding implementation result review gate" in text
    assert "not actual host-binding side effects" in text
    assert "does not mutate runtime UI" in text
    assert "Final Safety Gate v1" in text


if __name__ == "__main__":
    test_phase11_host_binding_implementation_result_review_gate_manifest_gates()
    test_readme_records_phase11_host_binding_implementation_result_review_gate()
    print("VALIDATION OK: phase11 read-only advisory panel host binding implementation result review gate manifest gates")
