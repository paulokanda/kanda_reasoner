...
def test_phase11_host_binding_final_safety_gate_manifest_gates() -> None:
    data = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert data[PREFIX + "status"] == "final_safety_gate_only_closes_phase11_host_binding_line_for_completion_handoff_no_actual_host_binding_no_ui_mutation_no_route_authority"
...
def test_readme_records_phase11_host_binding_final_safety_gate() -> None:
    text = README.read_text(encoding="utf-8")
    assert "Phase 11 read-only advisory panel host binding final safety gate" in text
    assert "safe prerequisite for a completion handoff" in text
    assert "does not introduce actual host-binding side effects" in text
    assert "must not grant route authority" in text


if __name__ == "__main__":
    test_phase11_host_binding_final_safety_gate_manifest_gates()
    test_readme_records_phase11_host_binding_final_safety_gate()
    print("VALIDATION OK: phase11 read-only advisory panel host binding final safety gate manifest gates")
