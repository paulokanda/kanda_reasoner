...
FEATURE_ID = "rss_ml_adv_phase6_guarded_runtime_advisory_display_implementation_v1"
PREFIX = "ml_advisory_phase6_guarded_runtime_advisory_display_implementation_v1_"


def test_phase6_display_implementation_manifest_gates():
    manifest = json.loads(
        Path("kanda_reasoner_app/routing_signal_scorer/box_manifest.json")
        .read_text(encoding="utf-8")
    )
    assert manifest[PREFIX + "feature_id"] == FEATURE_ID
    assert manifest[PREFIX + "runtime_display_implementation_enabled"] is True
    assert manifest[PREFIX + "runtime_telemetry_payload_builder_enabled"] is True
    ...
    assert manifest[PREFIX + "runtime_copilot_behavior_enabled"] is False
...
if __name__ == "__main__":
    test_phase6_display_implementation_manifest_gates()
    print("VALIDATION OK: phase6 display implementation manifest gates")
