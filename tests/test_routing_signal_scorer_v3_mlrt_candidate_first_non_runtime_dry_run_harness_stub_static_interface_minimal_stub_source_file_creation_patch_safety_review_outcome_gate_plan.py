...
DOC = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/MLRT_30_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_MINIMAL_STUB_SOURCE_FILE_CREATION_PATCH_SAFETY_REVIEW_OUTCOME_GATE_PLAN.md"
README = ROOT / "kanda_reasoner_app/routing_signal_scorer/mlrt_non_runtime_candidate_reliability/README.md"
MANIFEST = ROOT / "kanda_reasoner_app/routing_signal_scorer/box_manifest.json"
FEATURE_ID = "routing_signal_scorer_v3_mlrt_candidate_first_non_runtime_dry_run_harness_stub_static_interface_minimal_stub_source_file_creation_patch_safety_review_outcome_gate_plan_v1"
...
def test_mlrt30_docs_exist() -> None:
    assert DOC.exists()
    assert README.exists()
    text = read(DOC)
    assert "MLRT-30 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Safety Review Outcome Gate Plan v1" in text
    assert FEATURE_ID in text


def test_mlrt30_tests_do_not_start_yet() -> None:
    text = read(DOC)
    assert "Real candidate/dry-run tests must not start at MLRT-30." in text
    assert "does not create a source file" in text
    assert "does not authorize creating a source file" in text
    assert "testing is now not expected before MLRT-34 or later" in text
    assert "final first-dry-run execution gate" in text


def test_mlrt30_is_outcome_gate_planning_only() -> None:
    text = read(DOC)
    required = [
        "documentation-only first non-runtime dry-run harness stub static-interface minimal stub source-file creation patch safety-review outcome-gate planning milestone",
        "not source code",
        "not a source-file creation patch",
        "not a source-file safety review run",
        "not a safety-review outcome record",
        "not a safety-review approval",
        "not source-file authorization",
        "not a source creation approval",
...
def test_mlrt30_records_roadmap_progression() -> None:
    text = read(DOC)
    ...
    assert "MLRT-31 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Final Human Review Gate Plan" in text


def test_mlrt30_defines_future_outcome_gate_sections() -> None:
    text = read(DOC)
    for heading in [
        "When tests start",
        "Planned outcome-gate inputs",
        "Planned future source path",
        "Planned outcome-gate checklist",
        "Planned outcome-gate required rejection reasons",
        "Planned outcome-gate allowed outcome vocabulary",
        "Planned outcome-gate forbidden outcome vocabulary",
...
def test_mlrt30_future_inputs_are_read_only_planning() -> None:
    text = read(DOC)
    for phrase in [
        ...
        "Future safety-review rejection reasons",
        "Future human review notes",
        "MLRT-30 does not create, fetch, inspect, or validate those inputs",
    ]:
        assert phrase in text


def test_mlrt30_future_source_path_is_not_created_or_authorized() -> None:
    text = read(DOC)
    assert PLANNED_SOURCE in text
    assert "MLRT-30 does not create this path" in text
    assert "does not create the parent folder" in text
    assert "does not authorize creating this path" in text


def test_mlrt30_outcome_gate_checklist_is_safe() -> None:
    text = read(DOC)
    for phrase in [
        "MLRT-28, MLRT-29, and MLRT-30 are frozen",
...
def test_mlrt30_rejection_reasons_are_comprehensive() -> None:
    text = read(DOC)
    for reason in [
        ...
        "safety_review_claims_runtime_approval",
        "safety_review_claims_reliability",
...
def test_mlrt30_outcomes_remain_non_authoritative() -> None:
    text = read(DOC)
    for allowed in [
        "OUTCOME_GATE_NOT_RUN",
        "OUTCOME_GATE_REJECTED",
        "OUTCOME_GATE_ABORTED",
        "OUTCOME_GATE_READY_FOR_FINAL_HUMAN_REVIEW_GATE_PLANNING_ONLY",
    ]:
        assert allowed in text
    for forbidden in [
        "SOURCE_FILE_CREATED_BY_OUTCOME_GATE",
...
def test_mlrt30_positive_label_is_planning_only() -> None:
    text = read(DOC)
    assert "MLRT_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_MINIMAL_STUB_SOURCE_FILE_CREATION_PATCH_SAFETY_REVIEW_OUTCOME_GATE_PLAN_READY_FOR_FINAL_HUMAN_REVIEW_GATE_PLANNING_ONLY" in text
    ...
    assert "does not mean an outcome-gate result exists" in text
...
def test_mlrt30_manifest_records_planning_only_boundaries() -> None:
    import json

    data = json.loads(read(MANIFEST))
    prefix = "mlrt_candidate_first_non_runtime_dry_run_harness_stub_static_interface_minimal_stub_source_file_creation_patch_safety_review_outcome_gate_plan_"
    assert data[prefix + "feature_id"] == FEATURE_ID
    assert data[prefix + "documentation_only"] is True
    assert data[prefix + "outcome_gate_planning_only"] is True
    ...
    assert data[prefix + "next_safe_milestone"] == "MLRT-31 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Final Human Review Gate Plan after MLRT-30 freeze with FREEZE_MEMORY_STATUS OK"


def test_mlrt30_has_no_python_implementation_files() -> None:
...
def test_mlrt30_does_not_create_forbidden_implementation_paths() -> None:
    forbidden_paths = [
...
def test_mlrt30_readme_and_manifest_reference_feature() -> None:
    readme = read(README)
    manifest = read(MANIFEST)
    assert "MLRT-30 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Safety Review Outcome Gate Plan v1" in readme
    assert FEATURE_ID in manifest
    assert "MLRT-31 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Final Human Review Gate Plan" in readme
    assert "Tests do not start at MLRT-30" in readme


if __name__ == "__main__":
    test_mlrt30_docs_exist()
    test_mlrt30_tests_do_not_start_yet()
    test_mlrt30_is_outcome_gate_planning_only()
    test_mlrt30_records_roadmap_progression()
    test_mlrt30_defines_future_outcome_gate_sections()
    test_mlrt30_future_inputs_are_read_only_planning()
    test_mlrt30_future_source_path_is_not_created_or_authorized()
    test_mlrt30_outcome_gate_checklist_is_safe()
    test_mlrt30_rejection_reasons_are_comprehensive()
    test_mlrt30_outcomes_remain_non_authoritative()
    test_mlrt30_positive_label_is_planning_only()
    test_mlrt30_manifest_records_planning_only_boundaries()
    test_mlrt30_has_no_python_implementation_files()
    test_lab_python_surface_remains_exactly_three_files()
    test_mlrt30_does_not_create_forbidden_implementation_paths()
    test_mlrt30_readme_and_manifest_reference_feature()
    print(
        "CONTRACT_TEST_OK: MLRT-30 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Safety Review Outcome Gate Plan v1, "
        "immutable governed documentation-only minimal stub source-file creation patch safety-review outcome-gate planning after MLRT-29 freeze with FREEZE_MEMORY_STATUS OK, "
        "confirms tests must not start yet, testing not expected before MLRT-34 or later if no new safety gap appears, "
        ...
        "next safe milestone is MLRT-31 Candidate First Non-Runtime Dry-Run Harness Stub Static Interface Minimal Stub Source File Creation Patch Final Human Review Gate Plan"
    )
    print("SANDBOX_ROUTING_SIGNAL_SCORER_V3_MLRT_CANDIDATE_FIRST_NON_RUNTIME_DRY_RUN_HARNESS_STUB_STATIC_INTERFACE_MINIMAL_STUB_SOURCE_FILE_CREATION_PATCH_SAFETY_REVIEW_OUTCOME_GATE_PLAN_V1_VALIDATION_OK")
