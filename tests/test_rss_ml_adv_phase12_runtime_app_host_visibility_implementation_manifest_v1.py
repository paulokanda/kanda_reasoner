...
PREFIX = "ml_advisory_phase12_read_only_advisory_panel_runtime_app_host_visibility_implementation_v1_"
MANIFEST = Path("kanda_reasoner_app/routing_signal_scorer/box_manifest.json")


def test_phase12_visibility_implementation_manifest_gates():
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    assert manifest[PREFIX + "feature_id"] == "rss_ml_adv_phase12_read_only_advisory_panel_runtime_app_host_visibility_implementation_v1"
    assert manifest[PREFIX + "feature_title"] == "Routing Signal Scorer ML Advisory-Signal Phase 12 Read-Only Advisory Panel Runtime App-Host Visibility Implementation v1"
    assert manifest[PREFIX + "source_contract_result_review_gate_feature_id"] == "rss_ml_adv_phase12_read_only_advisory_panel_runtime_app_host_visibility_contract_result_review_gate_v1"
    assert manifest[PREFIX + "source_host_binding_implementation_feature_id"] == "rss_ml_adv_phase11_read_only_advisory_panel_host_binding_implementation_v1"
    assert manifest[PREFIX + "implementation_status"] == "feature_flagged_default_off_read_only_runtime_app_host_visibility_descriptor_builder"
    ...
    assert manifest[PREFIX + "read_only_visibility_descriptor_builder_enabled"] is True
    assert manifest[PREFIX + "runtime_app_host_visibility_descriptor_ready"] is True
    ...
    assert manifest[PREFIX + "planned_next_step"] == "Routing Signal Scorer ML Advisory-Signal Phase 12 Read-Only Advisory Panel Runtime App-Host Visibility Implementation Result Review Gate v1"


if __name__ == "__main__":
    test_phase12_visibility_implementation_manifest_gates()
    print("VALIDATION OK: phase12 read-only advisory panel runtime app-host visibility implementation manifest gates")
