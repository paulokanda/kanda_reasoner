...
class T4Q038CV10HelperManifestAndStaleArtifactCleanupTests(unittest.TestCase):
    """Validate targeted v10 helper manifest cleanup and stale artifact removal."""
    ...
    def test_target_helper_groups_have_no_stale_exports(self) -> None:
        cases = [
            (Path(MAIN_MANIFEST), "main_window"),
            (Path(RETRIEVER_MANIFEST), "reasoner_retriever"),
        ]
        for path, group in cases:
            data = json.loads(path.read_text(encoding="utf-8"))
            helpers = data.get("helpers", {})
            if isinstance(helpers, dict):
                for key in [group, group + ".py"]:
                    helper_data = helpers.get(key)
                    if isinstance(helper_data, dict):
                        exports = helper_data.get("exports")
                        if exports is not None:
                            self.assertEqual(exports, [], str(path) + " " + group)
